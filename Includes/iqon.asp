...
// 3a) ProcesaBuscadorDeParametros(sValor)  ROG 26/12/2018
...
// 3a)  en base a un string carga los parametros y les pone su valor
...
function ParametroDeVentana(sys, mnu, nombre, Valdefault){

     var Resultado = Valdefault
	 if( sys > -1 ) {
		 var sSQL = "Select Prm_Valor from Menu_Parametro "
			 sSQL += " where Sys_ID = " + sys
			 sSQL += " and Mnu_ID = " + mnu
			 sSQL += " and Usu_ID = -1 "  		 
			 sSQL += " and Prm_Nombre = '" + nombre + "'"		 
						 
		 var rsPrm = AbreTabla(sSQL,1,2)
		 if (!rsPrm.EOF){
			Resultado = "" + rsPrm.Fields.Item("Prm_Valor").Value 
			if( EsVacio(Resultado) ){
				Resultado = "" + Valdefault
			}
		 } else {
			 var sSQL = "INSERT INTO Menu_Parametro(Sys_ID, Mnu_ID, Usu_ID"
			     sSQL += ", Prm_Nombre, Prm_Valor)"
				 sSQL += " VALUES ( " + sys + ", " + mnu + ","
				 sSQL += " -1,'" + nombre + "'," + Valdefault + ")"
...
function ParametroDeVentanaConUsuario(sys, mnu, usu, nombre, Valdefault){

     var Resultado = Valdefault
	 if( sys > -1 ) {
		 var sSQL = "Select Prm_Valor, Usu_ID from Menu_Parametro "
			 sSQL += " where Sys_ID = " + sys
			 sSQL += " and Mnu_ID = " + mnu
			 sSQL += " and Usu_ID = " + usu
			 sSQL += " and Prm_Nombre = '" + nombre + "'"		 
		 ...
		 var rsPrm = AbreTabla(sSQL,1,2)
		 if (!rsPrm.EOF){
		 	if(rsPrm.Fields.Item("Usu_ID").Value == usu) {
				Resultado = "" + rsPrm.Fields.Item("Prm_Valor").Value 
			}
			if( EsVacio(Resultado) ){
				Resultado = "" + Valdefault
...
//			 var sSQL = "IF NOT EXIST (SELECT 1 FROM Menu_Parametro "
//			     sSQL += " WHERE Sys_ID = " + sys
//			     sSQL += " and Mnu_ID = " + mnu
//			     sSQL += " and Prm_Nombre = '" + nombre + "'")
			 var sSQL = "INSERT INTO Menu_Parametro(Sys_ID, Mnu_ID, Usu_ID"
...
//10b
function EjecutaObjetoBD(sConsultaSQL,Conexion) {

	var rsSP = AbreTabla(sConsultaSQL,1,Conexion)
	var sdato = rsSP.Fields.Item(0).Value 
	rsSP.Close()
	...
	for (i=0;i<rspcdsql.fields.count;i++) {
		if (!rspcdsql.EOF) { 
			if (bHayParametros) {
				...
				ParametroCambiaValor(String(rspcdsql.Fields.Item(i).Name),String(rspcdsql.Fields.Item(i).Value))  } 
	catch(err) {  ParametroCambiaValor(String(rspcdsql.Fields.Item(i).Name) ,"error en " + String(rspcdsql.Fields.Item(i).Name)) 
	Response.Write("No se pudo cargar a parametro el campo " + String(rspcdsql.Fields.Item(i).Name)) }
			}
			ParametroCambiaTipo(String(rspcdsql.Fields.Item(i).Name),String(rspcdsql.Fields.Item(i).Type),String(rspcdsql.Fields.Item(i).DefinedSize))
		} else {
			SigP = Parametros.length
...
function DespliegaAlPie() {
	var bDebug = ParametroDeVentanaConUsuario(SistemaActual, -1, IDUsuario, "Despliega al pie el debug", 0)
	 if (bDebug == 1) {
	 	bPuedeVerDebug = true
...
arCamposBD = (Campos)
var sSQLTD = "Select TOP 1 * from " + Tabla
...
var arCamposBD = new Array(0)
arCamposBD = (Campos)
var sSQLTDE = "Select TOP 1 * from " + Tabla
...
//	Response.Write("iqon_2199) iIdSeg&nbsp;"+iIdSeg + "&nbsp;iIdUsr&nbsp;" + iIdUsr)
	Session("Editar")  = 0
	Session("Agregar") = 0
	Session("Borrar")  = 0	
...
function formato_decimales(numero,dec,sepmiles){ 

	var decimales = dec
	var separador_decimal = "."
	var separador_miles = sepmiles
...
bPuedeVerDebug = ParametroDeVentanaConUsuario(SistemaActual, -1, IDUsuario, "PuedeVerDebug",0) == 1
...
var sRutapt =  String(Request.ServerVariables("PATH_TRANSLATED"))
var iRPos = sRutapt.indexOf("\Default.asp")
if (iRPos >= 0) {
var sqlUSR = "Insert into UsuarioFlujo ( iqCli_ID, Sys_ID, Mnu_ID, Usu_ID, UsuA_Sesion, UsuA_IP ) " 
	sqlUSR += " Values ('" + iqCli_ID + "', " + SistemaActual  + ", " + VentanaIndex + ", " + IDUsuario
	sqlUSR += " ,'" + String(Session.SessionID) + "','" + sIP + "')"
...
var JSON = {
	...
	, Convertir: function( prmObj, prmIntTip ){
		
		var strJSON = "{}";
		
		switch(parseInt(prmIntTip)){
			case this.Tipo.Exportacion: {
				...
				for( var i = 0; !(prmObj.EOF); i++, prmObj.MoveNext() ){
					...
					for( var j = 0; j < prmObj.Fields.Count; j++ ){
						...
						strJSON += '"';
						strJSON += prmObj.Fields.Item(j).Name
						strJSON += '": '
						strValor = "" + prmObj.Fields.Item(j).Value
						
						strValor = strValor.replace(/^\s+|\s+$/g,"");	
...
function ExisteEnArreglo( prmVal, prmArrVal ){
	
	var bolExiste = false

	for(var i=0; i<prmArrVal.length; i++){
		if( prmArrVal[i] == prmVal ){
			bolExiste = true
			break;
		}
	}
