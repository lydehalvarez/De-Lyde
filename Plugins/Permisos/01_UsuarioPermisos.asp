...
function ArmaArbol(ID) {
	...
	while (!rsINS.EOF){
		...
		if (bDebugWidgets) { sTitulo +=  " " + rsINS.Fields.Item("Mnu_ID").Value + "/" + rsINS.Fields.Item("Mnu_IDSeguridad").Value }
...
var sG = ""
    ...
    sG += "</td><td height=\"22\" align=\"right\">&nbsp;</td></tr>"
...
function DespliegaSeguridad() {
	...
	sG += "<tr><td width=\"72%\" height=\"25\" class=\"GridSubTitulos\">Permisos del Grupo: " + NombreUsuario + "&nbsp;</td>"
	sG += "<td width=\"28%\" >"
...
//	sG += "</td></tr>"
	...
	sG += "<input type=" + sC + "hidden" + sC + " name=" + sC + "Usu_ID" + sC + " id=" + sC + "Usu_ID" + sC + " value=" + sC + UsuID + sC + ">"
	...
	sG += "<script language='JavaScript'>"
	sG += " $('#BSpanel1').html('Usuario: <strong>' + $('#NombreGrupo').val() + '</strong>'); "
	sG += " $('#BSpanel2').empty(); "
	sG += "</script>"
...
if (Accion == "Guardar") {
	...
	var sSQL = " Update Usuario Set "
		sSQL  += " Usu_Grupo = -1 "
		sSQL  += " ,Usu_UltimaActualizacionJ = -1 "
