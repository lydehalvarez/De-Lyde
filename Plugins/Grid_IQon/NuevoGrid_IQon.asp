...
function ArmaGrid() {
	...
	var grdOrden = new Array(0)     //0 sin orden  1 ascendente  2 descendente
	var grdOculto = new Array(0)
	var ParamPermanente = new Array(0)   //es de los parametros permanentes
	var grdEsLlave = new Array(0)
	var grdEsLlavePrimaria = new Array(0)
	var grdValor = new Array(0)             //valor que se usara en la busqueda
	var grdEsCampoDeBusqueda = new Array(0)
	var grdTipoFiltro = new Array(0)          //0 sin filtro   1 texto  2 numero  3 fecha
	...
	var sATCps = "SELECT * "
		sATCps += " ,ISNULL((Select PP_Nombre from ParametrosPermanentes where ParametrosPermanentes.PP_Nombre = MenuGridCampos.MGrd_Campo and PP_Habilitado = 1),'No') as PP "
		...
		if (rsEnc.Fields.Item("MGrd_EsLlave").Value == 1 ) {
			...
			if ( rsEnc.Fields.Item("MGrd_EsCampoOculto").Value == 0 ) {
				...
				grdClass[iCampos]      = rsEnc.Fields.Item("MGrd_Class").Value
					...
					if ( String(rsEnc.Fields.Item("PP").Value) == "No") {
						ocultoPP[iCamposO] = 0
					} else {
						ocultoPP[iCamposO] = 1
					}
...
//condicion por parametro es para colocar en la parte de filtros de la sentencia sql campos que deberan tomar los parametros que estan volando 
...
//	}
	...
	var sFG = "<input type=" + sC + "hidden" + sC + " name=" + sC + "tmpPagina" + sC + " value=" + sC + iPagina + sC + ">"
		...
		var ppenc = ElParametroEsPermanente(String(CampoLlave))
		
		if ( ppenc == 0 ) {
			sFG += "<input type=" + sC + "hidden" + sC + " name=" + sC + CampoLlave + sC + " value=" + sC + Parametro(CampoLlave,-1)+ sC + ">"	
		}	
