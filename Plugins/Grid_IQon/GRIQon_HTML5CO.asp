...
var iIQonMuestraQueryPrincipal = ParametroDeVentanaConUsuario(SistemaActual, VentanaIndex, SysLog_ID, "Ver query principal",0)
	...
	var sSQLConfBase = " select Wgt_ID, WgCfg_ID, MW_Param, MW_PuedeAgregar, MW_PuedeBorrar, MW_PuedeEditar, "
		...
		sSQLConfBase += " AND Wgt_ID = 62 "  // + iWgID
...
if (iIQonDebug > 0) { 
	...
	if (!rsConfBase.EOF){
		...
		var sCondVS = " Sys_ID = " + SistemaActual + " and WgCfg_ID = " + iWgCfg_ID + " AND Wgt_ID = 62 "
...
if (iIQonDebug > 0) {	
	...
	switch (parseInt(Tarea)) {
		case 1:
			...
			sResultado += "<div id='DatosGrid'></div>"
	...
	function ArmaPaginacionCO(iPaginas,iRegistros) {
		...
		if (iMWPuedeAgregar >= 1 && Session("Agregar") == 1) {
				...
				sPaginacion += "<div class='row'>"
					...
					sPaginacion += "<div class='col-md-4'>"
						sPaginacion += "<div id='dvbtnNuevo' align='right'>"
							sPaginacion += "<a class='btn btn-green' href='"
			...
			sPaginacion += "<div class='form-horizontal' id='PagGrid'>"
				sPaginacion += "<div id='pag1' class='dataTables_length' align='right' style='margin-top:25px'>"
					sPaginacion += "<div class='form-group'>"
						sPaginacion += "<label class='col-md-2 control-label'>Registros por p&aacute;gina</label>"
						...
						sPaginacion += "</div>"	
						sPaginacion += "<label class='col-md-3 control-label'>" 	
							sPaginacion += "<p class='text-left'>Total:&nbsp;" + iRegistros
							sPaginacion += "&nbsp;Registros&nbsp;&nbsp;"
							sPaginacion += "P&aacute;gina actual</p>"
						sPaginacion += "</label>"
...
function ArmaPaginacion(iPaginas,iRegistros) {
	var sPaginacion = ""
	//sPaginacion += "<div id='sample_1_wrapper' class='dataTables_wrapper form-inline' role='grid'>" //CO
	if (iMWPuedeAgregar >= 1 && Session("Agregar") == 1) {
	...
	sPaginacion += "<div class='row'>"
		sPaginacion += "<div class='col-md-8'>"
			...
			sPaginacion += "<div id='pag1' class='dataTables_length' align='right' style='margin-top:25px'>" //
				sPaginacion += "<label>"
					...
					sPaginacion += "<select size='1' name='RegPorVentana' id='RegPorVentana'"
					//sPaginacion += " style='width: 60px;' "
					sPaginacion += " class=''" //CO m-wrap small select2-offscreen form-control input-sm
					sPaginacion += " onchange='AcPaginacion(this.value)' >"
					sPaginacion += "<option value='10'" 
						if ( iRegPorVentana <= 10) { sPaginacion += "selected " } 
					sPaginacion += ">10</option>"
					...
					sPaginacion += "<select name='IrAPagina' id='IrAPagina'"  
					sPaginacion += " onchange='AcIrAPagina(this.value)' style='width:60px;'>"     
					...
					sPaginacion += "</select>"
					sPaginacion += "&nbsp; de &nbsp;" + iPaginas + "&nbsp; P&aacute;gina(s)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>"
					iPaginas,iRegistros
		...
		sPaginacion += "</div>"
		sPaginacion += "<div class='col-md-4' align='center'>"
		if (iPaginas > 1) {
		
		sPaginacion += "<div class='dataTables_paginate paging_bootstrap'>"
		
			sPaginacion += "<ul id='pagination' class='pagination'>"
...
function ArmaEncabezados() {
	...
	for(ig=0;ig<grdCampo.length;ig++){ 
			...
			//if (grdClass[ig] != "")	sEncabezado +=" class='" + grdClass[ig] + "' "
		...
		while (!rsINS.EOF){
			...
			if (MGrdT_TieneChecks==1) {
				sDatos +="<td class='center' width='5px'>"
					sDatos += "<div class='checkbox-table' style='padding-left:0'>"
						sDatos += "<label>"
							sDatos +="<input name=\"chkR\" id=\"chkR\" type=\"checkbox\" "
							sDatos +=" class=\"chkR\" data-parms=\"" + sValoresPrm + "\" value=\"" + iRegistros + "\" />"
						sDatos += "</label>"
					sDatos += "</div>"
				sDatos += "</td>"
			}
