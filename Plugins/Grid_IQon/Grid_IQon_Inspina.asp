...
<%
	...
	var iIQonDebug = ParametroDeVentanaConUsuario(SistemaActual, VentanaIndex, IDUsuario, "Debug",0)	
	var iIQonQPrincipal = ParametroDeVentanaConUsuario(SistemaActual, VentanaIndex, IDUsuario, "Ver query principal",0)	
...
if (iIQonDebug == 1) { 
	...
	var iWgt_ID = 61
	if (iWgCfg_ID == 0 ) {
		var sConddv = "Wgt_ID = 61 and Sys_ID = " + SistemaActual + " and Mnu_ID = " + VentanaIndex
		iWgCfg_ID = BuscaSoloUnDato("WgCfg_ID","Menu_Widget",sConddv,0,2)
	}

	//var iWgt_ID = Parametro("Wgt_ID",61)
...
//	var sCondVS = " Sys_ID = " + SistemaActual + " and WgCfg_ID = " + iWgCfg_ID + " AND Wgt_ID = " + iWgt_ID
//	var WgCfg_LlavesALimpiar = BuscaSoloUnDato("WgCfg_LlavesALimpiar","Widget_Configuracion",sCondVS,0,2)
//		LimpiaLlaves(WgCfg_LlavesALimpiar)
...
//			sSQLDtBs += ", " + SegGrupo
//			sSQLDtBs += ", " + iqCli_ID + " )"
//	if (iIQonDebug == 1) { Response.Write("<br>167 sSQLDtBs " + sSQLDtBs + "<br>") }	
//		var rsSQLDtBs = AbreTabla(sSQLDtBs,1,2)
//		if (!rsSQLDtBs.EOF){
			iMWPuedeSeleccionar = 1 //rsSQLDtBs.Fields.Item(0).Value
//		}
//		rsSQLDtBs.Close()	
//	}
...
//Response.Write("<BR>IDUsuario " + IDUsuario + ", UsuarioTpo " + UsuarioTpo + ", @iMnuIDSiguiente " + IDUsuario + ", SistemaActual " + SistemaActual + ", SegGrupo " + SegGrupo + ", iqCli_ID " + iqCli_ID )
	...
	if (iIQonDebug == 1) {Response.Write("<br>211) bTTConfigurado " + bTTConfigurado + "<br>") }
    if (iIQonDebug == 1) { bTTConfigurado = false }  //Esto es superimportante no borrar ni cambiar
	if (!bTTConfigurado) {  
		...
		var sSQLTT = "Insert Into Widget_Configuracion_Complemento "
			...
			sSQLTT += " Values ( "
			sSQLTT += iqCli_ID + ", "  + SistemaActual + ", " + VentanaIndex + ", -1, -1, 61 ," 
			sSQLTT += iWgCfg_ID + ", 0 " 
			sSQLTT += " ) "
	...
	// ======== LEE LOS CAMPOS QUE FORMAN EL ENCABEZADO Y LOS QUE MOSTRARAN LA INFORMACIÓN
        if (iIQonDebug == 1) { Response.Write("<br />233) Carga de Parametros permanentes ") }
		var sATCps = "SELECT * "
...
//Armado de arreglos
	...
	if (!rsEncTB.EOF){
			...
			if (iIQonDebug == 1 ||  iIQonQPrincipal== 1) {
				Response.Write("<br />SQLTabla = " + SQLTabla  + "<br />")
			}		
	}
	rsEncTB.Close()
		...
		for (j=0;j<arrPrmCPP.length;j++) {
			...
			for (hi=0;hi<LLaveCampo.length;hi++) {	
				if (LLaveCampo[hi] == arrCampo[0]) {
					...
					LLaveCondicion[hi] = 1
					var sValTmp = ""
					if(String(arrCampo[1])=="-1"){
						sValTmp = -1
					} else {
						sValTmp = FiltraVacios(Parametro(String(arrCampo[1]),String(arrCampo[2])))
					}
...
var sCondBus = ""	
	...
	if (iIQonDebug == 1) {
		...
		Response.Write("<br />iSQLVieneDeBuscador " + iSQLVieneDeBuscador  + "<br />")
	...
	//if (iIQonDebug == 1) {Response.Write("<br />Armando criterios de busqueda con sqCampoLlavePrefijo " + sqCampoLlavePrefijo + "<br />")}
		...
		if ( iSQLVieneDeBuscador > 0 ) {
				...
				if (iIQonDebug == 1) {Response.Write("<br />427 Campos de búsqueda sSQLCamposBus = " + sSQLCamposBus  + "<br />")}
				...
				while (!rsSQLCamposBus.EOF){
					...
					if (iIQonDebug == 1) {Response.Write("<br />MFC_Campo = " + grdCampoBus[iCamposBus]  + "<br />")}
					
					grdTipoCampoBus[iCamposBus] 	= rsSQLCamposBus.Fields.Item("MFC_TipoCampo").Value
					if (iIQonDebug == 1) {Response.Write("<br />MFC_TipoCampo = " + grdTipoCampoBus[iCamposBus]  + "<br />")}
...
//					}
					...
					if (iIQonDebug == 1) {Response.Write("<br />MFC_EsBusqEstricta = " + grdEsBusqEstricta[iCamposBus]  + "<br />")}
					
					grdValor[iCamposBus]            = Parametro(grdCampoBus[iCamposBus],grdValorDefBus[iCamposBus])
					if (iIQonDebug == 1) {Response.Write("<br />grdValor = " + grdValor[iCamposBus]  + "<br />")}
					
					//if (iIQonDebug == 1) {Response.Write("<br />grdCampoBus[iCamposBus] = " + Parametro(grdCampoBus[iCamposBus],grdValorDefBus[iCamposBus])  + "<br />")}
				...
				if (iIQonDebug == 1) { Response.Write("<br />length = " + grdCampoBus.length + "<br />") }
				
				for(igB=0;igB<grdCampoBus.length;igB++){
					
					if (iIQonDebug == 1) { Response.Write("<br />grdValor = " + grdValor[igB] + "<br />") }
					...
					if (grdValor[igB] != "") {
						...
						if (grdTipoCampoBus[igB] == 2 || grdTipoCampoBus[igB] == 5) {
							if ( sCondBus != "") { sCondBus += "|" }
							if (grdValor[igB] > 0) {
								sCondBus += grdCampoBus[igB]	
								sCondBus += ":1" 
						...
						if (grdTipoCampoBus[igB] == 4 || grdTipoCampoBus[igB] == 7) {
							if (grdValor[igB] > -1) {
								if ( sCondBus != "") { sCondBus += "|" }
								sCondBus += grdCampoBus[igB]
								sCondBus += ":" + grdValor[igB]
						...
						if (iIQonDebug == 1) { Response.Write("<br />grdValor - JD = " + grdTipoCampoBus[igB] ) }
						...
						//Rangos de fecha date == datetime {end}
...
//							var sRango = grdValor[igB])
						...
						//Rangos de fecha date == datetime {start}
						if (grdTipoCampoBus[igB] == 19) {   //rangos del campo receptor tiene la fecha en formato date
						
							if ( sCondBus != "") { sCondBus += "|" }
							sCondBus += grdCampoBus[igB]
//							var sRango = grdValor[igB])
                            sCondBus += ":R19" + grdValor[igB]  +"R19"
						
							//if ( sCondBus != "") { sCondBus += "|" }
							//sCondBus += " (" + grdCampoBus[igB] + ":td19" + grdValor[igB] + "tdm19" + grdCampoBus[igB] + "tdmd19" + grdValor[igB] + "td19:"
						}	
										
											
						if (grdTipoCampoBus[igB] == 20) {   //rangos del campo receptor tiene la fecha en formato datetime no se usa
							if ( sCondBus != "") { sCondBus += "|" }
							sCondBus += " (" + grdCampoBus[igB] + ":td20" + grdValor[igB] + "tdm20" + grdCampoBus[igB] + "tdmd20" + grdValor[igB] + "td20:"
						}											
						...
						//if (grdTipoCampoBus[igB] == 22) { 	
							//if (iIQonDebug == 1) {
								//if (grdTipoCampoBus[igB] == 1 ) {
									//if ( sCondBus != "") { sCondBus += "|" }
										//WHERE Cli_ID IN (SELECT CLI.Cli_ID FROM Cliente CLI WHERE CLI.Cli_NombreCompleto LIKE ''+ @sNomCliente + '')
										//sCondBus += grdCampoBus[igB]
//										sCondBus += ":td21" + grdValor[igB] + "tdm21" + 
										//sCondBus += ":¬%" + grdValor[igB] +"%¬"
									//}
								//}				
							//}
...
var sGridllaves = ""
var sGridCompleto = "<div id='ContenidoGrid'></div>"
	
//coloco los campos ocultos y llaves	
//   Campo,EsPermanente class='iQon'

	var arrOcSec  = new Array(0)
	var arrOc     = new Array(0)
	if (iIQonDebug == 1) {Response.Write("<br />647 Campos Ocultos " + arrOcultos +"<br />")}	
	if (!EsVacio(arrOcultos) ) {		
		arrOcSec = arrOcultos.split("|")
		for (j=0;j<arrOcSec.length;j++) {
			if (iIQonDebug == 1) {Response.Write( sCondBus +"<br />")}
			var Txt = String(arrOcSec[j])
			if (iIQonDebug == 1) {Response.Write( Txt +"<br />")}
			var arrOc = Txt.split("{")
			if (arrOc[1] == 0) {
				sGridllaves += "<input type='hidden' name='" + arrOc[0] + "' " 
				sGridllaves += " id='" + arrOc[0] + "' "
				sGridllaves += " value='" + Parametro(String(arrOc[0]),-1) + "'> "
			}
		}
	}
	//Response.Write(LLaveCampo.length+"<br />")
	if (iIQonDebug == 1) {Response.Write("<br />662 Campos llave " + LLaveCampo +"<br />")}	
	for (t=0;t<LLaveCampo.length;t++) {
		//Response.Write(LLaveCampo[t] + " = " + LLavePP[t]+"<br />")
		if (LLavePP[t] == "No") {		
			if (LLaveHeredada[t] == 0) {
				sGridllaves += "<input type='hidden' name='" + LLaveCampo[t] + "' "  
				sGridllaves += " id='" + LLaveCampo[t] + "' value='" + LLaveValor[t] + "'> "
			}
		}	
	}	
	...
	if (iAbrirNuevaVentana == 1 ) {
		...
		sGridCompleto += "<div style='display:none'><img src='/images/x.png' width='25' height='29' /></div></div></div>"
	}
...
function grSelecciona(<%
	...
	for (t2=0;t2<LLaveCampo.length;t2++) {
		if (LLaveHeredada[t2] == 0) {
			...
			//Response.Write("$('input:hidden[name="+LLaveCampo[t2]+"]').val(prm" + LLaveCampo[t2] + "); ")
		}
	}
	%>
	 if (iSigVentana == 0 ) { 
