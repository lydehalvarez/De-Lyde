...
if (iIQonDebug == 1) { 
	...
	if (iWgCfg_ID == 0 ) {
		var sConddv = "Wgt_ID = 42 and Sys_ID = " + SistemaActual + " and Mnu_ID = " + VentanaIndex
		iWgCfg_ID = BuscaSoloUnDato("WgCfg_ID","Menu_Widget",sConddv,0,2)
	}

	var iWgt_ID = Parametro("Wgt_ID",42)
...
var sCondBus = ""	
	...
	//if (iIQonDebug == 1) {Response.Write("<br />Armando criterios de busqueda con sqCampoLlavePrefijo " + sqCampoLlavePrefijo + "<br />")}
		...
		if ( iSQLVieneDeBuscador > 0 ) {
				...
				if (iIQonDebug == 1) {Response.Write("<br />427 Campos de busqueda sSQLCamposBus = " + sSQLCamposBus  + "<br />")}
...
//   Campo,EsPermanente
	...
	for (t=0;t<LLaveCampo.length;t++) {
		...
		if (LLavePP[t] == "No") {
			//Response.Write(LLaveHeredada[t]+"<br />")
