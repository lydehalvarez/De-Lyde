...
var SysLog_ID       = Parametro("IDUsuario",-1)
...
if (iIQonDebug > 0) { 
	...
	if (!rsConfBase.EOF){
			...
			var MW_Param = FiltraVacios(rsConfBase.Fields.Item("MW_Param").Value)	
			if (MW_Param != "") {
				iRegPorVentana = MW_Param  
			}
	}
	rsConfBase.Close()
...
if (iIQonDebug > 0) { Response.Write("<br>linea 300 Armado de arreglos de campos<br>") }	
...
function ArmaPaginacion(iPaginas,iRegistros) {
	...
	if (iMWPuedeAgregar >= 1 && Session("Agregar") == 1) {
		sPaginacion += "<tr><td colspan=" + sC + "11" + sC + " align=" + sC + "right" + sC + ">"
