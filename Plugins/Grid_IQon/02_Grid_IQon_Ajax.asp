...
//	}
	...
	var iWgID         = 25 //Parametro("iWgID",0)
...
//Response.Write("<BR>IDUsuario " + IDUsuario + ", UsuarioTpo " + UsuarioTpo + ", @iMnuIDSiguiente " + IDUsuario + ", SistemaActual " + SistemaActual + ", SegGrupo " + SegGrupo + ", iqCli_ID " + iqCli_ID )
	...
	if (!rsSQLTT.EOF){
		...
		if (arrLLaves == "") { bTTConfigurado=false } 
		arrCampos  = String(rsSQLTT.Fields.Item("WgCfgC_Atributo2").Value)
		if (arrCampos == "") { bTTConfigurado=false } 
...
//Armado de arreglos
	...
	//if (iIQonDebug == 2) {Response.Write("<br>arrLLaves " + arrLLaves + "<br>") }
...
var sCondBus = ""	
	...
	if (iIQonDebug == 1) {Response.Write("<br />xxSQLCondicion = " + SQLCondicion  + "<br />")}	
	...
	//if (iIQonDebug == 1) {Response.Write("<br />Armando criterios de busqueda con sqCampoLlavePrefijo " + sqCampoLlavePrefijo + "<br />")}
		...
		if ( iSQLVieneDeBuscador > 0 ) {
				...
				if (iIQonDebug == 1) {Response.Write("<br />Campos de busqueda sSQLCamposBus = " + sSQLCamposBus  + "<br />")}
				...
				while (!rsSQLCamposBus.EOF){
					...
					//if (iIQonDebug == 1) {Response.Write("<br />MFC_IDCatalogoGeneral = " + grdIDCatGralBus[iCamposBus]  + "<br />")}
					grdCboTablaBus[iCamposBus]      = rsSQLCamposBus.Fields.Item("MFC_ComboTabla").Value
					//if (iIQonDebug == 1) {Response.Write("<br />MFC_ComboTabla = " + grdCboTablaBus[iCamposBus]  + "<br />")}
	...
	if (iIQonDebug == 1) {Response.Write("<br />sCondBus" + sCondBus +"<br />")}
...
//   Campo,EsPermanente
	...
	if (!EsVacio(arrOcultos) ) {
		...
		for (j=0;j<arrOcSec.length;j++) {
			...
			var arrOc = Txt.split("{")
			if (arrOc[1] == 0) {
				sGridCompleto += "<input type=" + sC + "hidden" + sC + " name=" + sC + arrOc[0] + sC  
				sGridCompleto += " id=" + sC + arrOc[0] + sC + " value=" + sC
				sGridCompleto +=  Parametro(String(arrOc[0]),-1)
				sGridCompleto += sC + " > "
			}
		}
	}
	for (t=0;t<LLaveCampo.length;t++) {
		if (LLavePP[t] == "No") {
			sGridCompleto += "<input type=" + sC + "hidden" + sC + " name=" + sC + LLaveCampo[t] + sC  
			sGridCompleto += " id=" + sC + LLaveCampo[t] + sC + " value=" + sC
			sGridCompleto +=  LLaveValor[t]
			sGridCompleto += sC + " > "				
		}	
	}
