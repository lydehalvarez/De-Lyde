...
if (rsTab.EOF) {
	...
	sFuncionCase = ""
    sTabsArmados = "<div id='tabs' class='jq-tabs' ><ul>"
    ...
    var TabRegreso = BuscaSoloUnDato("Mnu_ID","Menu",sCondicionTabSeguimiento,"",2) 
		if (TabRegreso != "") {
			var Mnu_IDLigaDelTab = BuscaSoloUnDato("Mnu_IDLigaDelTab","Menu",sCondicionTabSeguimiento,"",2)
			if (!EsVacio(Mnu_IDLigaDelTab)) {
			    TabRegreso =  Mnu_IDLigaDelTab
			}			
			...
			sDivsTabs += "<div id='tabs-" + iPos + "'></div>"
			iPos++       
	}
...
//        //}
    ...
    //sCodigoDeLosTabs += "jQuery('.jq-tabs').tabs();"
	//este codigo es para la version 1.11.0 de jquery ui
	sCodigoDeLosTabs += "jQuery('.jq-tabs').tabs({ active: " + iTabSeleccionado + " }); "
    sCodigoDeLosTabs += "jQuery('.ui-tabs .ui-tabs-nav li:last-child').addClass('last'); "      
	sCodigoDeLosTabs += "jQuery('.jq-tabs').on('tabsactivate', function(event, ui) { "
    //sCodigoDeLosTabs += " alert('selecionado ' + ui.newTab.index()  ); "
	sCodigoDeLosTabs += " switch (ui.newTab.index()) {  "	 		
