...
if (ConSeguridad) {
    ...
    var sSQL  = "Select Mnu_ID, Mnu_Padre, Mnu_IDLigaDelTab,Mnu_Titulo,Mnu_RutaImagen,Mnu_TituloVentana1, Mnu_Condicion "
        sSQL += " from Menu "
        sSQL += " WHERE Mnu_Padre = (select Mnu_Padre from Menu where Mnu_ID = " + VentanaIndex
        sSQL +=                   " AND Sys_ID = " +  SistemaActual
        sSQL +=                   " AND Mnu_EsTab = 1 AND Mnu_Habilitado = 1 ) "
...
if (rsTab.EOF) {
	...
	sTabsArmados = "<ul class='nav nav-tabs'>"
   // sTabsArmados += "<span class='pull-right small text-muted'>"
   // sTabsArmados += "<a href='javascript:CambiaSiguienteVentana(" + sSiguienteVentana + ")'>"
   // sTabsArmados += VentanaIndex + "/" + sSiguienteVentana + "</a></span>"
	...
	if (TabRegreso != "") {
		...
		var Mnu_RutaImagen = "fa-reply" // BuscaSoloUnDato("Mnu_RutaImagen","Menu",TabTitCondicion,"",2)

		sTabsArmados += "<li><a data-toggle='tab' data-mnuid='" + TabRegreso + "' "
        sTabsArmados += " data-idtab='" + iPos + "' href='#tab-" + iPos + "' class='tabslc' >"
		if(!EsVacio(Mnu_RutaImagen)) {
			sTabsArmados += "<i class='fa " + Mnu_RutaImagen + "'></i>"
		}
...
//	}
	...
	while (!rsTab.EOF){	
		Mnu_Condicion = FiltraVacios(rsTab.Fields.Item("Mnu_Condicion").Value)
		//evalua la consulta de filtrado
		if(Mnu_Condicion !="") {
			Mnu_Condicion = ProcesaBuscadorDeParametros(Mnu_Condicion)		
			//bTabPermitido = EjecutaObjetoBD(Mnu_Condicion,0)
			bTabPermitido = eval(Mnu_Condicion)
		} else {
			bTabPermitido = true
		}
		if(bTabPermitido) {
	...
	//        //}
			...
			sTabsArmados += "<a data-toggle='tab' data-mnuid='" + datamnuid + "'" 
			sTabsArmados += " data-idtab='" + iPos + "' href='#tab-" + iPos + "' class='tabslc' >"
			if(!EsVacio(Mnu_RutaImagen)) {
				sTabsArmados += "<i class='fa " + Mnu_RutaImagen + "'></i>"
			}
