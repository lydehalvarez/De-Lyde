...
//comienza tabla de secciones  
		...
		var sATCps = "SELECT * "
			...
			sATCps += " AND Sys_ID = " + Parametro("Sys_ID",1)
			sATCps += " AND Mnu_ID = " + Parametro("VentanaIndex",1)
		...
		if (rsSeccion.EOF){ 
			...
			var sFCOcultos = "SELECT * "
				...
				sFCOcultos += "  AND Sys_ID = " + rsSeccion.Fields.Item("Sys_ID").Value
					...
					if ( rsCampos.Fields.Item("MFC_Renglon").Value == iRen && rsCampos.Fields.Item("MFC_Columna").Value == iCol ) {
						...
						switch (parseInt( rsCampos.Fields.Item("MFC_TipoCampo").Value)) {
							case 4:  // 4 = combo
								...
								sCampo = CargaComboFicha(sNombreCampo,sEventos,sNombreCampo,sCampoDescripcion,sTabla,sCondicion,sOrden,ValorDelCampo,0,sTodos,Modo) 
								break;	
							case 7:    // 7 = combo catalogo general
								...
								sCampo = "&nbsp;" + ComboSeccionFicha(sNombreCampo,sEventos,sSeccion,ValorDelCampo,0,sTodos,sOrden,Modo)
								break;				
						}
						//Seccion para campos que es muy diferente la edicion a la consulta
						//Modo = "Editar"
						if (Modo == "Editar") { 
						Response.Write("<font color='red'><strong>Modo&nbsp;" + Modo + "</strong></font>")
						Response.Write("<font color='red'><strong>MFC_TipoCampo&nbsp;" + parseInt(rsCampos.Fields.Item("MFC_TipoCampo").Value) + "</strong></font>")
								...
								switch (parseInt(rsCampos.Fields.Item("MFC_TipoCampo").Value)) {
									...
									case 3:						//			3 = check
										...
										sCampo = "&nbsp;" + CargaComboFicha(sNombreCampo,Eventos,sNombreCampo,
														    ...
														    sNombreCampo,ValorDelCampo,0,"Seleccione una opción",Modo)
										break;
									case 4:  // 4 = combo
...
//										 }  
		...
		while (!rsOcultos.EOF){
			...
			sResultado += "<input type=" + sC + "hidden" + sC + " name=" + sC + sNombreCampo + sC + " value=" + sC + ValorDelCampo + sC + ">"
			rsOcultos.MoveNext()
		}	
		rsOcultos.Close()		 
