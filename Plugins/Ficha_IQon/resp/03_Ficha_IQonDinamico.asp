...
function ComboSeccionFicha(NombreCombo,Eventos,Seccion,sCondicion,Seleccionado,Conexion,Todos,Orden,Modo) {
	...
	if (Modo == "Editar") {
		...
		var CCSQL = "SELECT Cat_ID, Cat_Nombre FROM Catalogos WHERE Sec_ID = " + Seccion
...
//***************************************************************************************************************
			...
			var iColPorSecRegOriginal = iColPorSecReg
				...
				//valido cuantos no tiene valor al ancho 
				var iSinValor = 0
				var  iSumaAnchos = 0
					...
					//********* Armado de Objetos *****************
						...
						switch (parseInt(rsCampos.Fields.Item("MFC_TipoCampo").Value)) {
									...
									case 14:	//		14 = 	solo imprime un parametro											
											sCampo = "&nbsp;" + Parametro(rsCampos.Fields.Item("MFC_Campo").Value,sValorDefault)
										break;
...
//Cargado de Datos via sentencias SQL
	...
	while (!rsOtroQry.EOF){	
		...
		if (MFQ_TipoQuery == "FN") {
			...
			MFQ_Query += "(" + sCondicionPorParametros + ")" + MFQ_QueryFinal	
