...
//			}
		...
		case 4:
			...
			break;
		case 5:  //fechas solo identificacion y conversion
						...
						switch (parseInt(rsCampos.Fields.Item("MFC_TipoCampo").Value)) {
									...
									case 6:						//			6 = fecha modo texto formato dd/mm/yy
										if (sModoRO == "Editar") {
											...
											if (ValorDelCampo != "") {
													if (iFormato == 0) {
														var stfc6 = FormatoFechaII( ValorDelCampo,"yyyy-mm-dd","Consulta")
														if (stfc6 != "01/01/1900") { sCampo += stfc6 }
													}
													if (iFormato == 4) {
														//sCampo += FormatoFecha( ValorDelCampo ,"UTC a dd/mm/yyyy")
														sCampo += FormatoFechaII(ValorDelCampo,"yyyy-mm-dd","Consulta") 
														//sCampo += CambiaFormatoFecha(ValorDelCampo,"yyyy-mm-dd","dd/mm/yyyy")
													}
													if (iFormato == 5) {
														//sCampo += FormatoFecha( ValorDelCampo ,"UTC a dd/mm/yyyy")
														sCampo += FormatoFechaII(ValorDelCampo,"UTC","Consulta") 
													}													
											} 
