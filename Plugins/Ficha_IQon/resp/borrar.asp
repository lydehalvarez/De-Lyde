...
if (iIQonDebug == 1) {	Response.Write("<br /><br /><font color='red'><strong>Accion&nbsp;" + Accion + "&nbsp;Modo&nbsp;" + Modo  + "</strong></font><br />")}
...
if (iIQonDebug == 1) {	Response.Write("<br />tabla &nbsp;" + sFCHTabla) }
...
Response.End()	
	...
	if (!rsTabla.EOF) {	
		...
		SQLCondicion = rsTabla.Fields.Item("MFC_CondicionGeneral").Value
...
if (iIQonDebug == 1) {	Response.Write("<br />sTabla&nbsp;=" + sTabla + "&nbsp;SQLCondicion&nbsp;=" + SQLCondicion ) }
...
if (iIQonDebug == 1) {	Response.Write("<br />sFCHCampos&nbsp;" + sFCHCampos + "<br />") }
	...
	if (!EsVacio(sCondicionPorParametro) ) {
		...
		for (j=0;j<arrPrmCPP.length;j++) {
			...
			for (i=0;i<CampoNombre.length;i++) {
				if (CampoNombre[i] == arrCampo[0]) {			
					bEnc = true
					CampoCondicion[i] = 1
					var sValTmp = Parametro(String(arrCampo[1]),String(arrCampo[2]))    		
					CampoValor[i] = String(sValTmp)
					CampoFormato[i] = String(arrCampo[3])
					if (iIQonDebug == 1) {	Response.Write("<br />encontrado en " + i + ") CampoNombre&nbsp;" + CampoNombre[i]) + "<br />") }
				}
			}
			if (!bEnc) {
				iCamposLL++
				...
				if (iIQonDebug == 1) {	Response.Write("<br />no encontrado " + iCamposLL + ") CampoNombre&nbsp;" + CampoNombre[iCamposLL]) + "<br />" )}		
			}
		}
...
if (iIQonDebug == 1) {
	...
	if (SQLCondicion == "") {
		...
		if (sLLavePrimariaHeredada != "") { SQLCondicion  += " AND " }
		SQLCondicion  += sLLavePrimariaHeredada
	}
