	...
	if (!EsVacio(sCondicionPorParametro) ) {
		...
		for (j=0;j<arrPrmCPP.length;j++) {
			...
			for (fi=0;fi<CampoNombre.length;fi++) {
				if (iIQonDebug == 1) {	Response.Write("<br />campo " + fi + ") CampoNombre&nbsp;" + CampoNombre[fi] + "<br />") }
				if (CampoNombre[fi] == arrCampo[0]) {			
					...
					if (iIQonDebug == 1) {	Response.Write("<br />encontrado en " + fi + ") CampoNombre&nbsp;" + CampoNombre[fi] + " con valor de " + CampoValor[fi] + " es llave " + CampoLLave[fi] +"<br />") }
				}
			}
			if (!bEnc) {
				iCamposLL++
...
var iContLL = 0
	...
	if (iIQonDebug == 1) {	Response.Write("<br />SQLCondicion&nbsp;=" + SQLCondicion ) }
	if (iIQonDebug == 1) {	Response.Write("<br />sLLavePrimaria&nbsp;=" + sLLavePrimaria + "<br />") }
