...
var iOcultoTipo2 = 0    // tipo 2 es cuando en el campo de oculto viene un 2 entonces se 
						//  verificara que el campo venga como solo lectura y se
...
function CargaBotones() {
	var sBot = "<div class='RenglonDeControles' >"
		sBot += "<div id='areafunciones' ></div>" 
		sBot += "<div class='AccionesDelFormulario' id='areabotones' align='right'>"
	if (Modo == "Editar") {
		//if (iMWPuedeEditar >= 1 && Session("Editar") == 1) {
			sBot += "<button id=\"btnCancelar\" class=\"btn\" onClick=\"javascript:AcFCancelar();\">Cancelar</button>&nbsp;&nbsp;&nbsp;&nbsp;"
			sBot += "<button id=\"btnGuardar\"  class=\"btn btn-primary\" onClick=\"javascript:AcFGuardar();\" >Guardar</button>"
		//}
	} else {
		//if (iMWPuedeBorrar >= 1 && Session("Borrar") == 1) {
			sBot += "<button id=\"btnBorrar\" class=\"btn\" onClick=\"javascript:AcFBorrar();\">Borrar</button>&nbsp;&nbsp;&nbsp;&nbsp;"	
		//}
		//if (iMWPuedeEditar >= 1 && Session("Editar") == 1) {
			sBot += "<button id=\"btnEditar\"  class=\"btn btn-primary\" onClick=\"javascript:AcFEditar();\" >Editar</button>&nbsp;&nbsp;&nbsp;&nbsp;"
		//}
		//if (iMWPuedeAgregar >= 1 && Session("Agregar") == 1) {
			sBot += "<button id=\"btnNuevo\" class=\"btn\" onClick=\"javascript:AcFNuevo();\">Nuevo</button>&nbsp;&nbsp;&nbsp;&nbsp;"		
		//}
	}				
	sBot += "</div>"
	sBot += "</div>"
	sBot += "<div class='iqClear' ></div>" 
...
function CargaDatos() {
	...
	//comienza tabla de secciones  
		...
		while (!rsSeccion.EOF){
            ...
            sResultado += "<fieldset>"
            sResultado += "<legend>" + rsSeccion.Fields.Item("MFS_Nombre").Value + "</legend>"
...
//			    sFCHCps += " ) as colspan "
				...
				if ( VarDecision > 0 ) {
					sFCHCps += " AND (MFC_VariableDecision = 0 "
					sFCHCps += " or MFC_VariableDecision = " + VarDecision + ") "	
				}
				sFCHCps += " ORDER BY MFC_Renglon, MFC_Columna "
			
		if (iIQonDebug == 1) {	
		
		    Response.Write("<font color='red'><strong>Cargado de campos&nbsp;" + sFCHCps + "</strong></font><br /><br />")
		}
...
//					</div>
				...
				while (!rsCampos.EOF){ 
					...
					if (iRen != rsCampos.Fields.Item("MFC_Renglon").Value ) {
						sResultado += "<div class='iqClear' ></div>" 
						iRen = rsCampos.Fields.Item("MFC_Renglon").Value
					}
					
					sResultado += "<div class='columna'>"
					sResultado += "<label class='Titulo' for='Campo" + iNumCampo + "'>" + sEtiqueta + "</label>"
					sResultado += "<div class='Informacion'>"
						...
						switch (parseInt(rsCampos.Fields.Item("MFC_TipoCampo").Value)) {
										...
										if (sModoRO == "Editar") {
											sCampo = "<script type=" + sC + "text/javascript" + sC + ">"
												sCampo += "	$(function() {"
													...
													sCampo += "	buttonImage:" + sC + "/Template/Zactain/img/calendar16x16.png" + sC + "," 		
											...
											sCampo +=  sC + " placeholder='dd/mm/yyyy' />" 
										} else { 
											sCampo = ""
											if (iOcultoTipo2 == 2) {
												sCampo += " <input type=" + sC + "hidden" + sC 
									...
									case 15:	//		15 = 	Fecha 
										...
										if (sModoRO == "Editar") {
											...
											sCampo += ValorDelCampo
											sCampo += sC +" size=" + sC + "20" + sC + " maxlength=" + sC + "20" + sC + " placeholder='dd/mm/yyyy' />"
											//sCampo = ""
										} else { 
											sCampo = ValorDelCampo
										}
...
if (iIQonDebug == 1) {	
	...
	if (!rsTabla.EOF) {	
		...
		MFC_Conexion = FiltraVacios(rsTabla.Fields.Item("MFC_Conexion").Value)		
		...
		if (GFINSRT == 1) {
			...
			if (Parametro(sLLavePrimariaCampo,sLLavePrimariaValor) == -1) {  
				//if (Session("Agregar") == 1) {
					...
					LlaveABuscar = SiguienteID(sLLavePrimariaCampo,sTabla, SQLCondicion ,MFC_Conexion)
...
VarDecision = Parametro("VarDecision",0)
	if (iIQonDebug == 1) {	
		Response.Write("<font color='red'><strong>VarDecision&nbsp;" + VarDecision + "</strong></font><br /><br />")
	}
...
if (bLlavesVacias) {
	...
	Response.Write("<div class='iqFicha' >")
