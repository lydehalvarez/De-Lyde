		...
		//<h2><i class="fa fa-pencil-square teal"></i> REGISTER</h2>
			...
			var rsCampos = AbreTabla(sFCHCps,1,2)
				...
				while (!rsCampos.EOF){		//Campos [Inicio]
					...
					sMFCPlaceHolder = FiltraVacios(sMFCPlaceHolder)
					...
					var iRegPorSec = BuscaSoloUnDato("MAX(MFC_Renglon)","MenuFichaCampos",sCondRegCol,1,2)
						...
						case 5:				//	5 = caja seleccion - checkbox
							...
							sCampo = CajaSeleccion(sMFCCampo,Eventos,Parametro(sMFCCampo,1),1,iMFCRequerido,sModoRO)
							
							sCampo += "<script type='text/javascript'> "
		
									sCampo += " if($('input[type='checkbox']').length) { "
										sCampo += "$('input[type='checkbox'].grey').iCheck({ "
						...
						case 6:				//	6 = fecha
							...
							if (sModoRO == "Editar") {
								...
								if (iOcultoTipo2 == 2) {
									sCampo += " <p class='SoloConsulta'><input type='hidden'" 
									sCampo += " name='" + sMFCCampo + "'"
									sCampo += " id='" + sMFCCampo + "'" 
						...
						case 9:				//	9 = Text Area
							...
							if (sModoRO == "Editar") {
								...
								sCampo = "<p class='SoloConsulta'>" + sMFCValorDelCampo + "</p>"
