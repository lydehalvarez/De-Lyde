...
function CargaBotones() {
	...
	//var bMuestraLinea = false
		//sBot = "<div class='ibox'>"
        sBot += "<div class='ibox-content' style='padding-bottom: 31px;'>"
						
		sBot += "<div class='form-group'>"
		sBot += "<div id='areafunciones' class='col-xs-3'>&nbsp;</div>"
		
		sBot += "<div id='areabotones' class='col-xs-9' style='text-align: right;padding-right:50px;'>"
		//Response.Write("iMWPuedeBorrar&nbsp;" + iMWPuedeBorrar + "<br>")
		//Response.Write("Session-Borrar&nbsp;"  + Session("Borrar"))
			if (Modo == "Editar") {
				...
				if (iMWPuedeEditar >= 1) {	
					...
					sBot += "<button type='button' class='btn btn-danger' id='btnCancelar'  "
					sBot += " onclick='javascript:AcFCancelar(" + MW_Al_Borrar_Ir_A + ");'><i class='fa fa-reply'> </i> Cancelar&nbsp; "
					sBot += "</button>"
					...
					sBot += "&nbsp;<button type='button' class='btn btn-success' id='btnGuardar' "
					sBot += " onclick='javascript:AcFGuardar();'><i class='fa fa-save'> </i> Guardar&nbsp;</button>"					
...
//				}
					...
					if (iMWPuedeBorrar >= 1) {
						...
						sBot += "&nbsp;<button type='button' class='btn btn-danger' id='btnBorrar' "
						sBot += " onclick='javascript:AcFBorrar();'><i class='fa fa-eraser'> </i> Borrar&nbsp;</button>"							
					...
					if (iMWPuedeEditar >= 1) {
						...
						sBot += "&nbsp;<button type='button' class='btn btn-success' id='btnGuardar' "
						sBot += " onclick='javascript:AcFEditar();'><i class='fa fa-edit'> </i> Editar&nbsp;</button>"						
					...
					if (iMWPuedeAgregar >= 1) {
						...
						sBot += "&nbsp;<button type='button' class='btn btn-info' id='btnNuevo' "
	...
	//				}
		...
		sBot += "</div>"
		sBot += "</div>"		
		sBot += "<div id='areanotificaciones' class='col-xs-12'>&nbsp;</div>"		
		sBot += "</div>"	
		//sBot += "</div></div>"		
				...
				while (!rsCampos.EOF){		//Campos [Inicio]
					...
					if (biQ4Web) {
						...
						case 4:			//	4 = combo
							...
							sCampo = CargaComboFicha(sMFCCampo,sEventos,sLlave,sCampoDescripcion,sTabla,sCondicion,sOrden,Parametro(sMFCCampo,sMFCValorDefault),MFC_Conexion,sTodos,iMFCRequerido,sModoRO,sEstilo) 
...
//								}
//							}
								sCampo = ComboSeccionFicha(sMFCCampo,sEventos,sSeccion,Parametro(sMFCCampo,sMFCValorDefault),MFC_Conexion,sTodos,sOrden,iMFCRequerido,sModoRO,sEstilo)
...
//								}
//							}
								sCampo = ComboSeccionFichaXTXT(sMFCCampo,sEventos,sSeccion,Parametro(sMFCCampo,sMFCValorDefault),MFC_Conexion,sTodos,sOrden,iMFCRequerido,sModoRO,sEstilo)
...
function ArmaMarco() {
	...
	var sG = ""
		sG += "<div class='wrapper wrapper-content  animated fadeInRight'>"
		sG += "<div class='row'>"
	//sG = "<div class='panel-body'>"
		...
		sG += "<div class='form-horizontal' id='frmFicha' name='frmFicha'>"				
...
function BorrarLLavePrincipal() {
	...
	var sSQLConfBase = " SELECT MW_Param, MW_PuedeAgregar, MW_PuedeBorrar, MW_PuedeEditar "
		...
		sSQLConfBase += " AND Wgt_ID = 60 "  // + iWgID // 76
