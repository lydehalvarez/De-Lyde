...
function CampoModoInformativo(iMFCFormato, sMFCCampo, sMFCValorDelCampo){
	...
	if(iMFCInformativo == 0 || iMFCInformativo == 1) {
		...
		if(!EsVacio(sMFCValorDelCampo)) {	
				//Response.Write("!EsVacio") AplicaFormato(iMFCFormato,sMFCValorDelCampo)
			sCampo = "<p class='SoloConsulta'>" + IFAnidado(!EsVacio(AplicaFormato(iMFCFormato,sMFCValorDelCampo)),AplicaFormato(iMFCFormato,sMFCValorDelCampo),'&nbsp;') + "</p>"
		} else {
					//Response.Write("EsVacio")
	...
	function CargaBotones() {
		...
		var sBot = ""
			...
			sBot += "<div class='ibox'>"  //ibox {start}
				...
				sBot += "<div class='row'>"
					sBot += "<div class='col-md-12 CntBtn' "
					sBot += ">"  
						sBot += "<div class='col-md-offset-6 col-md-5' id='areabotones' style='text-align: right;padding-right:50px;'>"
			...
			var sFCHCps = "SELECT * "
				...
				if (biQ4Web) {
						...
						switch (parseInt(iMFCTipoCampo)) {													   
							...
							case 4:							  
								...
								if (!EsVacio(sMFCComboCondicion)) {
									...
									//sCondicion =  ProcesaBuscadorDeParametros(sCondicion)
								...
								if (!EsVacio(sMFCLeyendaSelecTodos)) {
									sTodos = sMFCLeyendaSelecTodos
										...
										sCampo += "<script type='text/javascript'> "
											...
											sCampo += " $('select#"+sMFCCampo+"').select2(); "
								...
								//sirve para inicializar
                                    sCampo += "	$(document).ready(function () { "
										sCampo += " $('.i-checks').iCheck({ "
											...
											sCampo += ", radioClass: 'iradio_square-green' "
										sCampo += " }); "
								//sirve para controlar el click al objeto de bootstrap	
										sCampo += " $('.i-checks').on('ifChanged',function(event){ "
									//esta linea checa o descheca el verdadero objeto checkbox al cual le puedes pedir de modo 
									//normal la informacion con el id del objeto
										sCampo += " $(this).find('input[type=checkbox]').attr('checked',event.target.checked); "
										sCampo += " ControlaObjeto($(this)); "
										
										//este lo puse para que vieras el menaje pero quita esta linea
										//QUITA ESTA LINEA, SOLO ESTA
										//sCampo += " if (event.target.checked) {	alert('1572) Click en el campo "+sMFCCampo+" ');  }  "
                  ...
                  sCampo += "</div>"									
							...
							case 14:	//14 = 	Hacer un div
																				
								sCampo += "<label class='col-md-offset-"+iMFCOffset+" col-md-"+sMFCAnchoEtiqueta+" control-label' id='lbl"+sMFCCampo + "'>"+sMFCEtiqueta+"</label>"
								sCampo += "<div class='col-md-"+sMFCAnchoCampo + "'>"												
										sCampo += "<div id=" + sC + sMFCCampo + sC + "></div>"
								sCampo += "</div>"
...
function CargaComboFicha(NombreCombo,Eventos,CampoID,CampoDescripcion,Tabla,Condicion,Orden,Seleccionado,Conexion,Todos,Requerido,Modo,Estilo,sOpcAyuda) {
	...
	if (Modo == "Editar") {		
		...
		if (iMFCInformativo > 0) {	//start iMFCInformativo
		...
		} else {	//end iMFCInformativo start != iMFCInformativo
