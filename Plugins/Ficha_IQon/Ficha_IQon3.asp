...
//comienza tabla de secciones  
		...
		while (!rsSeccion.EOF){
			sResultado += "<tr><td height=" + sC + "25" + sC + " colspan=" + sC + "6" + sC + " class=" + sC 
			...
			var bUsado = false
				...
				while (!rsCampos.EOF){
					...
					if ( rsCampos.Fields.Item("MFC_Renglon").Value == iRen && rsCampos.Fields.Item("MFC_Columna").Value == iCol ) {
						...
						var iTpoCampo = parseInt( rsCampos.Fields.Item("MFC_TipoCampo").Value)
						switch (iTpoCampo) {
							case 4:  // 4 = combo
								var sEventos = ""
...
//										 }  
					...
					sResultado += "<td width=" + sC + "15%" + sC + " class=" + sC + "FichaCampoValor" + sC + ">"
					sResultado += sCampo
					sResultado += "</td>"
...
var sConsultaSQL = "SELECT * "
	...
	//Response.Write(sConsultaSQL)
		AgregaDebugBD("sql ficha3 carga inicial",sConsultaSQL )
		bHayParametros = false
