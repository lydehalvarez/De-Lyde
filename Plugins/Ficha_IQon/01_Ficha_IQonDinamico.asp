...
function CargaBotones() {
	...
	sBot = "<tr><td width=\"72%\" height=\"25\"><div id=\"areafunciones\" >&nbsp;</div></td><td width=\"28%\" align=\"right\"><div id=\"areabotones\" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
	if (Modo == "Editar") {
...
//***************************************************************************************************************			
			...
			var sCondRegCol = " MFC_Habilitado = 1 "
				...
				if (iIQonDebug > 0) {
					Response.Write("<font color='red' size='2'><strong>Sección&nbsp;" + rsSeccion.Fields.Item("MFS_ID").Value + "</strong></font><br />")
					Response.Write("<font color='red' size='2'><strong>Máximo de columnas por Sección&nbsp;" + iColPorSecReg + "</strong></font><br />")
					Response.Write("<font color='red' size='2'><strong>Renglones por Sección&nbsp;" + iRegPorSec + "</strong></font><br /><br />")
				}

//***************************************************************************************************************
			sResultado += "<tr><td>"   //tabla general que envuelve todo
			sResultado += "<table width=" + sC + "100%" + sC + " border=" + sC + "0" + sC 
			sResultado += " cellpadding=" + sC + "0" + sC + " cellspacing=" + sC + "0" + sC 
			sResultado += " class=" + sC + "FichaCampoValor" + sC + ">"							
			sResultado += "<tr><td height=" + sC + "25" + sC + " class=" + sC 
			sResultado += rsSeccion.Fields.Item("MFS_Class").Value + sC + ">"
			sResultado += "" + rsSeccion.Fields.Item("MFS_Nombre").Value
			sResultado += "</td></tr></table>"
			sResultado += "</td></tr><tr><td>"  //es de la tabla general que envuelve todo
			...
			var iControl = 1
				...
				sResultado += "<table width=" + sC + "100%" + sC + " border=" + sC + "0" + sC + " cellpadding=" + sC + "0" + sC + " cellspacing=" + sC + "0" + sC + " class=" + sC + "FichaCampoValor" + sC + ">"
				sResultado += "<tr>"
				
				var rsCampos = AbreTabla(sFCHCps,1,2)
				iColPorSecReg = iColPorSecReg*2
				iPRCAnchoCol = 100/iColPorSecReg
				while (!rsCampos.EOF){  //Juego de Campos
				
					if (iRen < rsCampos.Fields.Item("MFC_Renglon").Value) {
						//insertar columnas faltantes,
						for (iCF=iCol;iCF<=iColPorSecReg;iCF++) {
							sResultado += "<td width=" + sC + iPRCAnchoCol + "%" + sC + " >&nbsp;</td>"
						}
						//sResultado += "<td></td>"
						//cerrar renglon,
						sResultado += "</tr>"
					...
					//****************************************************************
						...
						switch (parseInt(rsCampos.Fields.Item("MFC_TipoCampo").Value)) {
									...
									case 7:    // 7 = combo catalogo general
										...
										ValorDelCampo = ComboSeccionFicha(sNombreCampo,sEventos,sSeccion,ValorDelCampo,0,sTodos,sOrden,sModoRO)
										sCampo = "&nbsp;" + AplicaFormato(iFormato,ValorDelCampo)	
										break;
									case 8:						//			8 = password
										if (sModoRO == "Editar") {
									...
									case 9:						//			9 = Text Area
										if (sModoRO == "Editar") {
											...
											sCampo +=  " class='FichaObjetos' id=" + sC + sNombreCampo + sC 
									...
									case 15:	//		15 = 	Fecha en texto y su campo juliano
										if (sModoRO == "Editar") {
											sCampo = "<script type=" + sC + "text/javascript" + sC + ">"
												sCampo += "	$(function() {"
													sCampo += "	$( " + sC +	"#" + sNombreCampo + sC + " ).datepicker({"
													sCampo += "	changeMonth: true," 
													sCampo += "	changeYear: true," 
													sCampo += "	showOn:	" + sC + "button" + sC + ","  
													sCampo += "	buttonImage:" + sC + "/images/calendar.png" + sC + "," 		
													sCampo += " buttonImageOnly: true "
													sCampo += "});"
					...
					//columna pa etiqueta
						//abrir td
						sResultado += "<td width=" + sC + iPRCAnchoCol + "%" + sC   
						//manejo de colspan
						// if colspan no es nulo 
							//sResultado += " colspan=" + sC + "2" + sC + 
							//iCol = iCol + "2"
...
function ArmaMarco() {
	...
	sG += "<tr><td height=\"1\" colspan=\"2\"></td><div id=\"notificacionerror\"></div></tr>"
...
if (iIQonDebug == 1) {	
	...
	Response.Write("<br /><font color='red' size='2'><strong>sysid&nbsp;" + sysid + "&nbsp;:&nbsp;=&nbsp;:&nbsp;mnuid&nbsp;" + mnuid  + "</strong></font>")
}
...
//		}
	...
	if (iIQonDebug == 1) {	
		//Response.Write("<br /><font color='red' size='2'><strong>sql&nbsp;:=:&nbsp;" + sFCHTabla +  "</strong></font>")
		Response.Write("<br /><font color='red' size='2'><strong>sTabla:&nbsp;" + sTabla + "&nbsp;SQLCondicion&nbsp;" + SQLCondicion + "&nbsp;sOrdenadoPor&nbsp;" + sOrdenadoPor + "&nbsp;sCondicionPorParametro&nbsp;" + sCondicionPorParametro + "&nbsp;iSQLVieneDeBuscador&nbsp;" + iSQLVieneDeBuscador + "</strong></font>")
	}
	...
	if (!EsVacio(sCondicionPorParametro) ) {
		...
		for (j=0;j<arrPrmCPP.length;j++) {
			...
			if (!bEnc) {
				...
				if (iIQonDebug == 1) { Response.Write("<br /><font color='red' size='2'><strong>---no encontrado&nbsp;" + iCamposLL + ") CampoNombre&nbsp;" + CampoNombre[iCamposLL]+"</strong></font><br />")}		
			}
		}
...
//Cargado de Datos via sentencias SQL
	...
	if (iIQonDebug == 1) {
		Response.Write("<font color='red' size='2'><strong>CargadodeDatosviasentenciasSQL&nbsp;" + sOtroQry + "</strong></font><br />")
	}	
		...
		if (!EsVacio(MFQ_Parametros) ) {  		              //se extraen los parametros que se envian
			...
			for (oi=0;oi<arrOQPP.length;oi++) {	
				...
				if (sTmpPP != "" && sTmpPP != -1 ) {
					...
					MFQ_Condicion += " " + arrOQCampo[0] + sSimboloComp + tA + "" + sTmpPP + tB
				}
			}
		}
		if (MFQ_Condicion != "") {
