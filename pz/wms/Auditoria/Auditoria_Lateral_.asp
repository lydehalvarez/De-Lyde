...
<%	
	if( !(bolInicia) ){
		// Visita Posterior
%>
                    <button type="button" class="btn btn-primary btn-sm btn-block" onclick="AuditoriaSeleccionLPN.CerrarVisitaVigente({Aud_ID: <%= Aud_ID %>, Visita: <%= Visita + 1 %>});">
                        <i class="fa fa-envelope"></i> Cerrar Vista
                    </button>
