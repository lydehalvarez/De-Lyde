...
switch( parseInt(rqIntTarea) ){
    ...
    //Petición de carga de TODOS los SKUS
    case 2110: {

        var rqintAud_ID = Parametro("Aud_ID", -1);

        var sqlSelTod = "EXEC SPR_Auditoria_Seleccion_LPN "
			  + "@Opcion = 2110 "
            + ", @Aud_ID = " + ( (rqintAud_ID > -1) ? rqintAud_ID : "NULL" ) + " "
        ...
        var rsSelTod = AbreTabla(sqlSelTod, 1, cxnTipo)

        if( !(rsSelTod.EOF) ){
            intErrorNumero = rsSelTod("ErrorNumero").Value
            strErrorDescripcion = rsSelTod("ErrorDescripcion").Value
        } else {
            intErrorNumero = 1
            strErrorDescripcion = "No se conecto a la Base de Datos"
        }
