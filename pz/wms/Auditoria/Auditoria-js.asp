...
<%
    ...
    {
        var result = {};
		var sSQL = "SELECT Aud_Nombre, Aud_Descripcion,Aud_EstatusCG141 ";
			sSQL += " ,Aud_ID,Aud_VisitaActual,Aud_HayConteoExterno ";
			sSQL += " ,Cli_Nombre";
			...
			sSQL += ", (CASE WHEN Aud_EsCiego = 0 THEN 'No' ELSE 'Si' END) as ConteoCiego "
			sSQL += ", (CASE WHEN Aud_HayConteoExterno = 0 THEN 'Lyde auditoria' ELSE 'Lyde y Cliente auditan' END) as AudHayConteoExterno "
        ...
        if(!rs.EOF) {
            result.Aud_ID = rs.Fields.Item("Aud_ID").Value;
            result.Aud_VisitaActual = rs.Fields.Item("Aud_VisitaActual").Value;
            ...
            result.Aud_HayConteoExterno = rs.Fields.Item("Aud_HayConteoExterno").Value;
            result.ConteoCiego = rs.Fields.Item("ConteoCiego").Value;
            result.AudHayConteoExterno = rs.Fields.Item("AudHayConteoExterno").Value;
            result.Aud_Descripcion = rs.Fields.Item("Aud_Descripcion").Value;
            result.Aud_Nombre = rs.Fields.Item("Aud_Nombre").Value;
            result.Aud_EstatusCG141 = rs.Fields.Item("Aud_EstatusCG141").Value;
        }
    ...
    function getCompleteUbicationsInfo() {
        ...
        var rs = AbreTabla(sSQL,1,0);
        if(!rs.EOF) {
            result.Totals = rs.Fields.Item("Ubi_Totals").Value/2;
            result.PorcentComplete = (rs.Fields.Item("Ubi_Completed").Value/(rs.Fields.Item("Ubi_Totals").Value /2 )*100).toFixed(1);
