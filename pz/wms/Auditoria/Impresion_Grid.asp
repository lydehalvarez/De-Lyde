...
<%
/* HA ID: 2 2021-AGO-27 Ubicacion: Orden de la Ubicacion */
    ...
    /* HA ID: 2 Se agrega parametro de Ordenación */
    var rqBolOrdenarUbicacion = ( Parametro("OrdenarUbicacion", 0) == 1 );
   
/* HA ID: 2 se agrega campo de ordenación */
var sSQL = " SELECT AUDUBI.Aud_ID, AUDUBI.Pt_ID , AUDUBI.AudU_CodigoBarras, AUDUBI.AudU_ConteoInterno "
    ...
    sSQL += ",ISNULL((SELECT UBI.Ubi_Indice FROM Ubicacion UBI WHERE UBI.Ubi_ID = "
    sSQL += "     (SELECT AUDPALL.Ubi_ID FROM Auditorias_Pallet AUDPALL "
    sSQL += "           WHERE AUDPALL.Pt_ID = AUDUBI.Pt_ID  AND AUDPALL.Aud_ID = " 
    sSQL += "                AUDUBI.Aud_ID)),'') AS Ubi_Indice "
    ...
    /* HA ID: 2 INI Se agrega Validación de Ordenación */
    if( rqBolOrdenarUbicacion ){
        sSQL += " ORDER BY Ubi_Indice ASC, ARTICULO_SKU ASC "
    } else {
...
<table class="table table-striped table-hover table-bordered" width="100%">
    ...
    var iRegistros = 0
    var iAudUImpreso = 0
    var AudU_CodigoBarras = 0 
    var AudU_ConteoInterno = 0
	...
	if(!rsInv.EOF){
		while(!rsInv.EOF){
            ...
            AudU_CodigoBarras = rsInv.Fields.Item("AudU_CodigoBarras").Value
            AudU_ConteoInterno = rsInv.Fields.Item("AudU_ConteoInterno").Value
	...
	<tr>
    	...
    	<td class="text-left"><small>
            <% if (AudU_ConteoInterno == 0) {
                    Response.Write("Externo")
                } else {
                    Response.Write("Interno")
                }
                if(rsInv.Fields.Item("AUDITOR").Value != "") {
                     Response.Write("<br>" + rsInv.Fields.Item("AUDITOR").Value)
                }
...
%></small></td>
    	...
    	<td class="text-left"><small><%=AudU_CodigoBarras%></small></td>  
    	<td valign="middle" class="text-center">
			<button class="btn btn-info btnImprimeIndv btn-xs" 
                    data-audid="<%=rsInv.Fields.Item("Aud_ID").Value%>" data-codbar="<%=AudU_CodigoBarras%>"
                    data-ptid="<%=Pt_ID%>"><i class="fa fa-print"></i>&nbsp;&nbsp;<span class="bold">Imprimir</span></button>
		</td>
    </tr>
	<%
            /* HA ID: 2 Se libera memoria */
            Response.Flush()
            rsInv.MoveNext()
        } 
    rsInv.Close()
...
$(document).ready(function() {
    ...
    $('.btnImprimeIndv').click(function(e) {
        e.preventDefault();
        RecepImprime($(this).data("audid"),$(this).data("ptid"), $(this).data("codbar"));
    });        
        
        
    function RecepImprime(f,o,c){
        var newWin=window.open("/pz/wms/Auditoria/Impresion_Papeleta2.asp?Aud_ID="+f+"&PT_ID="+o+"&CB="+c);
    }        
