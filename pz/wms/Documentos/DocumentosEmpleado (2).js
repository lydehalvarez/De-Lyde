...
function LlamaDocumento(EmpID,DocID,DocsID,Cargado) {

	var llaves = "Emp_ID=" + EmpID 
		llaves += "&Doc_ID=" + DocID 
	...
	$('#dvCargaDeInformacion').load("/pz/agt/Documentos/DocumentoEmpleado.asp?"+llaves,function(){
		
		$("#DocsGrid").hide("slow");
		$("#dvCargaDeInformacion").show("slow");
		
		if (DocsID == -1) {
			$.post("/pz/agt/Documentos/DocsEmpleado_Ajax.asp"
			  , { Tarea:3,Emp_ID:EmpID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		} 
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() == -1) {
		$.post( "/pz/agt/Documentos/DocsEmpleado_Ajax.asp"
		      , { Tarea:3,Emp_ID:$("#Emp_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
...
function CargaArchivoAlServidor() {		
	...
	var sRutaArch = "/Media/agt/Empleado/";	
	var sMensaje = ""
					
	var sDatos = "Emp_ID=" + $("#Emp_ID").val();
...
//	});
	...
	*/
	...
	$.ajax({
		url: "/pz/agt/Documentos/uploadAccDocEmpleado.asp?"+sDatos,
		data: data,
		cache: false,
	...
	if(sNomArch.indexOf(".pdf") > -1) {
		$("#dvArchivoSubido").append($('<embed>')
										.attr('src', '/Media/agt/Empleado/' + sNomArch+'#toolbar=0')
										.attr('width','940')
										.attr('height','1000')
									)
	   ...
	   $("#dvArchivoSubido").html(sNomArch)	      
		...
		$("#dvArchivoSubido").append($('<img>')
										.attr('src', '/Media/agt/Empleado/' + sNomArch)
										.attr('border','0')
									)
	}
...
function GuardarABDNombreArchivo(sjsRutaArch) {
	...
	$.post("/pz/agt/Documentos/DocsEmpleado_Ajax.asp", { Tarea:1,Emp_ID:$("#Emp_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
