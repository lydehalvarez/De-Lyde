...
function LlamaDocumento(TAID,DocID,DocsID,Cargado) {

		var llaves = "TA_ID=" + TAID 
		llaves += "&Doc_ID=" + DocID 
	...
	$('#dvCargaDeInformacion').load("/pz/wms/Documentos/DocumentoTransferencia.asp?"+llaves,function(){
		...
		if (DocsID == -1) {
			$.post("/pz/wms/Documentos/DocsTransferencia_Ajax.asp"
			  , { Tarea:3,TA_ID:TAID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		}
