...
function LlamaDocumento(OVID,DocID,DocsID,Cargado) {

		var llaves = "OV_ID=" + OVID 
		llaves += "&Doc_ID=" + DocID 
	...
	$('#dvCargaDeInformacion').load("/pz/wms/Documentos/DocumentoAdminOC.asp?"+llaves,function(){
		
		$("#DocsGrid").hide("slow");
		$("#dvCargaDeInformacion").show("slow");
		//DocsCliente_Ajax.asp
		if (DocsID == -1) {
			$.post("/pz/wms/Documentos/DocsAdminOC_Ajax.asp"
			  , { Tarea:3,OV_ID:OVID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		} 
...
function DocBorrar(){
	...
	$.post("/pz/wms/Documentos/DocsAdminOC_Ajax.asp"
		   //alert($("#Docs_ID").val());
		  , { Tarea:2,OV_ID:$("#OV_ID").val(),Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val() }
