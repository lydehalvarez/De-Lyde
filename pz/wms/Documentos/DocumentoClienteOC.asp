...
if (Cargado == 1) {
	...
	var sCondArchivo = " Cli_ID = " + Cli_ID	
	sCondArchivo = " CliOC_ID = " + CliOC_ID	
	sCondArchivo += " AND Doc_ID = " + Doc_ID		
		sCondArchivo += " AND Docs_ID = " + Docs_ID	
		//Response.Write(sCondArchivo)
		
	var sSQL = "SELECT Cli_ID, CliOC_ID, Doc_ID, Docs_ID, Docs_Nombre, Docs_RutaArchivo, Docs_FechaRegistro"
...
<input type="hidden" name="CliOC_ID" id="CliOC_ID" value="<%=CliOC_ID%>">
...
function DocRechazar(){
	$.post("/pz/wms/Documentos/DocsClienteOC_Ajax.asp"
	  , { Tarea:4,Cli_ID:$("#Cli_ID").val(),CliOC_ID:$("#CliOC_ID").val()
...
//	});
	...
	*/
	...
	$.ajax({
		url: "/pz/wms/Documentos/uploadAccDocClienteOC.asp?"+sDatos,
		data: data,
		cache: false,
...
function GuardarABDNombreArchivo(sjsRutaArch) {
				...
				$.post("/pz/wms/Documentos/DocsClienteOC_Ajax.asp", { Tarea:1,Cli_ID:$("#Cli_ID").val(),CliOC_ID:$("#CliOC_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
