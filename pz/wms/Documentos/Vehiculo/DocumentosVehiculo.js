...
function LlamaDocumento(VehID,DocID,DocsID,Cargado) {

	var llaves = "Veh_ID=" + VehID 
		llaves += "&Doc_ID=" + DocID 
	...
	$('#dvCargaDeInformacion').load("/pz/agt/Documentos/Vehiculo/DocumentoVehiculo.asp?"+llaves,function(){
		
		$("#DocsGrid").hide("slow");
		$("#dvCargaDeInformacion").show("slow");
		
		if (DocsID == -1) {
			$.post("/pz/agt/Documentos/Vehiculo/DocsVehiculo_Ajax.asp"
			  , { Tarea:3,Veh_ID:VehID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		} 
...
function SubeArchivo() {
	...
	/*
	if($("#Docs_ID").val() == -1) {
		$.post( "/pz/cop/Documentos//Vehiculo/DocsVehiculo_Ajax.asp"
		      , { Tarea:3,Veh_ID:$("#Veh_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
                 console.log("DocsID = " + id);
				  $("#Docs_ID").val(id)
			 	 CargaArchivoAlServidor()
			  });
	} else {
...
function CargaArchivoAlServidor() {		
	...
	var sRutaArch = "/Media/agt/Vehiculo/";	
	var sMensaje = ""
					
	var sDatos = "Veh_ID=" + $("#Veh_ID").val();
...
//	});
    ...
    //console.log('size ' + document.getElementById('file').files[0].size);
  //data.append( 'file', $( '#file' )[0].files[0] );
	...
	*/
	...
	$.ajax({
		url: "/pz/agt/Documentos/Vehiculo/uploadAccDocVehiculo.asp?"+sDatos,
		data: data,
		cache: false,
	...
	if(sNomArch.indexOf(".pdf") > -1) {
		$("#dvArchivoSubido").append($('<embed>')
										.attr('src', '/Media/agt/Vehiculo/' + sNomArch+'#toolbar=0')
										.attr('width','940')
										.attr('height','1000')
									)
	   ...
	   $("#dvArchivoSubido").html(sNomArch)	      
		...
		$("#dvArchivoSubido").append($('<img>')
										.attr('src', '/Media/agt/Vehiculo/' + sNomArch)
										.attr('border','0')
									)
	}
...
//							$.jGrowl(sMensaje, { header: 'Guardar Archivo', sticky: false, life: 7000, glue:'before'});	
						...
						Avisa("error",'Error al subir el archivo',sMensaje)		
					}
				  },  
			...
			if(XMLHttpRequest.readyState == 0 || XMLHttpRequest.status == 0) {
              alert(" it's not really an error");
				...
				if (XMLHttpRequest.status == 500) {
					...
					var sMensaje = XMLHttpRequest.status + " " + XMLHttpRequest.statusText + " " + XMLHttpRequest.responseText
                    Avisa("error",'Error al subir el archivo',sMensaje);
				//	
//					var
//						oData = new FormData(document.forms.namedItem("file"));
//					var oReq = new XMLHttpRequest();
//					  oReq.open("POST",  "/pz/agt/Documentos/uploadAccDocServicio.asp?"+sDatos, true);
...
function GuardarABDNombreArchivo(sjsRutaArch) {
	...
	$.post("/pz/agt/Documentos/Vehiculo/DocsVehiculo_Ajax.asp", { Tarea:1,Veh_ID:$("#Veh_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
