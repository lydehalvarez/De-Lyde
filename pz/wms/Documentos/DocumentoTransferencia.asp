...
function DocRechazar(){
	$.post("/pz/wms/Documentos/DocsTransferencia_Ajax.asp"
	  , { Tarea:4,TA_ID:$("#TA_ID").val()
...
function DocGuardar(){
	$.post("/pz/wms/Documentos/DocsTransferencia_Ajax.asp"
	  , { Tarea:3,TA_ID:$("#TA_ID").val(),Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val() 
		 ,Docs_Observaciones:$("#Docs_Observaciones").val(),Docs_Titulo:$("#Docs_Titulo").val()
		 ,Usu_ID:$("#IDUsuario").val() }
	  , function(){
		  	var sMensaje= "El documento fue actualizado correctamente";
			Avisa("success",'Validacion de documentos',sMensaje)	
		});	
		SubeArchivo()
}
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() == -1) {
		$.post( "/pz/wms/Documentos/DocsTransferencia_Ajax.asp"
		      , { Tarea:3,TA_ID:$("#TA_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
...
//	});
	...
	*/
	...
	$.ajax({
		url: "/pz/wms/Documentos/uploadAccDocTransferencia.asp?"+sDatos,
		data: data,
		cache: false,
...
function GuardarABDNombreArchivo(sjsRutaArch) {
	...
	$.post("/pz/wms/Documentos/DocsTransferencia_Ajax.asp", { Tarea:1,TA_ID:$("#TA_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
