...
function LlamaDocumento(VehID,DocID,DocsID,Cargado) {
	...
	$('#dvCargaDeInformacion').load("/pz/agt/Documentos/DocumentoVehiculo.asp?"+llaves,function(){
		
		$("#DocsGrid").hide("slow");
		$("#dvCargaDeInformacion").show("slow");
		
		if (DocsID == -1) {
			$.post("/pz/agt/Documentos/DocsVehiculo_Ajax.asp"
...
//	});
  ...
  //data.append( 'file', $( '#file' )[0].files[0] );
	...
	*/
	...
	$.ajax({
		url: "/pz/agt/Documentos/uploadAccDocVehiculo.asp?"+sDatos,
		data: data,
		cache: false,
...
function GuardarABDNombreArchivo(sjsRutaArch) {
	...
	$.post("/pz/agt/Documentos/DocsVehiculo_Ajax.asp", { Tarea:1,Veh_ID:$("#Veh_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
