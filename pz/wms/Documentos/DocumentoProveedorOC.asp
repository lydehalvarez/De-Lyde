...
<%
	...
	var Doc_Nombre = ""
	var sRuta = "/Media/wms/Documentos/" 
	var sTitulo = "" 
...
if (Cargado == 1) {
	...
	var sCondArchivo = " Prov_ID = " + Prov_ID	
	sCondArchivo = " OC_ID = " + OC_ID	
	sCondArchivo += " AND Doc_ID = " + Doc_ID		
		sCondArchivo += " AND Docs_ID = " + Docs_ID	
		//Response.Write(sCondArchivo)
		
	var sSQL = "SELECT Prov_ID, OC_ID, Doc_ID, Docs_ID, Docs_Nombre, Docs_RutaArchivo, Docs_FechaRegistro"
	    sSQL += " , Docs_Titulo, Docs_NombreArchivo, Docs_Observaciones, Docs_Validado, Docs_UsuarioValido "
	...
	Docs_NombreArchivo = Parametro("Docs_NombreArchivo","")
...
<div class="fileinput input-group fileinput-new" data-provides="fileinput" id="btnSubir">
    ...
    <table width="940px" border="0" cellspacing="0" cellpadding="0">
     ...
     if (Cargado == 1) {
        var aPosition = Docs_NombreArchivo.indexOf(".pdf");
	    if(aPosition > -1) {
            Response.Write("<embed src='"+sRuta+Docs_NombreArchivo+"#toolbar=0' width='940' height='1000' >")		
        } else if(Docs_NombreArchivo.indexOf(".xml")>-1) { 
...
<%  
        } else if(Docs_NombreArchivo.indexOf(".txt")>-1) { 
			NombreXML = Docs_NombreArchivo
		 %>
                <pre class="info"></pre>
                <div id="xmlfile" style="text-align:left">
                    <pre class="formatted" style="text-align:left"></pre>
...
<%  
            ...
            Response.Write("<img src='"+sRuta+Docs_NombreArchivo+"' border='0'>")  
        }
	 }
        %></td>
        <td>&nbsp;</td>
...
<input type="hidden" name="Prov_ID" id="Prov_ID" value="<%=Prov_ID%>">
...
<% if(NombreXML != "") {%>
	...
	var url = "/Media/wms/Documentos/<%=NombreXML%>";
...
function DocRechazar(){
	$.post("/pz/wms/Documentos/DocsProveedorOC_Ajax.asp"
	  , { Tarea:4,Prov_ID:$("#Prov_ID").val(),OC_ID:$("#OC_ID").val()
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() == -1) {
		$.post( "/pz/wms/Documentos/DocsProveedorOC_Ajax.asp"
		      , { Tarea:3,Prov_ID:$("#Prov_ID").val(),OC_ID:$("#OC_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
...
function CargaArchivoAlServidor() {		
	...
	var sRutaArch = "/Media/wms/Documentos/";	
	var sMensaje = ""
					
		var sDatos = "Prov_ID=" + $("#Prov_ID").val();
		sDatos = "OC_ID=" + $("#OC_ID").val();
...
//	});
	...
	*/
	...
	$.ajax({
		url: "/pz/wms/Documentos/uploadAccDocProveedorOC.asp?"+sDatos,
		data: data,
		cache: false,
	...
	if(sNomArch.indexOf(".pdf") > -1) {
		$("#dvArchivoSubido").append($('<embed>')
										.attr('src', '/Media/wms/Documentos/' + sNomArch+'#toolbar=0')
										.attr('width','940')
										.attr('height','1000')
									)
	   ...
	   $("#dvArchivoSubido").html(sNomArch)	      
		...
		$("#dvArchivoSubido").append($('<img>')
										.attr('src', '/Media/wms/Documentos/' + sNomArch)
										.attr('border','0')
									)
	}
...
function GuardarABDNombreArchivo(sjsRutaArch) {
				...
				$.post("/pz/wms/Documentos/DocsProveedorOC_Ajax.asp", { Tarea:1,Prov_ID:$("#Prov_ID").val(),OC_ID:$("#OC_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
