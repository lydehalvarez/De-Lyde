...
function LlamaDocumento(SerID,DocID,DocsID,Cargado) {

	var llaves = "Ser_ID=" + SerID 
		llaves += "&Doc_ID=" + DocID 
	...
	$('#dvCargaDeInformacion').load("/pz/agt/Documentos/DocumentoServicio.asp?"+llaves,function(){
		
		$("#DocsGrid").hide("slow");
		$("#dvCargaDeInformacion").show("slow");
		
		if (DocsID == -1) {
			$.post("/pz/agt/Documentos/DocsServicio_Ajax.asp"
			  , { Tarea:3,Ser_ID:SerID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		} 
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() == -1) {
		$.post( "/pz/agt/Documentos/DocsServicio_Ajax.asp"
		      , { Tarea:3,Ser_ID:$("#Ser_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
...
function CargaArchivoAlServidor() {		
	...
	var sRutaArch = "/Media/agt/Servicio/";	
	var sMensaje = ""
					
	var sDatos = "Ser_ID=" + $("#Ser_ID").val();
...
//	});
	...
	*/
	...
	$.ajax({
		url: "/pz/agt/Documentos/uploadAccDocServicio.asp?"+sDatos,
		data: data,
		cache: false,
	...
	if(sNomArch.indexOf(".pdf") > -1) {
		$("#dvArchivoSubido").append($('<embed>')
										.attr('src', '/Media/agt/Servicio/' + sNomArch+'#toolbar=0')
										.attr('width','940')
										.attr('height','1000')
									)
	   ...
	   $("#dvArchivoSubido").html(sNomArch)	      
		...
		$("#dvArchivoSubido").append($('<img>')
										.attr('src', '/Media/agt/Servicio/' + sNomArch)
										.attr('border','0')
									)
	}
...
function GuardarABDNombreArchivo(sjsRutaArch) {
	...
	$.post("/pz/agt/Documentos/DocsServicio_Ajax.asp", { Tarea:1,Ser_ID:$("#Ser_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
