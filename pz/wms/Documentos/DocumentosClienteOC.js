...
function LlamaDocumento(CliID,CliOCID,DocID,DocsID,Cargado) {
	...
	$('#dvCargaDeInformacion').load("/pz/wms/Documentos/DocumentoClienteOC.asp?"+llaves,function(){
		...
		if (DocsID == -1) {
			$.post("/pz/wms/Documentos/DocsCliente_Ajax.asp"
			  , { Tarea:3,Cli_ID:CliID,CliOC_ID:CliOCID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		} 
...
function DocBorrar(){
	...
	$.post("/pz/wms/Documentos/DocsCliente_Ajax.asp"
		  , { Tarea:2,Cli_ID:$("#Cli_ID").val(),CliOC_ID:$("#CliOC_ID").val()
