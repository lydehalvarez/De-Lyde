...
if (Cargado == 1) {
	...
	var sSQL = "SELECT Cli_ID, Doc_ID, Docs_ID, Docs_Nombre, Docs_RutaArchivo, Docs_FechaRegistro"
	    sSQL += " , Docs_Titulo, Docs_Observaciones, Docs_Validado, Docs_UsuarioValido "
		sSQL += " FROM Cliente_Documentos "
...
<% if(NombreXML != "") {%>
	...
	var url = "/Media/agt/Cliente/<%=NombreXML%>";
...
function DocGuardar(){
	$.post("/pz/agt/Documentos/DocsCliente_Ajax.asp"
	  , { Tarea:5,Cli_ID:$("#Cli_ID").val(),Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val() 
