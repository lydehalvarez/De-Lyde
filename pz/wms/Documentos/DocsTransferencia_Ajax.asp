...
<%	
	...
	switch (parseInt(Tarea)) {
		...
		case 1:  //Guardando documento
			try {
				var SSQL = "SELECT ISNULL(MAX(Docs_ID)+1 ,1) AS Docs_ID FROM TransferenciaAlmacen_Documentos WHERE  TA_ID="+ TA_ID + " AND Doc_ID=" + Doc_ID 
				var rsDocs = AbreTabla(SSQL, 1,0)
				if(!rsDocs.EOF){
				var Docs_ID=rsDocs.Fields.Item("Docs_ID").Value
				}
			
				var sSQLACT =  " INSERT INTO TransferenciaAlmacen_Documentos (TA_ID, Doc_ID "
					sSQLACT += ", Docs_Nombre, Docs_RutaArchivo, Docs_Titulo, Docs_Observaciones) " 
					sSQLACT += " VALUES (" + TA_ID + "," + Doc_ID 
		...
		case 2:  //Borrando documento de la bd
			try {
				...
				var sSQLACT =  " DELETE FROM TransferenciaAlmacen_Documentos "	
	                sSQLACT += " WHERE TA_ID = " + TA_ID
		...
		case 3:  //Buscando el siguiente ID
			try {	
				...
				sResultado = SiguienteID("Docs_ID","TransferenciaAlmacen_Documentos",sCondicion,0)
		...
		case 4:  //Borrando documento de la bd
			try {
				...
				var sSQLACT =  " UPDATE TransferenciaAlmacen_Documentos "	
