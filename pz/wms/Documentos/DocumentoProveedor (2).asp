...
if (Cargado == 1) {
	...
	var sSQL = "SELECT Prov_ID, Doc_ID, Docs_ID, Docs_Nombre, Docs_RutaArchivo, Docs_FechaRegistro"
	    sSQL += " , Docs_Titulo, Docs_Observaciones, Docs_Validado, Docs_UsuarioValido "
		sSQL += " FROM Proveedor_Documentos "
...
<% if(NombreXML != "") {%>
	...
	var url = "/Media/agt/Proveedor/<%=NombreXML%>";
...
function DocGuardar(){
	$.post("/pz/agt/Documentos/DocsProveedor_Ajax.asp"
	  , { Tarea:5,Prov_ID:$("#Prov_ID").val(),Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val() 
