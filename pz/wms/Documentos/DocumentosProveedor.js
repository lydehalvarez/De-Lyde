...
function LlamaDocumento(ProvID,DocID,DocsID,Cargado) {
	...
	$('#dvCargaDeInformacion').load("/pz/agt/Documentos/DocumentoProveedor.asp?"+llaves,function(){
		
		$("#DocsGrid").hide("slow");
		$("#dvCargaDeInformacion").show("slow");
		
		if (DocsID == -1) {
			$.post("/pz/agt/Documentos/DocsProveedor_Ajax.asp"
			  , { Tarea:3,Prov_ID:ProvID,Doc_ID:DocID,Docs_ID:DocsID }
			  , function(ID){
				  $("#Docs_ID").val(ID) 
				});
		} 
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() == -1) {
		$.post( "/pz/agt/Documentos/DocsProveedor_Ajax.asp"
		      , { Tarea:3,Prov_ID:$("#Prov_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
...
function CargaArchivoAlServidor() {		
	...
	var sRutaArch = "/Media/agt/Proveedor/";	
	var sMensaje = ""
...
//	});
	...
	*/
	...
	$.ajax({
		url: "/pz/agt/Documentos/uploadAccDocProveedor.asp?"+sDatos,
		data: data,
		cache: false,
	...
	if(sNomArch.indexOf(".pdf") > -1) {
		$("#dvArchivoSubido").append($('<embed>')
										.attr('src', '/Media/agt/Proveedor/' + sNomArch+'#toolbar=0')
										.attr('width','940')
										.attr('height','1000')
									)
	   ...
	   $("#dvArchivoSubido").html(sNomArch)	      
		...
		$("#dvArchivoSubido").append($('<img>')
										.attr('src', '/Media/agt/Proveedor/' + sNomArch)
										.attr('border','0')
									)
	}
...
//					  oReq.open("POST",  "/pz/agt/Documentos/uploadAccDocProveedor.asp?"+sDatos, true);
...
function GuardarABDNombreArchivo(sjsRutaArch) {
	...
	$.post("/pz/agt/Documentos/DocsProveedor_Ajax.asp", { Tarea:1,Prov_ID:$("#Prov_ID").val()
			 ,Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
