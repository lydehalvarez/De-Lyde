...
<%
	var DsbR_ID = Parametro("DsbR_ID",-1)
   	...
   	var FI = Parametro("FI","")
        FI = CambiaFormatoFecha(FI,"dd/mm/yyyy",FORMATOFECHASERVIDOR)
   	var FF = Parametro("FF","")
        FF = CambiaFormatoFecha(FF,"dd/mm/yyyy",FORMATOFECHASERVIDOR)
   	...
   	var sSQLEtq  = DsbC_Query
        sSQLEtq += " and  Inv_FechaRegistro >= '" + FI + "'"
        sSQLEtq += " and  Inv_FechaRegistro <= '" + FF + "'" 
...
<div class="ibox float-e-margins">
	<div class="ibox-title">
		<span class="label label-success pull-right"><%=Estatus %></span>
        <h5><%=Titulo %></h5>
    </div>
    <div class="ibox-content">
    	<h1 class="no-margins"><%=Dato %></h1>
        <div class="stat-percent font-bold text-success"><%=Porcentaje %><i class="fa <%=Flecha %>"></i></div>
