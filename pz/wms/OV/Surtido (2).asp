...
<%
	...
	var Ordenes = "SELECT * "
		...
		Ordenes += " AND OV_CUSTOMER_NAME != 'JOSEFA  DAVILA LOPEZ' "
                ...
                <div class="ibox-content">
                    ...
                    <div class="form-group">
                      <div class="col-lg-12"  style="text-align: right;">
                            <input type="button" class="btn btn-info btnDownloadCorte" value="Descarga corte <%=Cort_ID%>"/>
                ...
                <%
                    ...
                    while (!rsOrd.EOF){
						...
						var Articulos = "SELECT * "
							          + " ,(SELECT OV_Folio FROM Orden_Venta p WHERE p.OV_ID = a.OV_ID) Folio"
							          + " ,ISNULL((SELECT ISNULL(Pro_Nombre,'') "
                                      + " FROM Producto p WHERE p.Pro_ClaveAlterna = a.OVA_PART_NUMBER)"
                                      + ",(SELECT ISNULL(Pro_Nombre,'') FROM Producto p "
                                      + " WHERE p.Pro_SKU = a.OVA_PART_NUMBER) "
                                      + " ) PRO_NOM "
							          + " FROM Orden_Venta_Articulo a"
							          + " WHERE OV_ID = " + OV_ID
                            ...
                            <table class="table shoping-cart-table">
                                ...
                                <tr>
                                    <td class="desc">
                                        ...
                                        <input type="text" value="" style="display:none;width:50%;color:#000;" data-ovid="<%=OV_ID%>" placeholder="Numero de serie" class="form-control InputStartPick" id="InputStartPick<%=OV_ID%>"/>
...
$(document).ready(function(e) {
	...
	$('.btnDownloadCorte').on('click',function(e){
		e.preventDefault()
		var btn = $(this)
		btn.val("Cargando corte...")
		btn.prop('disabled',true)
		$.post("/pz/wms/OV/DataCorte.asp",{Cort_ID:<%=Cort_ID%>}
		  , function(data){
			var response = JSON.parse(data)
			var ws = XLSX.utils.json_to_sheet(response);
			var wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Corte <%=Cort_ID%>");
			XLSX.writeFile(wb, "Corte <%=Cort_ID%> "+Fecha+" .xlsx");
			btn.prop('disabled',false)
			btn.val("Descarga corte <%=Cort_ID%>")
		});  
	});
...
function Picked(dato,input){
	$.post("/pz/wms/OV/OV_Ajax.asp",dato,function(data){
		...
		$('#SeriePickeada'+dato.OV_ID).html(dato.OVP_Serie+"<br>")
		...
		if(response.result < 0){
			...
			$('#SeriePickeada'+dato.OV_ID).html(response.message).css('color','red')
		}
		...
		$.ajax({
			...
			success: function(datos){
				...
				if(datos.result == 1){
					...
					var jsonMessage = datos.data
					swal({
					  title: "Detener folio, error en Izzi",
					...
					$.post("/pz/wms/OV/OV_Ajax.asp",{
						...
						OV_ID:request.OV_ID
							...
							sTipo = "info";   
							sMensaje = "Series borradas para volver a hacer picking";
							Avisa(sTipo,"Aviso",sMensaje);	
							console.log("Picking borrado OV_ID = "+request.OV_ID)
					});
