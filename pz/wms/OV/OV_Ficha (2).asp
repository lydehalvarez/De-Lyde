...
<%
	...
	//HA ID: 10 2021-ENE-08 Se agrega condición de Visualización de edición de Dirección 
	//HA ID: 11 2021-MAR-16 Se agrega validación de Existencia de Transportista
...
//			+ ", (SELECT Nombre FROM dbo.tuf_Usuario_Informacion(OV_UsuarioCambioDireccion) ) AS NombreUsuarioCambio " 
			+ ", ISNULL( (SELECT TOP 1 CONVERT(NVARCHAR(50), OV_FechaCambio, 103) FROM Orden_Venta_Historico_Direccion WHERE OV_ID = A.OV_ID ORDER BY OV_FechaCambio DESC ), '') AS OV_FechaModificada "
...
//			}
			...
			swal("Folio NO enviado a Fallido", "Folio no ha sufrido ningun tipo de cambio", "success");
		}
...
<% 		}
	} //HA ID: 11 FIN de existencia de Transportista
%>
                                </dl>
                            </div>	
<% 
...
// HA ID: 5 INI Se agrega Tabs para organizar la información
	...
	//HA ID: 11 INI Existencia de Transportista
	if( !(bolEsTransportista) ){
%>
...
<%
	} // HA ID: 11 FIN Existencia de Transportista
// HA ID: 5 FIN
%>
                        </div>
