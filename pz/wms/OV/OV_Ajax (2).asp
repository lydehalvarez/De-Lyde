...
switch (parseInt(Tarea)) {
		case 1:	
			try {  
				var sSQLACT = "SELECT Inv_EstatusCG20,Pro_ID,Inv_ID "
...
//			3 y ya se acabó
		...
		if(OVP_Serie.length == 3){
		var InvSerie =  "SELECT Inv_EstatusCG20,Cli_ID,Pro_ID,Inv_ID "
			InvSerie += " FROM Inventario "
			InvSerie += " WHERE Inv_Serie = '0"+OVP_Serie+"' "
			
		}else{
		var InvSerie =  "SELECT Inv_EstatusCG20,Cli_ID,Pro_ID,Inv_ID "
			InvSerie += " FROM Inventario "
			InvSerie += " WHERE Inv_Serie = '"+OVP_Serie+"' "
			
		}
		
			
		var Disponibilidad = -1	
		var ProductoDelCliente = -1
		var Pro_ID = -1
		var Inv_ID = -1
		var rsInv = AbreTabla(InvSerie,1,0)
		if(!rsInv.EOF){
			Disponibilidad  = rsInv.Fields.Item("Inv_EstatusCG20").Value
			ProductoDelCliente  = rsInv.Fields.Item("Cli_ID").Value
			Pro_ID  = rsInv.Fields.Item("Pro_ID").Value			
			Inv_ID  = rsInv.Fields.Item("Inv_ID").Value			
		}
		
		
//			var Disponibilidad = BuscaSoloUnDato("Inv_EstatusCG20","Inventario","Inv_Serie = '"+OVP_Serie+"'",-1,0)
			if(Disponibilidad == 1){ 
				var Existencia = BuscaSoloUnDato("OVP_ID","Orden_Venta_Picking","OVP_Serie = '"+OVP_Serie+"' AND OV_ID ="+OV_ID,-1,0)
				if(Existencia == -1){
//					var ProductoDelCliente = BuscaSoloUnDato("Cli_ID","Inventario","Inv_Serie = '"+OVP_Serie+"' AND Cli_ID = "+Cli_ID,-1,0)
						if(Cli_ID == ProductoDelCliente){				
//							var OVA_PART_NUMBER = BuscaSoloUnDato("Pro_ClaveAlterna","Producto p, Inventario i"
//							,"i.Inv_Serie = '"+OVP_Serie+"' AND i.Pro_ID = p.Pro_ID",-1,0)
//							
							var OVA_ID = BuscaSoloUnDato("OVA_ID","Orden_Venta_Articulo","OV_ID="+OV_ID+" AND Pro_ID = "+Pro_ID,-1,0)
							
							if(OVA_ID != -1){
								OVP_ID = BuscaSoloUnDato("ISNULL(MAX(OVP_ID),0)+1","Orden_Venta_Picking","OV_ID ="+OV_ID+" AND OVA_ID = "+OVA_ID,-1,0)
				
								if(OVP_ID <= Limite){	
									var CorteInse = "INSERT INTO Orden_Venta_Picking(OV_ID, OVA_ID,OVP_ID,OVP_Serie,OVP_Usuario,Pro_ID,Inv_ID)"
										CorteInse += "VALUES("+OV_ID+","+OVA_ID+","+OVP_ID+",'"+OVP_Serie+"',"+IDUsuario+","+Pro_ID+","+Inv_ID+")"
										
										if(Ejecuta(CorteInse,0)){
											
											var InventarioUpdate = "UPDATE Inventario" 
												InventarioUpdate += " SET Inv_EstatusCG20 = 14"
												InventarioUpdate += " WHERE Inv_ID = "+Inv_ID
		...
		//										Values ( 2, 1, Acc_ID, OV_ID ) 
												...
												var LastPicker = "UPDATE Orden_Venta"
													LastPicker += " SET OV_UsuarioCambioEstatus = "+IDUsuario
													LastPicker += " WHERE OV_ID = "+OV_ID
													
													Ejecuta(LastPicker,0)
											...
											sResultado =  '{"result":-1,"OVA_ID":'+OVA_ID+',"message":"Error en el picking, intente de nuevo"}'
										}
								}else{
			...
			sResultado =  '{"result":-1,"message":"Notificar al &aacute;rea de sistemas"}'
		}
		...
		if(!rsInfo.EOF){
			...
			if(Estatus != LastStatus){
				if((Estatus > LastStatus) || (LastStatus == 16)){
				
					var Tacos = "UPDATE Orden_Venta"
						Tacos += " SET OV_UsuarioCambioEstatus = "+IDUsuario
		...
		case 14:
			var OV_Folio = Parametro("OV_Folio","")
			var QuienRecibe = Parametro("QuienRecibe","")
			
			try{
				
				var upQuien = "UPDATE Orden_Venta"
					upQuien += " SET OV_PersonaRecibePaquete = '"+QuienRecibe+"'"
					upQuien += " , OV_FechaEntrega = getdate() "
					upQuien += " WHERE OV_Folio = '"+OV_Folio+"' "
