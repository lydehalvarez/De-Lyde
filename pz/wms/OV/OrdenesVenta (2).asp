...
<%
	...
	var sSQLO = "  SELECT *"
		sSQLO += " , h.Cat_Nombre as Estatus "
		sSQLO += " FROM Orden_Venta a, Cat_Catalogo h"
		sSQLO += " WHERE OV_Test = 0 AND h.Sec_ID = 51 AND h.Cat_ID = a.OV_EstatusCG51"
		sSQLO += " AND Cort_ID = 0"
		sSQLO += " AND OV_CUSTOMER_NAME NOT LIKE 'PRUEBAS%' "
		sSQLO += " ORDER BY OV_ID DESC "
		
	var sSQLNUM = "  SELECT COUNT(OV_ID) Cantidad"
		sSQLNUM += " FROM Orden_Venta"
		sSQLNUM += " WHERE OV_Test = 0"
		sSQLNUM += " AND Cort_ID = 0"
		sSQLNUM += " AND OV_CUSTOMER_NAME NOT LIKE 'PRUEBAS%' "
