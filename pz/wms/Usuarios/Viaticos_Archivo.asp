...
var sSQL  = " SELECT * "
    sSQL += " FROM Empleado_Viaticos_Detalle " 
	sSQL += " WHERE Emp_ID = " + Emp_ID
	sSQL += " AND EVtc_ID = " + EVtc_ID
	sSQL += " AND EVtcD_ID = " + EVtcD_ID	 

var rsPersona = AbreTabla(sSQL,1,0)

if (!rsPersona.EOF){
	DetDescripcion = rsPersona.Fields.Item("EVtcD_Descripcion").Value
}
rsPersona.Close() 

var EstatusCSS = "" 
var EVtc_EstatusCG89 = 0
var EVtc_Folio = ""
 
 var sSQL  = " SELECT * "
	 sSQL += " ,dbo.fn_CatGral_DameDato(89,EVtc_EstatusCG89) as Estatus "
	 sSQL += " FROM Empleado_Viaticos " 
	 sSQL += " WHERE Emp_ID = " + Emp_ID
	 sSQL += " AND EVtc_ID = " + EVtc_ID
...
var rsOrdenCompra = AbreTabla(sSQL,1,0)
  if (!rsOrdenCompra.EOF){
	  ...
	  EVtc_EstatusCG89 = rsOrdenCompra.Fields.Item("EVtc_EstatusCG89").Value	  
	  EVtc_Folio = rsOrdenCompra.Fields.Item("EVtc_Folio").Value
	  if(EVtc_EstatusCG89 == 1 ) {
	  	EstatusCSS = " label-primary "
      }
	  if(EVtc_EstatusCG89 == 2 ) {
	  	EstatusCSS = " label-success "
      } 	
	  if(EVtc_EstatusCG89 == 3 ) {
...
<% if(Doc_ID==7){  %>    
        <div class="row">
          <div class="col-md-2"><h4>Folio Factura</h4></div>
            <div class="col-md-8"><% 
                if (Cargado == 1) { 
                Response.Write("<h4>"+EVtc_Folio+"</h4>")
                } else { %>
                <input type="text" class="form-control arTexto" id="EVtc_Folio" name="EVtc_Folio" placeholder="Folio de la factura">
                <% } %><br />
          </div>
        </div>        
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() > -1) {
		...
		$.post( "/pz/agt/Usuarios/Usuarios_Ajax.asp"
		      , { Tarea:12,Emp_ID:$("#Emp_ID").val(),EVtc_ID:$("#EVtc_ID").val(),EVtcD_ID:$("#EVtcD_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
			 	  CargaArchivoAlServidor()
			  });
...
function CargaArchivoAlServidor() {			
	...
	var sRutaArch = "/Media/agt/EmpViaticos/";	
					
	var sDatos = "llaves=" + $("#Emp_ID").val() + "_" + $("#EVtc_ID").val() + "_" + $("#EVtcD_ID").val() + "_" + $("#Doc_ID").val() + "_" + $("#Docs_ID").val();
		sDatos += "&Ruta=/agt/EmpViaticos/";	
...
function GuardarABDNombreArchivo(sjsRutaArch, sNomArchivo) {
	...
	$.post("/pz/agt/Usuarios/Usuarios_Ajax.asp", { Tarea:13,Emp_ID:$("#Emp_ID").val()
			 ,EVtc_ID:$("#EVtc_ID").val(),EVtcD_ID:$("#EVtcD_ID").val(),Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val()
<% if(Doc_ID==7){  %>  ,EVtc_Folio:$("#EVtc_Folio").val() <% }  %>    
