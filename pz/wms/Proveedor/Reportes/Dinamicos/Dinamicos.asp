...
%> 
 ...
 %> 	
    ...
    <li><h4><a  href="javascript:CargarReportes(<%=rsRep.Fields.Item("Rep_ID").Value%>,'<%=rsRep.Fields.Item("Rep_Nombre").Value%>')">
...
<div id="dvVentana" style="width: 70%; min-height: 325px; background-color: rgb(255, 255, 255); box-shadow: rgb(119, 119, 119) 0px 10px 6px -6px; position: fixed; top: 85px; left: 15%; z-index: 2010; display: none;">
...
<input name="NombreReporte"  id="NombreReporte" type="hidden" value="" />
...
function CargarReportes(Rep_ID,h){
	$("#NombreReporte").val(h)
	var sData = "Rep_ID=" + Rep_ID
		...
		$('#dvVentanaContenido').fadeOut(800, function() {
			$(this).empty()
				   .load("/pz/wms/Proveedor/Reportes/Dinamicos/Filtro.asp?" + sData)
				   .fadeIn(1500);
