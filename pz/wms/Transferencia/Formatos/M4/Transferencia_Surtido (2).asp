...
%>
<input type="button" value="Guia AG" onclick='window.open("http://wms.lyde.com.mx/Media/wms/Etiqueta/EtiquetaAG.asp")' class="btn btn-info btnGuiaAg"/>

<input type="button" value="Etiqueta Elektra" onclick='window.open("http://qawms.lyde.com.mx/pz/wms/OV/EtiquetaElektra.asp")' class="btn btn-info btnEtiqueta"/>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="ibox">
                    ...
                    <input type="button" value="Ingresar pallet" data-taid="<%=TA_ID%>"  class="btn btn-info btnIngresarPallet"/>
...
$(".btnFormatoTelcel_DAT").click(function(){
	...
	var url = "/pz/wms/Transferencia/Formatos/M4/FormatoTelcel_Dat.asp?Mov_ID="+Mov_ID
...
$(".btnFormatoTelcel_BAN").click(function(){
	...
	var url = "/pz/wms/Transferencia/Formatos/M4/FormatoTelcel_Ban.asp?Mov_ID="+Mov_ID+"&Hora="+Hora+"&Minutes="+Minutes+"&Fecha="+Fecha+"&Factura="+Fac+"&Clave="+Clave+"&SKU="+SKU
...
var FunctionPallet = {
	AddPallet:function(TA_ID,Cajas){
		$.post("/pz/wms/Transferencia/Formatos/M4/Palletizado_Ajax.asp",
		{
			Tarea:2,
	...
	PutSerie:function(h,f,g,s,k,e){
		...
		if (e.keyCode == 13) {
			$.post("/pz/wms/Transferencia/Formatos/M4/Palletizado_Ajax.asp",request
			, function(data){
				var response = JSON.parse(data)
	...
	GetTabla:function(TA_ID,h){
		Renglon++
		$.post("/pz/wms/Transferencia/Formatos/M4/Pallet.asp",
		{Mov_ID:h,
		Renglon:Renglon
		}
		, function(data){ 
	...
	PrintLabel:function(Mov_ID,MovP_ID,MovM_ID){
		...
		window.open("http://qawms.lyde.com.mx/pz/wms/Transferencia/Formatos/M4/EtiquetaM4.asp?"+Datos)
	}
}
