...
<%
	...
	var sSQLO = " SELECT "
		...
		sSQLO += " ORDER BY  Ope_ApPaterno ASC "
...
<body>
    <table class="table table-striped table-bordered" id="Actividades" >
        <thead>
            <th>&nbsp;</th>
            <th style="text-align:center;" colspan="<%var NLimite = Limite + 4; Response.Write(NLimite)%>"><%=sMes%></th>
        </thead>
        <thead>
        <tr> 
            <th>Folio Lyde</th>
...
$(document).ready(function(e) {
	var Ver = <%=Ver%>
	if(Ver == 0){
		//window.open('data:application/vnd.ms-excel,' + $('#Actividades').html());
		 //exportTableToExcel('Actividades', )
		 $("#Actividades").table2excel({
			name:  'Tabla de asistencia <%=sMes%>',
			filename: 'Tabla de asistencia <%=sMes%>.xls', // do include extension
