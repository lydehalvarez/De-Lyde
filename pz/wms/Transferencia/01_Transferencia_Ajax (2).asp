	...
	switch (parseInt(Tarea)) {
		...
		var TA_ID = Parametro("TA_ID",-1) 
			...
			try {
					...
					var USQL2 = " UPDATE TransferenciaAlmacen "
						USQL2 += " SET TA_Start_Warehouse_ID = (SELECT ISNULL(Alm_ID,-1) FROM Almacen WHERE Alm_Nombre = [TA_Start_Warehouse])"
						USQL2 += " , TA_End_Warehouse_ID = (SELECT ISNULL(Alm_ID,-1) FROM Almacen WHERE Alm_Nombre = [TA_End_Warehouse])"
						USQL2 += " WHERE TA_ID = "+TA_ID
