...
switch( parseInt(rqIntTarea) ){
	...
	case 1: {
		...
		var sqlCliCr = "SELECT CliCr_ID, CliCr_Nombre "
			+ " FROM Cliente_Contrato_Corte  "
			+ " WHERE Cli_ID = " + rqIntCli_ID  
			+ " ORDER BY CliCr_FechaCorte DESC; "
...
<%		
		while( !(rsCliCr.EOF) ){
%>
		<option value="<%= rsCliCr.Fields.Item("CliCr_ID").Value %>">
        	<%= rsCliCr.Fields.Item("CliCr_Nombre").Value  %>
        </option>
<%
			rsCliCr.MoveNext
		}
...
%>
		<div class="ibox ">
            <div id="divNumerosSerie" class="forum-item active ibox-content" style="overflow: auto;">
