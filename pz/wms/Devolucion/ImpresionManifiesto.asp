...
+"INNER JOIN Almacen a on t.TA_End_Warehouse_ID = a.Alm_ID INNER JOIN Manifiesto_Devolucion m ON m.ManD_ID=t.ManD_ID "
+"INNER JOIN Proveedor p ON m.Prov_ID=p.Prov_ID LEFT OUTER JOIN Cat_Catalogo ct ON (ct.Cat_ID=ManD_TipoDeRutaCG94  and ct.Sec_ID =94)" 
+" LEFT OUTER JOIN Cat_Estado e ON e.Edo_ID=m.Edo_ID LEFT OUTER JOIN Cat_Aeropuerto ae ON ae.Aer_ID=m.Aer_ID "
+"WHERE  m.ManD_ID="+ManD_ID+" ORDER BY m.Edo_ID desc"
				
		
	var sSQLM = " SELECT * "	
		sSQLM += " FROM Manifiesto_Devolucion "		
		sSQLM += " WHERE ManD_ID = "+ManD_ID		
				
	var rsRe = AbreTabla(sSQLM,1,0)
	
	
	var rsRe = AbreTabla(sSQL,1,0)
	var Operador = rsRe.Fields.Item("ManD_Operador").Value
	var Placas = rsRe.Fields.Item("ManD_Placas").Value
	var Descripcion = rsRe.Fields.Item("ManD_Vehiculo").Value
	...
	var Ruta = "R " + rsRe.Fields.Item("ManD_Ruta").Value
