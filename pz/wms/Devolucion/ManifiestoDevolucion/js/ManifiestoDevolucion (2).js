...
var ManifiestoDevolucion = {
            ...
            $.ajax({
                ...
                , success: function(res){
                    if( res.Error.Numero == 0 ){
						...
						ManifiestoDevolucion.LanzaAlerta("Operaci&oacute;n exitosa!",res.Error.Descripcion,"success");
                        ManifiestoDevolucion.DecisionCancelacionModalCerrar();
                        ManifiestoDevolucion.DecisionListadoCargar();

                    } else {
						ManifiestoDevolucion.LanzaAlerta("Ups!",res.Error.Descripcion,"error");
    ...
    , DecisionFallidoEnviar: function(){
        ...
        if( bolError ){
            ...
            $.ajax({
                ...
                , success: function(res){
                    if( res.Error.Numero == 0 ){
						ManifiestoDevolucion.LanzaAlerta("SO Cancelada!",res.Error.Descripcion,"success");
                        //Avisa("success", "Manifiesto Devolucion", res.Error.Descripcion);
                        ManifiestoDevolucion.DecisionListadoCargar();
                    } else {
						ManifiestoDevolucion.LanzaAlerta("Ups!",res.Error.Descripcion,"error");
    ...
    , DetalleCargar: function(){
		...
		$("#loadingLateral").show('slow',function (){
			$("#divLateral").hide('slow');
			...
			$.ajax({
				...
				, success: function(res){
					$("#divLateral").html(res);
					$("#loadingLateral").hide('slow',function(){
						$("#divLateral").show('slow');
					})
				}
			})
		});
    ...
    , LateralLimpiar: function(){
        $("#divLateral").html("");
    },LanzaAlerta:function(Titulo,Descripcion,Tipo){
		swal({
			title: Titulo,
			text: Descripcion,
			type: Tipo,
