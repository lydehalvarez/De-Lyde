...
//			}
//				}
			 var Rollo = rsRol.Fields.Item("RolloActual").Value
		//	}else{
//			
var Cantidad  = "SELECT COUNT(*) Cantidad "
			Cantidad  += " FROM Producto_Movimiento_Series  "
...
//		   var rsTPro = AbreTabla(sSQLTr,1,0) 
		...
		%>
            ...
            <td><%=ProM_Cantidad%></td>
                ...
                <option value="" ></option>
                <%
				var sSQL = "SELECT ProMR_Rollo FROM Producto_Movimiento_Rollos WHERE ProM_ID = "+ProM_ID
           ...
           <%	
			...
			%>
         	<input class="form-control Serie"  id= "Serie" style="display:none;width:100%" placeholder="escanea el EPC" type="text" autocomplete="off" value="" onkeydown= "FunctionRecepcion.InsertSerie(event);"/>
           	<input class="form-control inputRol"  id="inputRol" placeholder="cantidad" type="text" autocomplete="off" value=""/>
			<button type="button" class="btn btn-primary BtnRol" id= "BtnRol"  onclick="FunctionRollo.InsertRollo()">
...
-->        
          ...
          <!--  ""-->
			...
			A&ntilde;adir Rollo</button>
           	<input class="form-control Serie" id= "Serie" placeholder="Escanea el EPC" type="text" autocomplete="off" value="" onkeydown="FunctionRecepcion.InsertSerie(event);" />
        	<button type="button" class="btn btn-primary BtnUbic" style="display:none;" id= "BtnUbic" data-toggle="modal" data-target="#myModal" onclick=		
            "FunctionUbic.InsertUbic()">Asignar ubicacion vacia</button>
        	<button type="button" class="btn btn-primary BtnMuestraUbic" style="display:none;" id= "BtnMuestraUbic" data-toggle="modal" data-target="#myModal" onclick=		
           "Ubicacion.SeleccionAvanzadaAbrir({Selector: 'inpUbiID', Etiqueta: 'lblUbiNombre'});">Asignar ubicacion manual</button>
           ...
           <%	
			...
			%>
                      	       <input type="hidden" class="form-control Alfanum" id="Alfanum" value="0"/>
...
<%
	...
	if(caracteres == 16){
		...
		if(Existencia > -1){
			...
			sSQLTr  += " VALUES("+Pro_ID+","+Cli_ID+","+ProM_ID+","+Rollo+",'"+Serie+"')"
		...
		if(!rsCan.EOF){
			...
			var sSQLTr  = "SELECT ProMR_Cantidad FROM Producto_Movimiento_Rollos WHERE ProMR_Rollo= "+Rollo
      			  sSQLTr += " AND ProM_ID = "+ProM_ID

		   var rsCan = AbreTabla(sSQLTr,1,0) 
		   	if(!rsCan.EOF){
			var Rollo_Cantidad = rsCan.Fields.Item("ProMR_Cantidad").Value
			}
	...
	var SeriesR  = "SELECT COUNT(*) SeriesR "
				...
				SeriesR  += " WHERE  ProM_ID = "+ProM_ID
	
		var rsCan = AbreTabla(SeriesR,1,0) 
		if(!rsCan.EOF){
			SeriesR = rsCan.Fields.Item("SeriesR").Value
		}
		 var sSQLTr  = "SELECT ProMR_Cantidad FROM Producto_Movimiento_Rollos WHERE ProM_ID = "+ProM_ID
			 
	 var rsPt = AbreTabla(sSQLTr,1,0) 
	
	if(!rsPt.EOF){
	  var Rollo_Cantidad = rsPt.Fields.Item("ProMR_Cantidad").Value
	}
				if(SeriesR == Rollo_Cantidad){
	...
	var sSQL = "SELECT  ISNULL(MAX(ProMR_Rollo),0) AS Rollo FROM Producto_Movimiento_Rollos WHERE ProM_ID = "+ ProM_ID
...
if(Cantidad_Rollo > -1){
			...
			sSQLTr  += " VALUES("+ProM_ID+","+Rollo+","+Cantidad_Rollo+")"
		if(Ejecuta(sSQLTr,0)){
				result = 1
