...
<%
	...
	var sSQLTr  = "SELECT TPro_ID, Pro_Nombre FROM Producto"
		...
		var rsTPro = AbreTabla(sSQLTr,1,0) 
			  Producto = rsTPro.Fields.Item("Pro_Nombre").Value
...
var FunctionRecepcion = {
	GetTabla:function(){
		...
		$.post("/pz/wms/Insumos/Insumos_Escaneo_RFID_Ajax.asp",
		{ProM_ID:<%=ProM_ID%>,
		Tarea:1
		}
		,function(data){
	...
	InsertSerie:function(event){
		...
		if(keyNum== 13 ){
			...
			$.ajax({
				method: "POST",
				url: "/pz/wms/Insumos/Insumos_Escaneo_RFID_Ajax.asp?",
				data: {
					ProM_ID:<%=ProM_ID%>,
					Tarea:2,
					Pro_ID:<%=Pro_ID%>,
