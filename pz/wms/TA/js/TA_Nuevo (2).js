...
var TipoDeCargas = {
	CargaIndividual:function(){
		$.post("/pz/wms/TA/TA_Ajax.asp",{Tarea:14},
		 function(response){
			if(response != ""){
				$('#TiposCarga').html(response)
			}
		});
	},
	CargaSerie:function(request){
		$.post("/pz/wms/TA/TA_Ajax.asp",request,
		 function(response){
		});
	},
	LayoutMasivo:function(){
		var date = new Date()
		...
		XLSX.writeFile(wb, "Layout transferencias "+Fecha+".xlsx");
	},
	findDup:function(arr) { 
	 ...
	 TA_ArchivoID_Masivo:function(Datos){
		$.post("/pz/wms/TA/TA_Ajax.asp",{Tarea:16,TA_ID:$('#TA_ID').val(),IDUsuario:$("#IDUsuario").val()},
		 function(response){
			var res = JSON.parse(response)
			if(res.result > -1){ 
				$("#ID_Masivo").val(res.result)
				
				TipoDeCargas.SubeSereies(res.result,Datos);
			}else{
				Avisa("error","Aviso",res.message)
				
			}
		});
	 },
	 SubeSereies:function(series){
		 $('#divGeneralResumen').hide('slow')
		...
		$.ajax({
			...
			url: "https://wms.lydeapi.com/wms/TransferenciaMasivo/Carga",
			success: function(response){
				$('#divLoading').hide('slow')
				$('#divGeneralResumen').show('slow')
				$("#divResumen").show('slow');
				//$('#divVerifica').show('slow');
				 $('#divArchivo').show('slow')
				 if(response.data.Resultado == 1){
					$('#ID_Masivo').val(response.data.Mas_ID);
					$('.btnTerminar').prop('disabled',false);
					
				 }else{
					$('.btnTerminar').prop('disabled',true);
				 }
				$('#tableResumenSolicitado').html(TipoDeCargas.GeneraTabla(response.data.Resumen_Solicitado))
				$('#tableResumenArchivo').html(TipoDeCargas.GeneraTabla(response.data.Resumen_Archivo))
				console.log(response) 
			}
		});
	 },
	 VerificaInfo:function(){
		$.ajax({
			async: false,
			type: 'GET',
			crossDomain: true,
			contentType:'application/json',
			url: "https://wms.lydeapi.com/wms/TransferenciaMasivo/Confirma?ID_Masivo="+$('#ID_Masivo').val(),
			success: function(response){
				console.log(response)
	 ...
	 GeneraTabla:function(arr){
		 ...
		 for(var i = 0;i<arr.length;i++){
			 var Solicitado = arr[i].Solicitado
			 var EnArchivo = arr[i].EnArchivo
			 if(Solicitado == EnArchivo){
				 clase = 'navy-bg'
				 Analisis = 'Correcto' 
			 }else{
				 clase = 'bg-warning'
				 Analisis = 'Verifica archivo'
			 }
			 if(Solicitado == -1){
				 Solicitado = "NO SOLICITADO"
				 clase = 'bg-danger'
				 Analisis = 'SKU no requerido'
			 }
			 Table = Table + '<tr class="'+clase+' text-left"><td>'+arr[i].SKU+'</td><td>'+Analisis+'</td><td>'+Solicitado+'</td><td>'+EnArchivo+'</td></tr>'
		 }
		 return Table
	 }
}
...
$("#fileUploader").change(function(evt){
	//$('#bodyTransferencias').html('<tr id="CargandoDatos"><td align="center" colspan="8"><img src="/Img/ajaxLoader.gif"/></td></tr>')
		...
		reader.onload = function(event) {
			...
			var Series = []
			var Enviar = []
			for(var j = 0;j<json_object1.length;j++){
				Series.push(json_object1[j].Serie)
				Enviar.push({SKU:""+json_object1[j].SKU,Serie:""+json_object1[j].Serie})
			}
			var result_Series = TipoDeCargas.findDup(Series)
			console.log("Series duplicadas "+result_Series.length)
			
			if(result_Series.length > 0){
				$(this).val("")
				swal({
				  title: 'Hay series repetidas('+result_Series.length+') en el archivo',
				  text: "Hemos detectado series repetidas en el archivo, por favor vulevelo a intentar",
				  type: "warning",
				...
				console.log(Enviar)
				TipoDeCargas.SubeSereies(Enviar)
			}
	};
...
var Transferencia = {
	...
	, Guardar: function(){
		...
		var strFacCli = $("#inpFacturaCliente").val();
