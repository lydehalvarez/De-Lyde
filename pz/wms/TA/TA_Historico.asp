    ...
    <div class="animated fadeInRight">
        <div class="ibox">
            <div class="ibox-content">
                <h3>Cajas</h3>
                <p class="small"><i class="fa fa-cube"></i> Cajas que componen la entrega</p>
...
<%
               ...
               var sSQLCaja = "SELECT TAC_ID,TAC_Codigo "
			   				+ ",CONVERT(NVARCHAR(10),TAC_FechaRayosX,103) +' - ' + CONVERT(NVARCHAR(10),TAC_FechaRayosX,108)+' hrs' as Fecha "
                            + ",(SELECT Nombre FROM dbo.tuf_Usuario_Informacion(TAC_UsuarioRx)) as UsuRx "
						    + " FROM TransferenciaAlmacen_Caja "
                            + " WHERE TA_ID = " + TA_ID
                            + " ORDER BY TAC_ID ASC " 
              
                 var rsBox = AbreTabla(sSQLCaja,1,0)
				 if(!rsBox.EOF){
					 while (!rsBox.EOF){
						 TAC_ID = rsBox.Fields.Item("TAC_ID").Value
						 TAC_Codigo = rsBox.Fields.Item("TAC_Codigo").Value
						 UsuRx = rsBox.Fields.Item("UsuRx").Value
						 TAC_FechaRayosX = rsBox.Fields.Item("Fecha").Value
%>    
    
                        <li class="info-element">
                            <strong>Caja No: <%=TAC_ID%></strong>
                            <br> <strong>Etiqueta: </strong> <span class="textCopy"><%=TAC_Codigo%></span>
                            <br> <strong>Usuario Rx: </strong> <%=UsuRx%>
                            <div class="agile-detail">
                                <!-- a href="#" class="pull-right btn btn-xs btn-white">Mark</a 
                                     aqui pondremos la liga para que levante el modal con la imagen de rx    -->
                                <i class="fa fa-clock-o"></i> <%=TAC_FechaRayosX%>
                            </div>
                        </li>    
<%
						rsBox.MoveNext() 
                    ...
                    var sCondicion = "TA_ID = " + TA_ID
                    var sCampo = "ISNULL((CONVERT(NVARCHAR, TA_FechaRayosX,103) + ' ' + CONVERT(NVARCHAR, TA_FechaRayosX,108)),'')" 
                    var RayosX =  BuscaSoloUnDato(sCampo,"TransferenciaAlmacen",sCondicion,1,0)
                    var TCajas =  BuscaSoloUnDato("TA_CantidadCaja","TransferenciaAlmacen",sCondicion,1,0)
...
%>
                    <li class="info-element">
                       ...
                       <div class="agile-detail">
                            <!-- a href="#" class="pull-right btn btn-xs btn-white">Mark</a 
                                 aqui pondremos la liga para que levante el modal con la imagen de rx    -->
                            <i class="fa fa-clock-o"></i> <strong>FechaRx: </strong>  <%=RayosX%>
                        </div>
                    </li>    
<%				}
				rsBox.Close()   
...
<div class="ibox float-e-margins">
   <div class="ibox-title">
       <h3>Trazabilidad</h3>
            <p class="small"><i class="fa fa-clock-o"></i> Hist&oacute;rico de la transferencia</p>
          ...
          <table class="table table-hover no-margins">
             ...
             <tbody>
                <%
                   ...
                   var sSQLHist = "SELECT dbo.fn_CatGral_DameDato(51,TA_EstatusCG51) AS ESTATUS, TA_EstatusCG51,TAH_ID "
                                ...
                                + " WHERE TA_ID = " + TA_ID
                                + " ORDER BY TA_Reintento ,TA_FechaRegistro ASC " 
...
<%              
    ...
    var Vuelta = 0
    var estiloMan = ""
    var Man_EstatusCG48 = 0
    var TipoManifiesto = ""
    var UsuarioQuito = ""
    var UsuarioValida = ""
    var Llaves = "0,0"
               
    var sSQLMan  = "SELECT ISNULL((CONVERT(NVARCHAR, Man_FechaRegistro,103) + ' ' + CONVERT(NVARCHAR, Man_FechaRegistro,108)),'') as Fecha"
                 + ", Man_EstatusCG48, Man_ID, ManD_ID "
	             ...
	             + "    END as Estado "
	             + "  , CASE WHEN ISNULL(Man_UsuarioQuita,0) > 0 THEN dbo.fn_Usuario_DameNombreUsuario(Man_UsuarioQuita)  "
	             + "         ELSE '' "
	             + "    END as UsuarioQuito "
	             + "  , CASE WHEN ISNULL(Man_UsuarioQuita,0) > 0 THEN ISNULL((CONVERT(NVARCHAR, Man_UsuarioQuitaFecha,103) + ' ' + CONVERT(NVARCHAR, Man_UsuarioQuitaFecha,108)),'') "
	             + "         ELSE '' "
	             + "  END as FechaUsuarioQuito "
	             + "  , dbo.fn_Usuario_DameNombreUsuario(Man_Usuario) UsuarioOrigino "
	             + "  , CASE WHEN ISNULL(Man_UsuarioValida,0) > 0 THEN dbo.fn_Usuario_DameNombreUsuario(Man_UsuarioValida) ELSE ''  "
	             + "  END as UsuarioValida  "
	             + "  , CASE WHEN ISNULL(Man_UsuarioValida,0) > 0 THEN ISNULL((CONVERT(NVARCHAR, Man_FechaValidacion,103) + ' ' "
                                                                          + "+ CONVERT(NVARCHAR, Man_FechaValidacion,108)),'') "
	             +        "  ELSE ''  "
            ...
            while(!rsMan.EOF){    
               TipoManifiesto = rsMan.Fields.Item("TipoManifiesto").Value  
               Man_EstatusCG48 = rsMan.Fields.Item("Man_EstatusCG48").Value
               UsuarioValida = rsMan.Fields.Item("UsuarioValida").Value
               Llaves = rsMan.Fields.Item("Man_ID").Value 
                      + "," + rsMan.Fields.Item("ManD_ID").Value
                
               //1	Registrado
               estiloMan = "success-element"
               //2	Quitado
               if (Man_EstatusCG48 == 2 ){
                    estiloMan = "danger-element"
               }
               //3	Reingresado
               if (Man_EstatusCG48 == 3 ){
                    estiloMan = "info-element"
               }
               if(TipoManifiesto != "Salida"){
                   estiloMan = "warning-element" 
               }

               UsuarioQuito = FiltraVacios(rsMan.Fields.Item("UsuarioQuito").Value)
               if(Vuelta == 0) {
 %>	
<div class="ibox float-e-margins">
    <div class="animated fadeInRight"> 
        <div class="ibox">
            <div class="ibox-content">
                <h3>Manifiestos</h3>
                <p class="small"><i class="fa fa-truck"></i> Remisi&oacute;n de embarques</p>

                <ul class="sortable-list connectList agile-list" > 
<%	           }    %>

                        <li class="<%=estiloMan%>">
                            <strong>Folio: </strong> <a onClick="javascript:Manifiesto.AbrirModal(<%=Llaves %>)" ><%=rsMan.Fields.Item("Folio").Value %></a>
                            <br><strong>Tipo: </strong> <%=TipoManifiesto %> 
                            <br><strong>Estatus: </strong> <%=rsMan.Fields.Item("Estatus").Value%>
                            <br><strong>Origino: </strong> <%=rsMan.Fields.Item("UsuarioOrigino").Value%>
<% if (UsuarioValida != ""){ %>
                            <br><strong>Usuario valid&oacute;: </strong> <%=UsuarioValida%></td> 
                            <br><strong>Fecha: </strong> <%=rsMan.Fields.Item("FechaValidacion").Value %>        
<% } %>
                            <br><strong>Transportista: </strong> <%=rsMan.Fields.Item("Transportista").Value%> 
                            <br><strong>Estado: </strong> <%=rsMan.Fields.Item("Estado").Value%></td> 
                            <br><strong>Fecha: </strong> <%=rsMan.Fields.Item("Fecha").Value %>
<% if (UsuarioQuito != ""){ %>
                            <br><strong>Usuario quit&oacute;: </strong> <%=UsuarioQuito%></td> 
                            <br><strong>Fecha: </strong> <%=rsMan.Fields.Item("FechaUsuarioQuito").Value %>        
<% } %>
