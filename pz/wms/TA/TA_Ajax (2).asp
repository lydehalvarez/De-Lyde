...
<%			
	...
	case 16: {
		...
		var sqlExpTra = "SELECT TA_Folio,Pro_SKU,Pro_Nombre,Tas_Serie  " +
						" FROM TransferenciaAlmacen a,TransferenciaAlmacen_Articulo_Picking b, Producto c "+
...
//					+ ', "CantidadPicking": "' + rsExpTra("CantidadPicking").Value + '" '
	...
	case 18: //cambio de hora a un estatus de una transferencia
        ...
        var TA_EstatusCG51 = Parametro("TA_EstatusCG51", 0)
		var Respuesta = ""
        var bPasa = true

        
        if (TA_EstatusCG51 == 0){
            Respuesta = '{"result":-1,"message":"No se recibi&oacute; el estatus"}'
            bPasa = false
        }
        ...
        if(bPasa) {    
            ...
            var ActualizaFecha = "UPDATE TransferenciaAlmacen_Historico "
                               ...
                               + " WHERE TA_ID = " + TA_ID 
                               + " AND TA_EstatusCG51 = " + TA_EstatusCG51
	...
	case 19: {
		var TA_ID = Parametro("TA_ID", -1)
		var TA_EstatusCG51 = Parametro("TA_EstatusCG51", -1)
		var IDUsuario = Parametro("IDUsuario", -1)
		
		var GeneraEvento = "UPDATE TransferenciaAlmacen "
			GeneraEvento += " SET TA_EstatusCG51 =  "+TA_EstatusCG51
			GeneraEvento += " ,TA_UltimoUsuarioModifico = "+IDUsuario
			GeneraEvento += " WHERE TA_ID = "+TA_ID
			
		if(TA_EstatusCG51 > -1){	 
			if(Ejecuta(GeneraEvento,0)){
				result = 1;
				message= "Se hizo el cambio correctamente"
			}else{
				result = -1;
				message= "Error al ejcutar"
			}
		}else{
			result = -1;
			message= "Error en el estatus, no se escogi&oacute; alg&uacute;n estatus"
		}
		
		var Titulo = ""
		var Descripcion = ""
		
		if(TA_EstatusCG51 == 18){
			Titulo = "Siniestro"
			Descripcion = "Ocurrió un siniestro"
			var Incidencia = "INSERT INTO Incidencia(Ins_Titulo,Ins_Descripcion,TA_ID,Ins_Usu_Reporta) "
				Incidencia += " VALUES('"+Titulo+"','"+Descripcion+"',"+TA_ID+","+IDUsuario+")"
				
			Ejecuta(Incidencia,0)
		}
