var AuditoriaSeleccionLPN = {
	SyncDatos:function(){
		Avisa("success","Generando sincronización","Espere un momento por favor");
		$.ajax({
		url: "https://wms.lydeapi.com/api/sandbox/v1/Auditoria/Summary?Aud_ID="+$("#Aud_ID").val()+"&AudC_ID="+$("#AudC_ID").val()
		, method: "GET"
		, async: true
		...
		console.log(res)
			if(res.result == 1){
				...
				$("#tab-#tab-4").load("/pz/wms/Auditoria/Auditoria_Existencias.asp", dato);
				...
				$('.SyncDCats').removeClass('disabled');
			}
			
			}
			, error: function(){
				Avisa("error","Ocurri&oacute; un error en la conexi&oacute;n", "Intente de nuevo por favor");
				$('.SyncDCats').removeClass('disabled')
			}
	...
	SyncDcats:function(){
		$('.SyncDCats').addClass('disabled')
		var AudC_ID = $("#AudC_ID").val()
		swal({
		  title: "&iquest;Desea sincronizar el conteo: "+AudC_ID+" con DCats?",
		  text: '<strong>Al oprimir "ok" ya no se podr&aacute; agregar informaci&oacute;n ya que no puede ser editado &iquest;Desea sincronizar el conteo '+AudC_ID+'?</strong>',
		  type: "warning",
		...
		function(data){
			if(data){
				$.ajax({
					  url: "https://wms.lydeapi.com/api/sandbox/v1/Auditoria/Buscaid?Aud_ID="+$("#Aud_ID").val()+"&AudC_ID="+AudC_ID
					, method: "GET"
					, async: true
					, success: function(res){
						console.log(res)
						if(res.result == 1){
							Avisa("success","Auditoria de DCats encontrada", res.message);
							$('.SyncDCats').hide('slow')
							AuditoriaSeleccionLPN.SyncDatos();
							
							
							
						}else{
							Avisa("error","Ocurri&oacute; un error", "Intente de nuevo por favor");
							$('.SyncDCats').removeClass('disabled')
						}
	...
	, CerrarVisitaVigente: function(myjson){
		
		swal({
		  title: "Desea terminar el conteo numero: "+myjson.Conteo,
		  text: '<strong>Al oprimir "ok" ya no se podr&aacute; agregar informaci&oacute;n ya que no puede ser editado &iquest;Desea terminar el conteo '+myjson.Conteo+'?</strong>',
		  type: "warning",
		  showCancelButton: true,
		  confirmButtonClass: "btn-success",
		  confirmButtonText: "Ok" ,
		...
		function(data){
			if(data){
					...
					var intAud_ID = myjson.Aud_ID;
					var intVisita = myjson.Visita;
						...
						$.ajax({
							...
							, success: function( res ){
								...
								if( res.Error.Numero == 0 ){
									...
									Avisa("error", "Ocurri&oacute; un error", res.Error.Descripcion );
								}
