...
<%
			sSQL = "SELECT p.Pro_SKU AS SKU, i.TA_ID, i.Pro_ID, i.Inv_ID_Cambio, p.Pro_Nombre AS Producto, pc.Pro_SKU AS SKUC, pc.Pro_Nombre AS ProductoC, iv.Inv_Serie as Serie1,  iv2.Inv_Serie as Serie2 FROM Incidencia_SKU i"
					  ...
					  + " LEFT JOIN Inventario iv2 ON i.Inv_ID_Cambio=iv2.Inv_ID"
					  + " WHERE  i.Ins_ID="+Ins_ID
					  + " GROUP BY p.Pro_SKU,  i.TA_ID, i.Pro_ID,  p.Pro_Nombre,  pc.Pro_SKU, pc.Pro_Nombre, i.Inv_ID_Cambio,iv.Inv_Serie,  iv2.Inv_Serie "
		        var rsTransferencia = AbreTabla(sSQL,1,0)
		if (!rsTransferencia.EOF){
%>
<table class="table table-hover">
  	  <tbody>
	<th>  <small> SKU correcto </small></th>
<!--	<th>  <small> Modelo correcto </small></th>
-->	<th>   <small> Serie correcta </small></th>
	<th>  <small> SKU enviado </small></th>
<!--	<th>  <small> Modelo enviado </small></th>
-->	<th>  <small> Serie enviada </small></th>
	<%
    ...
    <!--    <td class="project-title">
		...
		<td class="project-title">
             <small>  <%=rsTransferencia.Fields.Item("Serie1").Value%>  </small> 
			  ...
			  <td class="project-title">
        <small> <%=rsTransferencia.Fields.Item("SKUC").Value%> </small> 
        </td>
       <!-- <td class="project-title">
            <small><%=rsTransferencia.Fields.Item("ProductoC").Value%>  </small> 
        </td>-->
		<td class="project-title">
             <small> <%=rsTransferencia.Fields.Item("Serie2").Value%>   </small> 
        </td>
				<td>
         	<input class="form-control Serie<%=rsTransferencia.Fields.Item("Inv_ID_Cambio").Value%>"  id= "Serie" style="width:100%" placeholder="escanea la serie" type="text" autocomplete="off" value="" <%/*%>maxlength="<%=Digitos%>"<%*/%> onkeydown= "FunctionActualiza.ActualizaSerie(<%=rsTransferencia.Fields.Item("TA_ID").Value%>, <%=rsTransferencia.Fields.Item("Pro_ID").Value%>, <%=rsTransferencia.Fields.Item("Inv_ID_Cambio").Value%>, event);"/>
