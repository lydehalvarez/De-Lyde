...
var sSQL = "SELECT *,  dbo.fn_Usuario_DameCorreo( Ins_Usu_Reporta ) as REPORTA "
  ...
  var sSQL  = " SELECT *,  dbo.fn_CatGral_DameDato(51,t.TA_EstatusCG51) Cat_Nombre,"
  				   + " e.TA_FolioRemision,  CONVERT(VARCHAR(17),h.TA_FechaRegistro, 113) AS F_Shipping"
       sSQL += " FROM TransferenciaAlmacen t, TransferenciaAlmacen_Historico h, cliente c, TransferenciaAlmacen_FoliosEKT e, Almacen a "
       sSQL += " WHERE t.Cli_ID=c.Cli_ID AND t.TA_End_Warehouse_ID = a.Alm_ID  AND e.TA_ID=t.TA_ID AND h.TA_ID=t.TA_ID"   
 	   sSQL += " AND t.TA_ID = " + TA_ID+ " AND h.TA_EstatusCG51=4"
        var rsTransferencia = AbreTabla(sSQL,1,0)
       	var F_Shipping = rsTransferencia.Fields.Item("F_Shipping").Value
