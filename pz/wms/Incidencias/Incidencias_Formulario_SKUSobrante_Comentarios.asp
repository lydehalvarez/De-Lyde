...
<%
		sSQL = "SELECT p.Pro_SKU AS SKU,   i.TA_ID, i.Pro_ID, i.Inv_ID, p.Pro_Nombre AS Producto, i.Inv_ID, iv.Inv_Serie as Serie1 FROM Incidencia_SKU i"
					  + " LEFT JOIN Producto p ON p.Pro_ID=i.Pro_ID"
					  + " LEFT JOIN Inventario iv ON iv.Inv_ID=i.Inv_ID"
					  + " WHERE  i.Ins_ID="+Ins_ID
					  + " GROUP BY p.Pro_SKU, p.Pro_Nombre, i.Inv_ID, iv.Inv_Serie ,  i.TA_ID, i.Pro_ID, i.Inv_ID"
		        var rsTransferencia = AbreTabla(sSQL,1,0)
		if (!rsTransferencia.EOF){
%>
<br/>
<strong>SKU sobrante: </strong>
<br/>
<table class="table table-hover">
  	  <tbody>
...
//			 var rsSKU = AbreTabla(sSQL,1,0)
	...
	%>
	   <tr>
         <td class="project-title">
            <a href="#"><%=rsTransferencia.Fields.Item("SKU").Value%></a>
        </td>
        <td class="project-title">
            <a href="#"><%=rsTransferencia.Fields.Item("Producto").Value%></a>
        </td>
			 <td class="project-title">
            <a href="#"><%=rsTransferencia.Fields.Item("Serie1").Value%></a>
        </td>
		<%
			if(IDUsuario == 36){
		%>
			<td>
				<input class="form-control Serie<%=rsTransferencia.Fields.Item("Inv_ID").Value%>"  id= "Serie" style="width:100%" placeholder="escanea la serie" type="text" autocomplete="off" value="" <%/*%>maxlength="<%=Digitos%>"<%*/%> onkeydown= "FunctionActualiza.ActualizaSerie(<%=rsTransferencia.Fields.Item("TA_ID").Value%>, <%=rsTransferencia.Fields.Item("Pro_ID").Value%>, <%=rsTransferencia.Fields.Item("Inv_ID").Value%>, event);"/>
				</td>
		<%
			}
		%>
