	...
	sSQL = "SELECT InsT_ID, InsT_Nombre FROM Incidencia_Tipo WHERE InsT_ID = " + InsT_Padre
            ...
            <input type="hidden" class="form-control col-md-3 InsT_IDPadre" value="<%=InsT_Padre%>"></input>
               ...
               <div class="form-group">
                    ...
                    <div class="col-md-4">
            		  <span class="text-left">
                   	<label class="col-md-9 Procede2" id="Procede2"><%=Proviene%></label>
				 	   </span>
                    </div>
				...
				if(!rsInsTipo.EOF){
					%>
                         <label class="control-label col-md-2"><strong>Subtipo</strong></label>
			             <div class="col-md-3">
				<%
                var sCondicion = "InsT_Padre = "+ InsT_Padre
                var campo = "InsT_Nombre"
                
                CargaCombo("InsT_IDHijos2","class='form-control'","InsT_ID",campo,"Incidencia_TIpo",sCondicion,"","Editar",0,"Selecciona")%>
             </div>
											...
											if(ventana==2544){
												...
												if(ventana==331){
								 				params += "?CliOC_ID=" +   $('#CliOC_ID').val()
													...
													if(Origen==6||Origen==7){
													params += "?Ins_Proveedor=1"
													}
													if(Ins_ID > -1){
													params += "?Ins_ID="+Ins_ID
													}
...
//		$("#divHijos").load("/pz/wms/Incidencias/Incidencias_Formulario.asp?" + sDatos)
			 ...
			 $('#InsT_IDHijos2').change(function(e) {
             e.preventDefault()
			 console.log($(this).val())
			 var procede = "<%=Procedencia%>"
			if(procede != "1"){
