...
<%
	...
	Procedencia = Procedencia.replace("-",",")
				...
				if(Ins_ID >-1){
					sSQL = "SELECT i.*, a.Alm_Numero FROM Incidencia i INNER JOIN Almacen a ON i.Alm_ID = a.Alm_ID WHERE Ins_ID = " + Ins_ID	
					...
					var Alm_Numero = rsIncidencias.Fields.Item("Alm_Numero").Value
...
%>
                ...
                <%
                             sSQL = "SELECT Prov_Nombre, Prov_ID FROM Proveedor WHERE Prov_EsPaqueteria = 1"
	...
	%>
	 ...
	 $('.btnCerrar').click(function(e) {
 	   ...
 	   data: { 
						   ...
						   Ins_Descripcion:encodeURI($('.Descripcion').val()),
