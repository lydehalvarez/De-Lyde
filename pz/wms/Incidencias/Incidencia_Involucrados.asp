...
<%
	...
	var Ins_ID_Asignados = Parametro("Ins_ID_Asignados",-1)
...
<div class="modal-header" >
                	...
                	<div class="form-group" id="divUsuarios">
                            ...
                            <div class="form-group" id="divGrupos">
                                    ...
                                    <%
										var sCondicion = ""
										CargaCombo("Gru_ID","class='form-control'","IncG_ID","IncG_Nombre","Incidencia_Grupo",sCondicion,"","Editar",0,"--Seleccionar--")
									%>
                         </div>
                         </div>
                          <div class="form-group">
	    	...
	    	sSQL = "SELECT   *, dbo.fn_Usuario_DameNombreUsuario(Ins_UsuarioID) as usuario "
					...
					+" LEFT JOIN Incidencia_Grupo g ON i.Ins_GrupoID=g.IncG_ID"
...
//				}
      ...
      <tr id="tr_<%=InsI_ID%>">
         ...
         <%
			 if(rsInvol.Fields.Item("Ins_GrupoID").Value>-1){
				...
				SegGrupo = 1
  		  %>
		 	<%=rsInvol.Fields.Item("IncG_Nombre").Value%>
		<%
             }else{
				 IDUsuario =1
				SegGrupo = -1
  ...
  <%
    ...
    %>
                 	     ...
                 	     <input type="hidden" id="tarea" value="<%=Ins_ID_Asignados%>"></input>
		...
		function(data){
       	var response = JSON.parse(data)
				...
				var dato = "InsO_ID="+<%=InsO_ID%>
					   dato += "&Ins_ID="+<%=Ins_ID%>
					   dato += "&Ins_ID_Asignados="+ $('#tarea').val()
					var tarea = $('#tarea').val()
					if(tarea >-1){
			       $("#divAsignados").load("/pz/wms/Incidencias/Incidencia_Involucrados.asp", dato)
					   }else{
			       $("#divInvolucrados").load("/pz/wms/Incidencias/Incidencia_Involucrados.asp", dato)
					   }
	    });
		}
