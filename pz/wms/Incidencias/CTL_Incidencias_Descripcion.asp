...
var sSQL = "SELECT *,   dbo.fn_Usuario_DameNombreUsuario( Ins_Usu_Reporta ) as REPORTA "
                ...
                +  " , dbo.fn_Usuario_DameNombreUsuario( Ins_Usu_Escalado ) as ESCALADO "
				...
				+  " FROM Incidencia i INNER JOIN Incidencia_Tipo t ON i.InsT_ID=t.InsT_ID WHERE i.Ins_ID = " + Ins_ID 
					...
					var Escalado = rsIncidencias.Fields.Item("Ins_Usu_Escalado").Value
					if(Escalado >-1){
					Atendio = rsIncidencias.Fields.Item("ESCALADO").Value
					}
		...
		var Formulario=rsFormulario("For_Archivo").Value
					...
					var sSQL= " SELECT * FROM Incidencia_Involucrados  WHERE Ins_ID= "+Ins_ID+" AND (Ins_GrupoID=" + SegGrupo
...
if(InsT_ID ==19){
	%>
                <div class="mail-body col-md-12" id="divFormulario" >
                </div>
<%
}else{
%>
...
%><%*/%>
                  ...
                  <div  class="col-md-3">
                    ...
                    <h4><strong>Estatus:</strong></h4> 
							...
							%>
        		 <button type="button" class="btn btn-xs btn-primary Conclusion" onclick="ModalTorreControl()">Conclusion</button>   
					...
					if(Permiso>2){
					%>
					 <button type="button" class="btn btn-xs btn-primary Asignar" onclick="ModalAsigna()">Asignar</button>   
					 <%
                        }
                   	if(Permiso==4){
					%>
   					 <button type="button" class="btn btn-xs btn-primary Involucrados" onclick="Involucrados()">Involucrados</button>   
    				<%
					}
					%>
						<br/>
...
%>
                         ...
                         <input type="hidden" value="<%=Ins_ID%>" class="agenda" id="Ins_IDT"/>
...
<script type="application/javascript">
   ...
   $(document).ready(function(){
	...
	$("#divComentarios").load("/pz/wms/Incidencias/CTL_Incidencias_Comentarios.asp?Ins_ID="+ <%=Ins_ID%>+"&Reporta="+<%=Reporta%>+"&Recibe="+<%=Recibe%>+"&InsO_ID="+<%=InsO_ID%>+"&Permiso="+ <%=Permiso%>)
    ...
    $.ajax({
		...
		function ModalTorreControl(){
              	$("#mdlTorreControl").modal('hide').remove();

            $.ajax({
                  url: "/pz/wms/Incidencias/Incidencias_Modal_TorreControl.asp"
                , method: "post"
                , async: false
                , data: {
                     InsO_ID: <%=InsO_ID%>,
			...
			$("#mdlTorreControl").modal('show');
    }
	function Cerrar(){
		$('#divInvolucrados').empty()
		$('#divInvolucrados').hide('slow')
		$("#divAsignados").empty()
		$('#divAsignados').hide('slow')
		$('#T_Involucrado').val('')
		$('#Involucrado').val('')
	}
		function ModalAsigna(){
            ...
            $.ajax({
                  url: "/pz/wms/Incidencias/Incidencias_Modal_Asigna.asp"
                , method: "post"
                , async: false
                , data: {
                     InsO_ID: <%=InsO_ID%>,
            ...
            $("#divInvolucrados").load("/pz/wms/Incidencias/Incidencia_Involucrados.asp"
                               , dato
                               , function(){
            });  
            }

	var FunctionActualiza = {
			ActualizaSerie:function(taid, proid, invid, event){
    ...
    success: function(data){
		...
		if(resp==1){
			var Tipo = "success"
			var sMensaje = "La serie se ha ingresado correctamente "
			Avisa(Tipo,"Aviso",sMensaje);
  	    }
		if (resp==0){
			var Tipo = "error"
			var sMensaje = "La serie no fue ingresada! Comunicarse a sistemas. "
	
				Avisa(Tipo,"Aviso",sMensaje);
		}
		if(resp==2){
			var Tipo = "success"
			var sMensaje = "La serie se ha ingresado correctamente al inventario y a la transferencia."
		...
		if (resp==3){
			var Tipo = "error"
			var sMensaje = "La serie no se encuentra en inventario! Comunicarse a sistemas. "
	
				Avisa(Tipo,"Aviso",sMensaje);
		}
		if (resp==0){
			var Tipo = "error"
			var sMensaje = "La serie no existe en la transferencia. "
