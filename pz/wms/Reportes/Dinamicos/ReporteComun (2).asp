...
if (!rsReporte.EOF) {
    ...
    if (rsReporte.Fields.Item("Rep_PorParametro").Value == 0 ) {
    ...
    //en lugar de hacer un arreglo de parametors va a sustituir los parametros que vienen en la funcion
    if (rsReporte.Fields.Item("Rep_PorParametro").Value == 1 ) {
        
        
         ParametroCambiaValor("RepCli_ID",Parametro("RepCli_ID","NULL"))
         ParametroCambiaValor("RepProv_ID",Parametro("RepProv_ID","NULL")) 
        
         var sFecha = Parametro("txtFechaDesde","NULL")
         if(sFecha == "NULL"){
             ParametroCambiaValor("FechaDesde","NULL")
         } else {
             sFecha = CambiaFormatoFecha(sFecha,"dd/mm/yyyy",FORMATOFECHASERVIDOR)
             ParametroCambiaValor("FechaDesde",sFecha)
         }
         sFecha = Parametro("txtFechaHasta","NULL")
         if(sFecha == "NULL"){
             ParametroCambiaValor("FechaHasta","NULL")
         } else {
             sFecha = CambiaFormatoFecha(sFecha,"dd/mm/yyyy",FORMATOFECHASERVIDOR)
             ParametroCambiaValor("FechaHasta",sFecha)
         }

         ParametroCambiaValor("Rep_CorteTras",Parametro("Rep_CorteTras","NULL"))
         ParametroCambiaValor("Rep_CorteOV",Parametro("Rep_CorteOV","NULL"))  
        
         sQuerySQLFROM = ProcesaBuscadorDeParametros(sQuerySQLFROM) 
         sQuerySQLFROM = sQuerySQLFROM.replace(/'NULL'/g, "NULL");
        
        
        //lista de parametros disponibles
        //Rep_CorteTras   id del corte de la transferencia
        //Rep_CorteOV     id del corte de la orden de venta
        //RepCli_ID       id del cliente
        //RepDeu_ID       id del deudor ( factoraje )
        //RepProv_ID      id del proveedor
    ...
    if (rsReporte.Fields.Item("Rep_PorParametro").Value == 0 ) {
	//Clientes Rep_CorteTras
	
		if (rsReporte.Fields.Item("Rep_Cliente").Value == 1) { 
			if (Parametro("Rep_CorteTras",-1) > -1 ) {
				sCondicionExtra = " TA_ArchivoID = " 
				sCondicionExtra += Parametro("Rep_CorteTras",-1)
			}
		}
    
	//Clientes Rep_CorteOV
	
		if (rsReporte.Fields.Item("Rep_CorteOV").Value == 1) { 
			if (Parametro("Rep_CorteOV",-1) > -1 ) {
				sCondicionExtra = " Cort_ID = " 
				sCondicionExtra += Parametro("Rep_CorteOV",-1)
			}
		}
	...
	//Deudor Rep_Proveedor
	
		if (rsReporte.Fields.Item("Rep_Proveedor").Value == 1) { 
			if (Parametro("RepProv_ID",-1) > -1 ) {
				sCondicionExtra = " " +  rsReporte.Fields.Item("Rep_CampoProveedor").Value + " = " 
				sCondicionExtra += Parametro("RepProv_ID",-1)
			}
		}
...
<% if(Parametro("iExport",0) == 0){ %>
    ...
    <link href="/js/TableExport/dist/css/tableexport.min.css" rel="stylesheet" type="text/css">
...
Response.Write("<table class='table table-striped table-bordered table-hover' id='TbResultado'><thead><tr>")
...
//Armado final de la agrupación y el orden básico
    ...
    sConsultaSQL = ProcesaBuscadorDeParametros(sConsultaSQL)

	if(bIQon4Web) {
        Response.Write("<br>linea 582")
		Response.Write("<br>iArticulos" + iArticulos)
...
<script src="/js/TableExport/dist/Blob.min.js"></script>
<script src="/js/TableExport/dist/xls.core.min.js"></script>                 
<script src="/js/TableExport/dist/js/tableexport.min.js"></script> 
<script type="text/javascript">
    
    $(document).ready(function() {
        $("#TbResultado").tableExport({
            formats: ["xlsx","txt", "csv"], //Tipo de archivos a exportar ("xlsx","txt", "csv", "xls")
            position: 'button',  // Posicion que se muestran los botones puedes ser: (top, bottom)
            bootstrap: true,//Usar lo estilos de css de bootstrap para los botones (true, false)
            fileName: "<%=sNombreReporte%>"     //Nombre del archivo 
        });
    });
