        ...
        <div class="ibox float-e-margins">
                ...
                <div class="row">
                    <div class="col-sm-12">        
                        <div class="col-sm-6 text-left">
                             <a  onclick="$('#Man_ID').val("")" class="btn btn-primary btnNuevoManifiesto">
                                      <i class="fa fa-plus"></i>&nbsp;&nbsp;Nuevo manifiesto</a>  
                             <a class="btn btn-primary btnConfirma" id= "BtnCMnf" style="display:none" >
                                    <i class="fa fa-check"></i> Confirmar manifiesto</a>  
                         </div>
                ...
                <div class="row"> 
                    <div class="col-sm-12 m-b-xs">        
                         ...
                         <div class="col-sm-4 m-b-xs">
                           <input class="form-control busFolio" placeholder="Escribe el folio manifiesto" title="Escanea el folio que deseas encontrar o solo escribe los numeros" type="text" autocomplete="off" value="" />
                         </div>
                    </div>  
                </div>
                <div class="row"> 
            ...
            <div class="row">
				...
				<%
                    ...
                    CargaCombo( "Alm_Ruta",sEventos,"DISTINCT Alm_Ruta","'R '+CAST(Alm_Ruta as nvarchar)","Almacen",sCondicion,"Alm_Ruta","Editar",0
                               ,"--Seleccionar--")
                %>
                    </div>
...
var ManifiestoFunciones = {
		Contenido_Reporte:function(manid){
			...
			Folio_Salida:function(manid){
			var newWin=window.open("http://qawms.lyde.com.mx/pz/wms/Salidas/FolioSalidaImpreso.asp?Man_ID="+manid);
		},
       ...
       Contenido_EliminarSO:function(ovid){
			...
			setTimeout(function(){
				...
				$.ajax({
					...
					error: function(){
							swal({
							  title: "Error!",
							  text: "Comprueba la conexi&oacute;n de red o intentalo de nuevo",
							  type: "error",
							  confirmButtonClass: "btn-success",
							  confirmButtonText: "Ok" ,
							  closeOnConfirm: true,
		...
		EscaneaCaja:function(Caja,input){
			 var dato = {
					 Folio:Caja,
					 Man_ID:$('#Man_ID').val(),
					 IDUsuario:$("#IDUsuario").val(),
				...
				$.ajax({
					...
					data: JSON.stringify(dato), 
					contentType:'application/json',
					url: "https://wms.lydeapi.com/api/Embarques/Manifiesto/Add/Box",
					success: function(response){
						console.log(response)
						if(response.result == 1){
							$('#botonesHidden').css('display','block');
							var Datos = response.data
							if($('#Entrante_'+Datos.TA_ID).length == 1){
								var Caja = Datos.TA_ID+'_Caja_'+Datos.TAC_ID
								$('#'+Caja).removeClass('btn-outline btn-primary').addClass('btn-primary');
							}else{
								var total = parseInt($('#Total').val())
								total = total + 1
								$('#Total').val(total)
								$('#TotalAcumulado').html(total)
								$('#addManifiesto').prepend(ManifiestoFunciones.RenglonCaja(response.data));	
								setTimeout(function(){$('#Entrante_'+response.data.TA_ID).removeClass('bg-primary')},1800);
							}
							Avisa("success","Aviso",response.message);
						}else{
							Avisa("error","Aviso",response.message);
		...
		RenglonCaja:function(arr){
			var data = arr.DataPedido;
			var boxes = ''
			var Total = arr.Escaneadas + arr.Faltan
			var color = ''	

			for(var i = 1;i<=Total;i++){
				if(i != arr.TAC_ID){color = 'btn-outline btn-primary'}else{color = 'btn-primary'}
				boxes = boxes + '<a class="btn '+color+'" id="'+arr.TA_ID+'_Caja_'+i+'">'+i+'</a>'
			}
			
			var Nuevo = '<tr class="bg-primary" id="Entrante_'+arr.TA_ID+'">'+
						'<td class="project-title">'+
						   '<a href="#">'+data.Cli_Nombre+'</a>'+
							'<br/>'+
							'<small>Transportista:'+data.Transportista+'</small>'+
						'</td>'+
						'<td class="project-title">'+
							'<a data-clipboard-target="#copytext'+arr.TA_ID+'" id="copytext'+arr.TA_ID+'"  class="textCopy">'+data.Folio+'</a>'+
							'<br/>'+
							'<small><strong>Registro:</strong>'+data.FechaRegistro+'</small>'+
							'<br/>'+
							'<small><strong>Escaneado por:</strong>'+data.EscaneadoPor+'</small>'+
						'</td>'+
						'<td class="project-title">'+
							'<a href="#">No. Tienda: '+data.Tienda_Numero+'</a>'+
							'<br/>'+
							'<small> Nombre: '+data.Tienda_Nombre+'</small>'+
						'</td>'+
						'<td class="project-title">'+boxes+'</td>'+
						'<td class="project-title">'+
							'<a href="#">C.P. '+data.Alm_CP+'</a>'+
							'<br/>'+
							'<small> Elaboracion: '+arr.FechaElaboracion+'</small>'+
						'</td>'+
						'<td class="project-title">'+
							'<a href="#">'+data.Alm_Estado+'</a>'+
							'<br/>'+data.Alm_Ciudad+
						'</td>'+
						'<td class="project-actions">'+
							'<a class="btn btn-danger btn-sm" href="#" onclick="ManifiestoFunciones.EliminaPedido('+arr.TA_ID+');  return false"><i class="fa fa-trash"></i> Eliminar</a>'+
						'</td>'+
					 '</tr>'
			return Nuevo;
		...
		GPS:function(manid){
		    var Man_ID = manid
                ...
                $('<tr id="tr_gps_'+Man_ID+'"><td colspan="6" id="td_gps_'+Man_ID+'">'+loading+'</td></tr>').insertAfter($("#Man_" +Man_ID));
            var dato = {
                Man_ID:Man_ID
            }
            $("#td_gps_"+Man_ID).load("/pz/wms/Salidas/Manifiesto_GPS.asp"
                               , dato
                               , function(){
                                   // $("#tr_gps_"+Man_ID).show("slow")
            });  
		},

		ValidaFolioAPI:function(dato,input){
			setTimeout(function(){
				input.val("");
			},200)
				$.ajax({
					...
					data: JSON.stringify(dato), 
					contentType:'application/json',
					url: "https://wms.lydeapi.com/api/s2012/Manifiesto/Valida",
					success: function(response){
						console.log(response)
						if(response.result == 1){
							$('#Renglon_'+response.data.TA_ID).addClass('bg-primary');
							$('#Validados').html(response.data.Cantidad)
							$('html, body').animate({ scrollTop: $('#Renglon_'+response.data.TA_ID).offset().top }, 'slow');	
						}else{
							swal({
							  title: "Ups!",
							  text: response.message,
