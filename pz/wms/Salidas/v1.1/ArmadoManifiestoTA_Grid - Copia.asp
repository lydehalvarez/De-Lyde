...
//      sSQL += " , (SELECT count(*) FROM TransferenciaAlmacen_Documentos d WHERE d.TA_ID = t.TA_ID ) as CANTIDAD "
        ...
        sSQL += " WHERE t.Cli_ID = c.Cli_ID AND t.TA_End_Warehouse_ID = a.Alm_ID AND TA_EstatusCG51 = 4 "
...
<%
			   ...
			   if(!rsOV.EOF){
			   	   ...
			   	   if(rsOV.Fields.Item("Man_FolioCliente").Value != ""){ 
				        FolCli = rsOV.Fields.Item("Man_FolioCliente").Value
				   }
				   if(rsOV.Fields.Item("Man_Vehiculo").Value != ""){
				        ManD_Vehiculo = rsOV.Fields.Item("Man_Vehiculo").Value
				   }
				   if(rsOV.Fields.Item("Man_Operador").Value != ""){
				        ManD_Operador = rsOV.Fields.Item("Man_Operador").Value
				   }
				   if(rsOV.Fields.Item("Man_Placas").Value != ""){
				       ManD_Placas = rsOV.Fields.Item("Man_Placas").Value
				   }
...
case 9:
    ...
    var sSQL  = " SELECT  * "
        ...
        sSQL +=    " WHERE d.Man_ID = t.Man_ID AND Man_CargaTransportista = 1) as ESCANEADOS "
        sSQL += ", (SELECT count(*) "
        sSQL +=     " FROM TransferenciaAlmacen d "
