...
//                     + " GROUP BY TA_Transportista2 "
            ...
            var sSQL = "select Prov_ID, Prov_Nombre "
                     + " FROM Proveedor where Prov_Habilitado = 1 and Prov_EsPaqueteria = 1 "
			...
			while (!rsTsp.EOF){
%>
                  <option  value="<%=rsTsp.Fields.Item("Prov_ID").Value%>" ><%=rsTsp.Fields.Item("Prov_Nombre").Value%></option>
<%	
			    rsTsp.MoveNext() 
				}
			rsTsp.Close()   	
...
+="Order by Alm_Ciudad "
  ...
  <div class="modal-dialog modal-lg" role="document">
   ...
   <input type="hidden" value="<%=TA_ID%>" id="TA_ID" />
   <input type="hidden" value="<%=Cli_ID%>" id="Cli_ID" />
             ...
             <div class="form-group">
               <label class="control-label col-md-3"><strong>Tipo del veh&iacute;culo</strong></label>
				...
				<input class="form-control agenda" id="IR_DescripcionVehiculo" 
                       placeholder="Descripci&oacute;n de veh&iacute;culo" type="text" autocomplete="off" value="">
				</div>
      ...
      <div class="modal-footer">
        <button type="button"  class="btn btn-primary" id="BtnAgendar"  data-min= "<%=MinutosPallet%>">Guardar</button>
