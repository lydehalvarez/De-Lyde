	...
	switch (parseInt(Tarea)) {
		case 1:
			...
			if(Man_ID != -1){
					...
					if( Disponible > -1){
						...
						if(HojaRuta != -1){
							...
							var sSQLTr = "UPDATE TransferenciaAlmacen SET Man_ID = " + Man_ID
												+" , Man_Usuario= "+Usu_ID+" , Man_FechaRegistro = getdate() WHERE TA_ID= " + TA_ID		
				...
				var Disponible = BuscaSoloUnDato("OV_ID","Orden_Venta","OV_Cancelada = 0 AND OV_EstatusCG51 =4  AND OV_ID = "+OV_ID,-1,0) 
					if( Disponible > -1){
							if(Existe == -1){
							//	var Man_ID = SiguienteID("Man_ID","Manifiesto_Salida","",0)
								//var Man_ID = Man_ID -1
							var sSQLTr = "UPDATE Orden_Venta SET Man_ID = " + Man_ID
												+" , Man_Usuario= "+Usu_ID+" , Man_FechaRegistro = getdate() WHERE OV_ID= " + OV_ID		
		...
		case 3:	
			...
			var sSQLTr = "UPDATE Manifiesto_Salida"
				...
				sSQLTr += " ,  Man_TipoDeRutaCG94 = " +Man_TipoDeRutaCG94
				...
				var Disponible = BuscaSoloUnDato("TA_ID","TransferenciaAlmacen","TA_Cancelada = 0 AND TA_EstatusCG51 = 5 AND TA_ID = "+TA_ID,-1,0) 
					if( Disponible > -1){
						...
						if(HojaRuta != -1){
							...
							//	var Man_ID = SiguienteID("Man_ID","Manifiesto_Salida","",0)
								//var Man_ID = Man_ID -1
							var sSQLTr = "UPDATE TransferenciaAlmacen SET Man_CargaTransportista = 1, TA_EstatusCG51 = 5"
