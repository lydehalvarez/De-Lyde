...
<%
	...
	var ManG_GPS = Parametro("ManG_GPS","") 
		...
		case 1:
			...
			if(Man_ID != -1){
				...
				var Cancelada = BuscaSoloUnDato("TA_ID","TransferenciaAlmacen"," TA_EstatusCG51 = 11 AND TA_ID = "+TA_ID,-1,0) 
						if(Cancelada == -1){
							if(Existe == -1){
								var Remision =  BuscaSoloUnDato("ISNULL(TA_FolioRemision,-1)","TransferenciaAlmacen_FoliosEKT","TA_ID = "+TA_ID,-1,0) 
								var Cli_ID = BuscaSoloUnDato("Cli_ID","TransferenciaAlmacen","TA_ID = "+TA_ID,-1,0)  
								if((Remision != -1 && Cli_ID == 6) || (Remision == -1 && Cli_ID != 6)){
									if(Disponible > -1){
									...
									result = -1
									message = "Esta caja no contiene folio de remision, devolver embarques"
								}
						...
						if(Remision != -1){
							...
							//	var Man_ID = SiguienteID("Man_ID","Manifiesto_Salida","",0)
								//var Man_ID = Man_ID -1
								   ...
								   sSQL = "UPDATE TransferenciaAlmacen_Caja SET TAC_UsuarioSalida = "+Usu_ID+", TAC_ValidaManifiesto = 1 WHERE TAC_Codigo= '"+ TAC_Codigo +"'"
									Ejecuta(sSQL,0)
								   ...
								   result = 1
										...
										var ExisteCaja = BuscaSoloUnDato("TAC_ID","TransferenciaAlmacen_Caja","TA_ID = "+TA_ID +" AND (TAC_ValidaManifiesto = 0 OR TAC_ValidaManifiesto IS NULL)",-1,0) 
									  // Response.Write(ExisteCaja)
								   if(ExisteCaja==-1){
...
//											 			+ " WHERE c.Man_ID = "+Man_ID +" AND TAC_ValidaManifiesto=0"
...
//										}else{
							...
							result = -1
							message = "Esta caja no contiene remision, devolver embarques"
						}
				   } else {//respuesta en caso de no estar disponible
					  result = 0
        ...
        rsGuia.Close()    
		  ...
		  break;
		    case 14:
		  	...
		  	var sSQL = " INSERT INTO Manifiesto_Salida_GPS(ManG_FechaRegistro, ManG_SerieGPS, Man_ID, ManG_Usuario) "
							+ " VALUES(getdate(), '"+ManG_GPS+"', "+Man_ID+", "+Usu_ID+")"	
						//Response.Write(sSQL)
		  	 	 if(Ejecuta(sSQL,0)){
					result = 1
					message = "Datos de GPS agregados correctamente al manifiesto"
				} else {
					result = -1
					message = "Error al insertar dato, intenta de nuevo y comunicate a sistemas de ser necesario"
				}
			sResultado = '{"result":'+result+',"message":"'+message+'","manid":'+Man_ID+'}'
			Response.Write(sResultado)
		  break;
		  ...
		  case 15:
		  	  sSQL = "SELECT * FROM Manifiesto_Salida_GPS g WHERE Man_ID =" + Man_ID
