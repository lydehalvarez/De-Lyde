...
<%
    ...
    var sSQL  = "SELECT *, CONVERT(NVARCHAR(12), m.Man_FechaRegistro, 103) as Fecha "
			  ...
			  + " CONVERT(VARCHAR,DATEADD(HOUR, a.Alm_TiempoEntregaHrs,  m.Man_FechaConfirmado), 108) as HoraEntrega"
...
<table width="1148" class="table table-striped table-bordered">
    <thead>
  <th width="24"><FONT size="1">Num</FONT></th>
        <th width="126"><FONT size="1">Gu&iacute;a</FONT></th>
        <th width="47"><FONT size="1">No. cajas</FONT></th>
        <th width="89"><FONT size="1">Pedido</FONT></th>
        <th width="62"><FONT size="1">Ruta</FONT></th>
        <th width="83"><FONT size="1">Fecha a entregar</FONT></th>
        <th width="108"><FONT size="1">Estado</FONT></th>
        
        <th width="66"><FONT size="1">No. Tienda</FONT></th>
        <th width="356"><FONT size="1">Tienda</FONT></th>
        
        <th width="143"><FONT size="1">Tel&eacute;fono</FONT></th>
        <tbody>
		<%
	var iCons = 1
	while (!rsRe.EOF){
		 ...
		 var Dir_Tienda = rsRe.Fields.Item("Alm_Calle").Value +", No. Ext. " +  rsRe.Fields.Item("Alm_NumExt").Value
          ...
          var  FechaEntrega =  rsRe.Fields.Item("FechaEntrega").Value
          var  HoraEntrega =  rsRe.Fields.Item("HoraEntrega").Value
	...
	if(Estado == "CIUDAD DE MEXICO"){
		...
		<tr>
       	  ...
       	  <td valign="top">    <FONT size="1"><%=iCons%>   </FONT></td> 
         
          
       	  <td valign="top">   <small><FONT size="1" ><%=Guia%>  </FONT></small></td>
              <td valign="top">   <FONT size="1"><%=Cajas%>  </FONT></td>
          <td valign="top" > <FONT size="1"><%=Pedido%>  </FONT></td>
       	  <td valign="top">   <FONT size="1"><%=Ruta%>  </FONT></td>
          <td valign="top">   <FONT size="1"><%=FechaEntrega%></FONT></td>
       	  <td valign="top">   <FONT size="1"><%=Estado%>  </FONT></td>
          
       	  <td valign="top">   <FONT size="1"><%=Num_Tienda%>  </FONT></td>
       	  <td valign="top">   <FONT size="1"><%=Tienda%>  </FONT>
              <br><small> <FONT size="1"> <%=Contacto%></FONT></small></td>
