...
<%
    ...
    var sSQL  = " SELECT TOP (100) *,  (SELECT count(*) "
        ...
        sSQL +=     " WHERE d.Man_ID = m.Man_ID) as CANTIDAD "
		sSQL += " FROM Manifiesto_Salida m INNER JOIN Proveedor p ON m.Prov_ID=p.Prov_ID"
					+ " LEFT OUTER JOIN Cat_Catalogo c ON c.Cat_ID=Man_TipoDeRutaCG94 AND  c.Sec_ID =94 "
					+ " LEFT OUTER JOIN Cat_Estado e ON e.Edo_ID=m.Edo_ID"
        			+ " LEFT OUTER JOIN Cat_Aeropuerto a ON a.Aer_ID=m.Aer_ID  WHERE  Man_Borrador = 1"
