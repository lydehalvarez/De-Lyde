...
case 4:
    ...
    var sSQL  = " SELECT  *,(SELECT Nombre FROM dbo.tuf_Usuario_Informacion(t.Man_Usuario)) Usuario "
        sSQL += ", (SELECT COUNT (*) FROM TransferenciaAlmacen d where e.Man_ID = d.Man_ID)  AS CANTIDAD"
...
%>    
      <tr id="Entrante_<%=TA_ID%>">
        ...
        <td class="project-title">
<%
		sSQL = "SELECT TAC_ID, TAC_UsuarioMan FROM TransferenciaAlmacen_Caja WHERE TA_ID =" + TA_ID
      var rsCaja = AbreTabla(sSQL,1,0)
			if(!rsCaja.EOF){
			  ...
			  var TAC_ID
			  while (!rsCaja.EOF){
				TAC_ID = rsCaja.Fields.Item("TAC_ID").Value
				if(rsCaja.Fields.Item("TAC_UsuarioMan").Value == -1){
					 color = 'btn-outline btn-primary'
				}else{
					color ='btn-primary'	
				}
%>
        <a class="btn <%=color%>" id="<%=TA_ID%>_Caja_<%=TAC_ID%>"><%=TAC_ID%></a>
	   <%
            rsCaja.MoveNext() 
            	}
        rsCaja.Close()   
...
case 9:
    ...
    var sSQL  = " SELECT  * "
        sSQL += ", (SELECT count(*) FROM TransferenciaAlmacen_Caja  d WHERE d.Man_ID = t.Man_ID AND TAC_ValidaManifiesto = 1) as ESCANEADOS "
        sSQL += ", (SELECT count(*) FROM TransferenciaAlmacen_Caja d  WHERE d.Man_ID = t.Man_ID) as CANTIDAD "
		sSQL += " FROM TransferenciaAlmacen t, cliente c, Almacen a, Manifiesto_Salida e"
        sSQL += " WHERE t.Cli_ID=c.Cli_ID "
...
<%/*%>    <button type="button" class="btn btn-success btnImprimir" onclick="ManifiestoFunciones.Contenido_Reporte(<%=Man_ID%>)">Imprimir</button><%*/%>
      <button type="button" class="btn btn-success btn_FSalida" onclick="ManifiestoFunciones.Folio_Salida(<%=Man_ID%>)">Folio de salida</button>
  ...
  <table class="table table-hover">
    <tbody>
      ...
      <tr id="Renglon_<%=TA_ID%>">
        ...
        <td class="project-title">
<%
		sSQL = "SELECT TAC_ID, TAC_ValidaManifiesto FROM TransferenciaAlmacen_Caja WHERE TA_ID=" + TA_ID
		//Response.Write(sSQL)  
      var rsCaja = AbreTabla(sSQL,1,0)
			if(!rsCaja.EOF){
	  ...
	  var color = ""
                while (!rsCaja.EOF){
					var	TAC_ID = rsCaja.Fields.Item("TAC_ID").Value
					if(rsCaja.Fields.Item("TAC_ValidaManifiesto").Value == 1){
						 color = 'class="btn btn-primary"'
					}else{
						color ='class="btn btn-outline btn-success"'	
					}
%>
        <button type="button"  <%=color%> id="ta<%=TA_ID%>tac<%=TAC_ID%>" data-tacid="<%=TAC_ID%>"  data-taid="<%=TA_ID%>" ><%=TAC_ID%></button>
	   <%
            rsCaja.MoveNext() 
            	}
        rsCaja.Close()   
...
$(document).ready(function(){    
	...
	$('.Folio').on('keypress',function(e) {
		if(e.which == 13) {
			...
			$(this).val("") 
			if(DatoIngreso.length == 10){
				ManifiestoFunciones.EscaneaFolioApi(DatoIngreso,$(this))
			}else{
				ManifiestoFunciones.EscaneaCaja(DatoIngreso)
			}
		}
	});
		...
		if(e.which == 13) {
			...
			$.post("/pz/wms/Salidas/Manifiesto_Ajax.asp",
			{
				...
				var response = JSON.parse(data)
				  $("#ta"+response.taid+"tac"+response.tacid).toggleClass("btn-outline btn-success btn-primary");
				  if(response.validada ==1 && response.result==1){
					$("#Renglon_"+response.taid).css('background-color', '#1ab394' );
					var validados = 0
					 validados = parseInt($("#Validados").html());
					 validados = validados + 1
				  	 $("#Validados").html(validados)
					}if(response.validada ==0 && response.result==1){
					$("#Renglon_"+response.taid).css('background-color', 'yellow' );
					var validados = 0
					 validados = parseInt($("#Validados").html());
					validados = validados + 1
						$("#Validados").html(validados)
