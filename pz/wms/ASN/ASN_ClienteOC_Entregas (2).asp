    ...
    <tbody>
      ...
      <%/*%>   <td class="project-title">
        ...
        <td class="project-actions" width="31">
          ...
          <a class="btn btn-white btn-sm btnDespliegaPallets" data-asnid="<%=rsCliOC.Fields.Item("ASN_ID").Value%>"  href="#"><i class="fa fa-cubes"></i> Pallets</a>
				<button class="btn btn-danger btnCierra btn-xs" id="btnC<%=rsCliOC.Fields.Item("ASN_ID").Value%>" 
...
$(document).ready(function(){    
    ...
    $('.tooltip-demo').tooltip({
		...
		$('.btnDespliegaPallets').click(function(e) {
	         e.preventDefault();
            $(this).hide('slow')
            var ASN_ID = $(this).data('asnid')
            $('#btnC'+ASN_ID).show('slow')


            $('<tr id="tr_pt'+ASN_ID+'"><td colspan="12" id="td_pt'+ASN_ID+'">'+loading+'</td></tr>').insertAfter($(this).closest('tr'));
            var dato = {
                ASN_ID:ASN_ID
            }
            $("#td_pt"+ASN_ID).load("/pz/wms/ASN/ASN_Pallets.asp", dato);  
});    
