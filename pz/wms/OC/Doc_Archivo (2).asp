...
<%	
var VI = Parametro("VI",-1)
var CoP = ParametroDeVentana(SistemaActual, VI, "Clinete/Proveedor", "C")
//var Doc_ID = ParametroDeVentana(SistemaActual, VI, "Tipo Documento", 0)   
...
if(CoP == "C") {
    var sSQL  = " SELECT ISNULL(Cli_Clave,'') as Clave, Cli_RazonSocial, Cli_Nombre, Cli_RFC " 
        sSQL += " FROM Cliente " 
        sSQL += " WHERE Cli_ID = " + Cli_ID
    var rsCliente = AbreTabla(sSQL,1,0)

    if (!rsCliente.EOF){
        NombreCliente = rsCliente.Fields.Item("Clave").Value
        //if (rsCliente.Fields.Item("Cli_RazonSocial").Value != "") {
        //	NombreCliente += " - "  + rsCliente.Fields.Item("Cli_RazonSocial").Value
        //}
        if (rsCliente.Fields.Item("Cli_Nombre").Value != "") {
            NombreCliente += " - "  + rsCliente.Fields.Item("Cli_Nombre").Value
        }	
    }
    rsCliente.Close()    
...
if(CoP == "P") {
    var sSQL  = " SELECT Prov_RazonSocial, Prov_Nombre, Prov_RFC " 
        sSQL += " FROM Proveedor " 
        sSQL += " WHERE Prov_ID = " + Prov_ID
    var rsCliente = AbreTabla(sSQL,1,0)

    if (!rsCliente.EOF){
        NombreCliente =  rsCliente.Fields.Item("Prov_RazonSocial").Value
    }
    rsCliente.Close() 
}

var EstatusCSS = "" 
var CliOC_EstatusCG52 = 0
var OC_Factura = ""

if(CoP == "C") {
 var sSQL  = " SELECT * "
	 sSQL += " ,dbo.fn_CatGral_DameDato(52,CliOC_EstatusCG52) as Estatus "
...
if(CoP == "P") {
 var sSQL  = " SELECT * "
	 sSQL += " ,dbo.fn_CatGral_DameDato(51,OC_EstatusCG51) as Estatus "
	 sSQL += " FROM Proveedor_OrdenCompra " 
	 sSQL += " WHERE Prov_ID = " + Prov_ID
...
var rsOrdenCompra = AbreTabla(sSQL,1,0)
  if (!rsOrdenCompra.EOF){
      if(CoP == "C") {   
          Estatus = rsOrdenCompra.Fields.Item("Estatus").Value
          CliOC_EstatusCG52 = rsOrdenCompra.Fields.Item("CliOC_EstatusCG52").Value	  
      }
      if(CoP == "P") {   
          Estatus = rsOrdenCompra.Fields.Item("Estatus").Value
          CliOC_EstatusCG52 = rsOrdenCompra.Fields.Item("OC_EstatusCG51").Value	  
      }
	  if(CliOC_EstatusCG52 == 1 ) {
	  	EstatusCSS = " label-primary "
      }
	  if(CliOC_EstatusCG52 == 2 ) {
	  	EstatusCSS = " label-success "
      } 	
	  if(CliOC_EstatusCG52 == 3 ) {
...
if(CoP == "P") {
 ...
 if (!rsArchivos.EOF){
   ...
   //if(tp == "xml") {
	   Docs_Nombre = rsArchivos.Fields.Item("Docs_NombreArchivo").Value
 //  }   
   ...
   UUID = rsArchivos.Fields.Item("Docs_UUID").Value    
...
<% if(Doc_ID==7){  %>    
        <div class="row">
          <div class="col-md-2"><h4>Folio Factura</h4></div>
            <div class="col-md-8"><% 
                if (Cargado == 1) { 
                Response.Write("<h4>"+OC_Factura+"</h4>")
                } else { %>
                <input type="text" class="form-control arTexto" id="OC_Factura" name="OC_Factura" placeholder="Folio de la factura">
                <% } %><br />
          </div>
        </div>        
...
function SubeArchivo() {
	...
	if($("#Docs_ID").val() > -1) {
		...
		$.post( "/pz/wms/OC/Ajax.asp"
		      , { Tarea:13,Prov_ID:$("#Prov_ID").val(),OC_ID:$("#OC_ID").val(),Doc_ID:$("#Doc_ID").val()}
		      , function(id){
				  $("#Docs_ID").val(id)
			 	  CargaArchivoAlServidor()
			  });
...
function CargaArchivoAlServidor() {			
	...
	var sRutaArch = "/Media/wms/Cte_OC/";	
					
	var sDatos = "llaves=" + $("#Prov_ID").val() + "_" + $("#OC_ID").val() + "_" + $("#Doc_ID").val() + "_" + $("#Docs_ID").val();
		sDatos += "&Ruta=/wms/Cte_OC/";	
...
//	data.append("elArchivo", archivoBlob); 
	...
	$.ajax({
		...
		url: "/pz/wms/OC/uploadAcc.asp?"+sDatos,
		data: fdDatos,
		cache: false,
...
function GuardarABDNombreArchivo(sjsRutaArch, sNomArchivo) {
	...
	$.post("/pz/wms/OC/Ajax.asp", { Tarea:12,Prov_ID:$("#Prov_ID").val()
			 ,OC_ID:$("#OC_ID").val(),Doc_ID:$("#Doc_ID").val(),Docs_ID:$("#Docs_ID").val(),OC_Serie:$("#OC_Serie").val()
<% if(Doc_ID==7){  %>  ,OC_Factura:$("#OC_Factura").val() <% }  %>    
