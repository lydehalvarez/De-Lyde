...
// HA ID: 4 2020-sep-11 Agregados de Carga
...
switch( parseInt(rqIntTarea) ){
	...
	case 1: {
		...
		var sqlPro = "SELECT Pro.Pro_Nombre "
				+ ", Pro.Pro_SKU "
			+ "FROM Cliente_OrdenCompra_Articulos CliOCP "
				+ "INNER JOIN Producto Pro "
					+ "ON CliOCP.Pro_ID = Pro.Pro_ID "
			+ "WHERE CliOCP.CliOCP_ID = " + rqIntCliOCP_ID + " "
					+ "AND CliOCP.CliOC_ID = " + rqIntCliOC_ID + " "
					+ "AND CliOCP.Cli_ID = " + rqIntCli_ID + " "
		
		var rsPro = AbreTabla(sqlPro, 1, cxnIntTipo)

		if( !(rsPro.EOF) ) {
			strProNombre = rsPro("Pro_Nombre").Value
			strProSKU = rsPro("Pro_SKU").Value
		}
...
%>
			<div class="col-form-label form-group col-md-12 forum-item active">
                <i class="fa fa-tag fa-lg text-success"></i> <label>(<%= strProSKU %>) <%= strProNombre %></label>
            </div>
...
<%		
	...
	case 2: {
		...
		var  sqlInvLot = "SELECT DISTINCT INV.Inv_Serie "
			+ "FROM Inventario INV "
				+ "INNER JOIN Inventario_Historico INVH "
					+ "ON INV.Inv_ID = INVH.Inv_ID "
			+ "WHERE INVH.Inv_LoteActual = " + rqIntLot_ID + " "
				+ "AND INV.Pro_ID = " + rqIntPro_ID + " "


		var sqlPro = "SELECT Pro.Pro_Nombre "
				+ ", Pro.Pro_SKU "
			+ "FROM Producto Pro "
			+ "WHERE Pro.Pro_ID = " + rqIntPro_ID + " "
