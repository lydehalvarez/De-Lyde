...
<%	
	...
	var sSQLEmp = " SELECT OC_BPM_Flujo "
		sSQLEmp += " FROM OrdenCompra "
		sSQLEmp += " WHERE OC_ID = " + OC_ID
        //Response.Write(sSQLEmp)
	ParametroCargaDeSQL(sSQLEmp,0)
    
	var OCParD_Autorizado = 0

	var btnHabilitados = 0
	if ( Parametro("OC_BPM_Flujo",0) == 1 ) {
		btnHabilitados = 1
	}    
...
<div class="table-responsive">
        <table class="footable table table-striped table-hover" data-sort="false">
        <thead>
            <tr>
        ...
        <tbody>
            <%
                ...
                var sSQL = " SELECT OC_ID, OCParD_ID, OCParD_Fecha, OCParD_Folio, OCParD_Descripcion, OCParD_Autorizado "
                ...
                while (!rsRDet.EOF){
                   iRenglon++
                   OC_ID = rsRDet.Fields.Item("OC_ID").Value
            ...
            <tr onDblClick="javascript:EditaDocumento(<%=OCParD_ID%>)">
                ...
                <table width="100%" border="0">
                  <tbody>
                    <tr onDblClick="javascript:EditaDocumento(<%=OC_ID%>,<%=OCParD_ID%>)">
                ...
                <td width="85" align="center" valign="middle" class="text-right footable-visible footable-last-column"><% if (btnHabilitados == 1){                             %><a href="javascript:EditaDocumento(<%=OC_ID%>,<%=OCParD_ID%>)">Seleccionar</a><% }%></td> 
            </tr>
            <%
            	rsRDet.MoveNext() 
            	}
...
<script language="jscript" type="application/javascript">
	...
	$(document).ready(function() { 
        //http://mossbk.no/sites/all/libraries/FooTable/demos/expand-collapse-all.htm#docs
        $('.table').footable({
            //"expandAll": true
            addRowToggle: true
        });        
