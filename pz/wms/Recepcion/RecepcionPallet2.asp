...
function RecepImprime(lpn, folio){
	...
	$('.btnEliminar').click(function(e) {
		...
		datoAgenda['Tarea'] = 5
		$("#dvPallet").load("/pz/wms/Recepcion/RecepcionPallet_Ajax2.asp",datoAgenda)
...
function GuardaLPN(Evento){
			$("#dvPallet").load("/pz/wms/Recepcion/RecepcionPallet_Ajax2.asp",Evento
...
function BorraEvento(id){
	$.post("/pz/wms/Recepcion/Recepcion_Ajax2.asp",{
		Tarea:4
		,IR_ID:id
		,IDUsuario:$("#IDUsuario").val()
	}
...
$('#cboProducto').change(function(e) {
        e.preventDefault()
		$.get("/pz/wms/Recepcion/RecepcionPallet_Ajax2.asp",{
			Tarea:2
			,Pro_SKU:$(this).val()
		}, function(data){
			if (data != "") {
			...
			var sDatos = "Pro_SKU=" + $(this).val();	 
				sDatos += "&Tarea="+3
		$("#dvAlfanum").load("/pz/wms/Recepcion/RecepcionPallet_Ajax2.asp?" + sDatos)
				sDatos = "Pro_SKU=" + $(this).val();	 
				sDatos += "&Tarea="+7
		$("#divSims").load("/pz/wms/Recepcion/RecepcionPallet_Ajax2.asp?" + sDatos)
