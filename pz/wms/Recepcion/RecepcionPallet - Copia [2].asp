...
<%
	...
	if(CliOC_ID > -1){
		...
		if (OC_ID > -1) {
			var sSQL = "SELECT c.Prov_ID, a.Pro_ID, c.OC_Folio, a.OCP_SKU, OCP_Cantidad as articulos, Prov_Nombre "
				sSQL += " FROM Proveedor_OrdenCompra c INNER"
				sSQL += " JOIN  Proveedor_OrdenCompra_Articulos a "
				sSQL += " ON c.OC_ID = a.OC_ID AND c.Prov_ID=a.Prov_ID "
				sSQL += " inner join proveedor p on  p.Prov_ID=c.Prov_ID"
				sSQL += " WHERE c.Prov_ID = " + Prov_ID
				sSQL += " AND c.OC_ID = " + OC_ID
		...
		if (!rsArt.EOF) {
			...
			while (!rsArt.EOF) {
				...
				if (Cli_ID > -1) {
					var sSQL = "SELECT  sum(Pt_Cantidad)  as Cantidad_Art "
				...
				if (OC_ID > -1) {
					...
					sSQL += " INNER JOIN Proveedor_OrdenCompra e  "
					sSQL += "  ON e.OC_ID = p.OC_ID AND e.Prov_ID = p.Prov_ID "
					sSQL += " WHERE p.IR_ID = " + IR_ID
					sSQL += " AND p.OC_ID= " + OC_ID
				...
				var restantes = 0 
				if(!rsArtRes.EOF){
					restantes = rsArtRes.Fields.Item("Cantidad_Art").Value
				}
	...
	//	        }
				...
				if (OC_ID > -1) {
					sSQL = "SELECT r.OCP_Cantidad as articulos "
						+ " FROM Producto_Proveedor c "
						+ " INNER JOIN Producto p ON p.Pro_ID = c.Pro_ID "
						+ " INNER JOIN Proveedor_OrdenCompra_Articulos r ON p.Pro_ID = r.Pro_ID   "
						+ " WHERE  r.OC_ID = " + OC_ID + " AND r.Prov_ID = " + Prov_ID
				} else {
					sSQL = "SELECT r.CliEnt_CantidadArticulos as articulos, r.CliEnt_CantidadPallet "
						+ " FROM  Producto_Cliente c "
						+ " INNER JOIN Producto p ON p.Pro_ID = c.Pro_ID "
						+ " INNER JOIN Cliente_OrdenCompra_EntregaProducto r "
						+ " ON p.Pro_ID = r.Pro_ID "
						+ " INNER JOIN  Cliente_OrdenCompra_Entrega en ON  en.CliOC_ID = r.CliOC_ID "
						+ " AND en.Cli_ID=r.Cli_ID and en.CliEnt_ID = r.CliEnt_ID "
						+ " WHERE  en.IR_ID = " + IR_ID
	...
	var Cons_Pallet = SiguienteID("Pt_Pallet", "Recepcion_Pallet", "IR_ID = " + IR_ID, 0)
								...
								<table class="table shoping-cart-table">
									<tbody>
										<tr>
											...
											<td class="desc"><%Cantidad_Art = Cantidad_Art - restantes%>
                        ...
                        %>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <span class="pull-right"> <a data-irid="" data-folio="" class="text-muted btnEditar"><i
                                                class="fa fa-pencil fa-fw"></i>&nbsp;<strong></strong></a></span>
                                    <span class="pull-right"> <a data-ptid="<%=Pt_ID%>" class="text-muted btnEliminar"><i
                                                class="fa fa-trash-o"></i>&nbsp;<strong>|</strong></a></span>
                                    <span class="pull-right"> <a data-lpn="<%=Pt_LPN%>" data-folio="<%=Folio%>" class="text-muted btnImprimeLPN"><i
                                                class="fa fa-print"></i>&nbsp;<strong>|&nbsp;</strong></a></span>
                                    <a data-taid="0" data-cliocid="<%=CliOC_ID%>" data-ocid="<%=OC_ID%>" data-provid="<%=Prov_ID%>"
                                        data-cliid="<%=Cli_ID%>" data-ptid="<%=Pt_ID%>" data-proid="<%=Pro_ID%>" data-pt="<%=Pt%>"
                                        class="text-muted btnEscaneo"><i class="fa fa-inbox"></i>&nbsp;<strong>Escanear </strong></a>
...
-->
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-3">
                            <input class="form-control agenda" id="Pt_Color" placeholder="" type="text"
                                autocomplete="off" value="" />
                        </div>
                        <label class="control-label col-md-3"><strong>LPN cliente</strong></label>
                        <div class="col-md-3">
                            <input class="form-control agenda" id="Pt_LPN" placeholder="" type="text" autocomplete="off"
                                value="" />
                            <label class="control-label col-md-3 agenda" id="Pt_LPN2"
                                value="<%=LPN%>"><strong><%=LPN%></strong></label>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-md-3"><strong>Cantidad producto</strong></label>
                        <div class="col-md-3">
                            <input class="form-control agenda" id="Pt_Cantidad" placeholder="" type="number" min="1"
                        ...
                        <div class="col-md-3">
                            <input class="form-control agenda" id="Pt_CantidadEsperada" placeholder="" min="1"
                                type="number" autocomplete="off" value="1" />
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-3">
                            <input class="form-control agenda" id="CliEnt_CantidadArticulos" placeholder="" min="1"
                                type="number" autocomplete="off" value="1" />
                        ...
                        <div class="col-md-3">
                            <input class="form-control agenda" id="Digitos" placeholder="" type="number" min="1"
                                autocomplete="off" value="1" /> <br />
...
-->
                    ...
                    <div class="form-group">
                        <label class="control-label col-md-3" style="display:none;"><strong>Filas
                                Pallet</strong></label>
                        <div class="col-md-3">
                            <input class="form-control agenda" id="FilasPallet" placeholder="" type="number" min="1"
                                autocomplete="off" value="1" style="display:none;" />
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <input class="form-control inpUbiID" id="inpUbiID" style="display:none;width:55%" placeholder=""
                    type="text" autocomplete="off" value="" />
                <button type="button" class="btn btn-danger" id="BtnCancelar">Cancelar entrega</button>
                <button type="button" class="btn btn-primary BtnMuestraUbic" id="BtnMuestraUbic" data-toggle="modal"
                    data-target="#myModal"
                    onclick="Ubicacion.SeleccionAvanzadaAbrir({Selector: 'inpUbiID', Etiqueta: 'lblUbiNombre'});">Asignar
                    ubicacion</button>
...
$(document).ready(function() {
	...
	$('#BtnGuardar').click(function (e) {
	    e.preventDefault()
	    var Pro_TipoProductoCG70 = $("#cbo_TipoProductoCG70").val()
	    if (Pro_TipoProductoCG70 == 0) {
	        sTipo = "warning";
	        sMensaje = "Es necesario asignar el tipo de producto. ";
	        Avisa(sTipo, "Aviso", sMensaje);
	    } else {
	        var datoAgenda = {}
	        ...
	        datoAgenda['Pro_TipoProductoCG70'] = $('#cbo_TipoProductoCG70').val()
