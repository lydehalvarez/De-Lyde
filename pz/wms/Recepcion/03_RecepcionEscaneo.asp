...
<%
	...
	if (Tarea==-1){
			 ...
			 var inputUbicacion =  'style="display:none;width:150%"'
			var Pro_ID = Parametro("Pro_ID",-1)
...
if(CliOC_ID > -1){
	 ...
	 sSQLTr += "TA_ID = "+TA_ID
			...
			Articulos = rsSer.Fields.Item("Articulos").Value 
				   ...
				   if(OC_ID > -1){
sSQL = "SELECT  r.CliEnt_CantidadArticulos as Pro_CantidadMB, r.CliEnt_CantidadPallet as Pro_CantidadPt FROM  Producto_Cliente c INNER JOIN Producto p ON p.Pro_ID = c.Pro_ID INNER JOIN Proveedor_OrdenCompra_EntregaProducto r ON  p.Pro_ID = r.Pro_ID  WHERE  r.OC_ID = "+OC_ID+" AND r.Pro_ID= "+Pro_ID+"  AND r.ProvEnt_ID = "+CliEnt_ID
				   }else{
sSQL = "SELECT  r.CliEnt_CantidadArticulos as Pro_CantidadMB, r.CliEnt_CantidadPallet as Pro_CantidadPt FROM  Producto_Cliente c INNER JOIN Producto p ON p.Pro_ID = c.Pro_ID INNER JOIN Cliente_OrdenCompra_EntregaProducto r ON  p.Pro_ID = r.Pro_ID  WHERE  r.CliOC_ID = "+CliOC_ID+" AND r.Pro_ID= "+Pro_ID+" AND r.CliEnt_ID = "+CliEnt_ID
				   }
	...
	if(CliOC_ID > -1){
   	var sSQL1  = "select  p.Pro_ID, p.ProC_Nombre, p.ProC_SKU, CONVERT(VARCHAR(17), getdate(), 111) AS Hoy, CONVERT(VARCHAR(8),  getdate(), 108) AS Hora from Producto_Cliente p "
		...
		if(OC_ID > -1){
   	var sSQL1  = "select  p.Pro_ID, p.ProC_Nombre, p.ProC_SKU, CONVERT(VARCHAR(17), getdate(), 111) AS Hoy from Producto_Proveedor p "
...
<div class="form-horizontal" id="toPrint">
    ...
    sSQLTr += "WHERE CliOCP_SKU = '"+ rsPro2.Fields.Item("ProC_SKU").Value+"' AND CliOC_ID = "+CliOC_ID
	 ...
	 var condicion3 = "FROM Recepcion_Series WHERE  Ser_Serie = '"+Ser_Serie+"'"
}
...
if(OC_ID > -1){
	 ...
	 var condicion = "FROM Proveedor_Inventario WHERE  OC_ID = '"+ OC_ID +"'"
	 var condicion2 = "FROM Proveedor_Inventario WHERE PInv_NumeroSerie = '"+Ser_Serie+"' "
	 var condicion3 = "FROM Recepcion_Series WHERE   Ser_Serie = '"+Ser_Serie+"'"
	 ...
	 var condicion3 = "FROM Recepcion_Series WHERE  TA_ID = '"+ TA_ID +"'"
        ...
        sSQLTr += " WHERE Pt_ID = " + Pt_ID
		   ...
		   var rsPallet = AbreTabla(sSQLTr,1,0) 
			 ...
			 case 1:
			 		 var inputEscan = ""
			  var sSQLTr  = "SELECT * FROM Recepcion_Series where Ser_SerieEscaneada = 1 AND Ser_Serie = '"+Ser_Serie+"'"
				 var rsSer = AbreTabla(sSQLTr,1,0) 
...
%>
     <input type="button" value="Asignar ubicacion a pallet"  data-ptlpn="<%=Pt_LPN%>" data-folio="<%=Folio%>" id="BtnUbicar" class="btn btn-info btnUbicar"/>
						<%			} 
				 }
			
if(MB<= Cantidad_Pallet){
								
 	  var sSQLTr  = "SELECT * "+ condicion //BUSCA SI HAY DATOS EN Cli_Inventario DE LA OC
	  var rsSer = AbreTabla(sSQLTr,1,0) 
	
	  	 var sSQLTr  = "SELECT * "+ condicion2 //BUSCA SI SI EXISTE LA SERIE EN Cli_Inventario
	  var rsSer2 = AbreTabla(sSQLTr,1,0) 

	 var sSQLTr  = "SELECT * "+ condicion3 // BUSCA SI EXISTE LA SERIE EN Recepcion_Series 
	  var rsSer3 = AbreTabla(sSQLTr,1,0) 
	
		  if( !(rsSer.EOF) ){
			  if( !(rsSer2.EOF) ){
				    if( !(rsSer3.EOF) ){
sSQLTr = "UPDATE Recepcion_Series SET Ser_SerieEscaneada = 1, Pt_ID = "+Pt_ID+",  Ser_MB= "+MB+", Ser_Pallet="+Pallet
...
sSQLTr  += "Ser_MB, Ser_Pallet, Ser_SerieEscaneada,Pt_ID, CliEnt_ID, Ser_Incidencia)  "
sSQLTr  += "values("+Ser_ID+"," +OC_ID +"," +Prov_ID +"," +CliOC_ID +", " +Cli_ID+", '"+Ser_Serie+"', "+Pro_ID+",  "+MB+","+Pallet+",1,"+Pt_ID+","+CliEnt_ID+",1)"				
		Ejecuta(sSQLTr, 0)
			}
		}else	{
...
//						Ejecuta(sSQLTr, 0)
  ...
  sSQLTr="EXEC SPR_UbicarPallet  @Are_ID=1,@Pt_LPN='"+Pt_LPN+"'"
  
 AbreTabla(sSQLTr,1,0) 
	
sSQLTr=" UPDATE Inventario_Recepcion SET IR_IngresoFecha = '"+rsPro2.Fields.Item("Hoy").Value+" "+rsPro2.Fields.Item("Hora").Value+"' , IR_IngresoResultado=1, IR_IngresoUsuario = "+IDUsuario+" WHERE IR_ID ="+IR_ID
	Ejecuta(sSQLTr, 0)
...
var cons = "SELECT CliEnt_CantidadPallet, CliEnt_CantidadArticulos AS Pro_Cantidad, CliEnt_TipoMasterBox FROM Cliente_OrdenCompra_EntregaProducto "
			var cond ="WHERE CliOC_ID= "+CliOC_ID+" and  Pro_ID = "+Pro_ID+" AND CliEnt_ID = "+CliEnt_ID
			}
				if(OC_ID > -1){
...
var cons = "SELECT ProvEnt_CantidadPallet, ProvEnt_CantidadArticulos AS Pro_Cantidad, ProvEnt_TipoMasterBox FROM Proveedor_OrdenCompra_EntregaProducto "
	var cond = "WHERE OC_ID= "+OC_ID+" and  Pro_ID = "+Pro_ID+" AND ProvEnt_ID = "+CliEnt_ID
			...
			var Cantidad_MB = rsMB.Fields.Item("Pro_Cantidad").Value
    ...
    <input type="text" value="<%=Pt_LPN%>" style="display:none;width:150%"  class="objAco agenda"  id="Pt_LPN">
            <tr>
                ...
                <td><%=escaneadas%></td>
                                     ...
                                     <td class="desc">
 <label class="control-label col-md-1" id="InputPeso<%=Pt_ID%>" style="display:none;width:80%"><strong>Rayos X <%=Peso%> </strong></label>   <input type="button"  data-ptid="<%=Pt_ID%>" id="BtnAprobado<%=Pt_ID%>" class="btn btn-primary BtnAprobado"  value= "Aprobar" style="display:none;width:70%"> </input>
                                   ...
                                   <%
								    Pallet = Pt_LPN.charAt(Pt_LPN.length - 1);
...
<input type="button" value="Escanear masterbox"  <%=inputMB%> data-ptid="<%=Pt_ID%>" id="btnEscanear<%=Pt_ID%>" class="btn btn-info btnEscanear"/><%
	...
	<%
			var sSQLTr  = "SELECT  top(20) * FROM Recepcion_Series  WHERE Ser_SerieEscaneada = 1 AND Pt_ID = "+Pt_ID+" AND Pro_ID = "+Pro_ID
	if(CliOC_ID > -1){
	...
	%>
       ...
       <div class="col-md-3">
			...
			sSQL += " AND Pro_ID = "+Pro_ID+" AND Ser_MB= "+MB+" AND Ser_Pallet = "+Pallet
			}
				  if(OC_ID > -1){
		...
		if(rsMasterbox.EOF==false){
			 var Articulos = rsMasterbox.Fields.Item("Cantidad").Value 			
				...
				MB=0		
					...
					%>
                  <td>Articulos masterbox: <%=Articulos%>/<%=Cantidad_MB%> <BR />Masterbox:  <%=MB%>/<%=Cantidad_Pallet%>  <br />Pallet: <%=Pallet%></td>
...
$('#BtnUbicar').click(function(e) {
	...
	var datoAgenda = {}
		...
		datoAgenda['IDUsuario'] = <%=IDUsuario%>
		var Pt_LPN = $(this).data("ptlpn")
		
		datoAgenda['Pt_LPN'] = Pt_LPN
		datoAgenda['Tarea'] =3
	$("#Contenido").load("/pz/wms/Recepcion/RecepcionEscaneo.asp",datoAgenda ,
	 function(data){
		...
		var newWin=window.open("http://qawms.lyde.com.mx/pz/wms/Recepcion/RecepcionLPNImpreso.asp?Pt_LPN="+ lpn+"&Folio="+ folio+"&Tarea=1");
