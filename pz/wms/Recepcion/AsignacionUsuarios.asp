...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="FichaCampoValor">  
  <tr>
    <td>&nbsp;</td>
    <td colspan="3"><label class="col-sm-1 control-label">Linea :</label>  <h3>
<%
	var sEventos = " class='form-control col-sm-6' onchange='javascript:CambioRol()' "
	var sCondicion = " Pro_ID = " + Pro_ID

	CargaCombo("xProR_ID",sEventos,"ProR_ID","ProR_Nombre","BPM_Proceso_Rol",sCondicion,"",ProR_ID,0,"Seleccione una linea","Edicion") 
...
%>
          <tr>
            <td align="center">
              <input type="checkbox" id="chkSelecc" class="chHab" data-usuid="<%=Usu_ID%>" <%=chkHabilitado%> >
            </td>
            <td>&nbsp;<%=Usu_Siglas%></td>
            <td>&nbsp;<%=Usu_Nombre%></td>
            <td>&nbsp;<%=Usu_Email%></td>            
...
function CambiaEstatus(Checado,usuid) {
	...
	$.post( "/pz/agt/Usuarios/Usuarios_Ajax.asp"
		  , { Tarea:10,ID_Unico:usuid,IDUsuario:$("#IDUsuario").val()
		      ,Pro_ID:$("#Pro_ID").val()
