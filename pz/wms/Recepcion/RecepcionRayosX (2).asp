...
%>
 <a  data-ocid= "<%=OC_ID%>"  data-cliid= "<%=Cli_ID%>" data-provid= "<%=Prov_ID%>" data-cliocid= "<%=CliOC_ID%>" data-irid= "<%=IR_ID%>" data-client="<%=CliEnt_ID%>"  class="text-muted btnClasificar"><i class="fa fa-inbox"></i>&nbsp;<strong>Regresar a clasificacion de Pallets</strong></a> <br />
                <a data-taid="0" data-cliocid="<%=CliOC_ID%>" data-ocid="<%=OC_ID%>" data-provid="<%=Prov_ID%>" data-cliid="<%=Cli_ID%>" data-ptid="<%=Pt_ID%>" data-proid="<%=Pro_ID%>" data-pt="<%=Pallet%>"  class="text-muted btnEscaneo"><i class="fa fa-inbox"></i>&nbsp;<strong>Escanear series</strong></a>
                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                        <div class="form-group">
                            <legend class="control-label col-md-12" style="text-align: left;"><h1>Folio:&nbsp;<%=Folio%> - RAYOS X</h1></legend>
                        </div>
                    ...
                    <div style="overflow-y:scroll; height:100px;">
                    	...
                    	<div class="table-responsive" id="dvTablaInfoMB"></div>
                    </div>
                </div>
            </div>
        </div>
...
var FunctionRecepcion = {
	GetTabla:function(){
		$.post("/pz/wms/Recepcion/RecepcionRayosX_Ajax.asp",
		{Pt_ID:<%=Pt_ID%>,
		Tarea:1
		}
		,function(data){
	...
	InsertSerie:function(event){
		...
		if( keyNum== 13 ){
			...
			$.ajax({
				method: "POST",
				url: "/pz/wms/Recepcion/RecepcionRayosX_Ajax.asp?",
				data: {
					Pt_ID:<%=Pt_ID%>,
					Tarea:2,
					Pro_ID:<%=Pro_ID%>,
...
//		});	
	...
	var FunctionSerie = {
			InsertDigitos:function(){
				
					$.ajax({
    method: "POST",
    url: "/pz/wms/Recepcion/RecepcionRayosX_Ajax.asp?",
    data: {Pt_ID:<%=Pt_ID%>,
		 	  Pt_Digitos:$('#Digitos').val(),
		Tarea:5
		},
    cache: false,
    success: function (data) {
