...
<%
	...
	var SIMIni = Parametro("SIMIni", "")
	var SIMFin = Parametro("SIMFin", "")
	var Escaneos = Parametro("Escaneos", -1)
	var Respuesta = ""
	var result = 0
	var message = ""
...
<%    
	 var sSQLTr  = "SELECT *, "
		sSQLTr  += " p.Pro_SKU  SKU,  Pro_ID_RFID, Pro_RFIDCG160"
		sSQLTr  += " , a.Pt_Modelo  as Modelo, Pro_SerieAlfanumerico, a.Pro_ID, a.Pro_ID, Pro_EsSerializado"
...
//	 	 Response.Write(sSQLTr)
	...
	if(!rsPt.EOF){
	  ...
	  var Pro_ID_RFID = rsPt.Fields.Item("Pro_ID_RFID").Value
	  
		sSQL = "SELECT Pro_PrefijoRFID FROM Producto WHERE Pro_ID="+Pro_ID_RFID
		var rsRFID = AbreTabla(sSQL,1,0)
		var Prefijo = "" 
		if(!rsRFID.EOF){
			 Prefijo =  rsRFID.Fields.Item("Pro_PrefijoRFID").Value
		}else{
			Prefijo = "9000"
		}
...
var Cantidad  = "SELECT COUNT(*) Cantidad "
		...
		%>
           ...
           <%	
            ...
            <td>
           	<%
			if(rsTPro.Fields.Item("TPro_ID").Value != 20){	
				%>
     	...
     	<input class="form-control inputMBSerie"  id="inputMBSerie" style="display:none;width:55%" placeholder="etiqueta MB" type="text" autocomplete="off" value="" onkeydown="FunctionRecepcion.InsertMBSerie(event);"/>
           ...
           <%	
            ...
            <input class="form-control inputMB"  id="inputMB" style="display:none;width:55%" placeholder="cantidad" type="text" autocomplete="off" value="" />
            ...
            "FunctionUbic.InsertUbicManual()">Guardar ubicacion</button>
           	...
           	<input class="form-control Serie"  id= "Serie" style="display:none;width:100%" placeholder="escanea la serie" <%/*%>maxlength="<%=Digitos%>" <%*/%>type="text" autocomplete="off" value="" onkeydown= "FunctionRecepcion.InsertSerie(event);"/>
            			<input class="form-control Serie3D"  id= "Serie3D" style="display:none;width:100%" placeholder="escanea la serie 3D" type="text" autocomplete="off" value="" onkeydown= "FunctionRecepcion.InsertSerie3D(event);"/>
			...
			%>
                ...
                <input class="form-control SIMIni" id="SIMIni" autocomplete="off" value="" placeholder="escanea la primer SIM" type="text" onkeydown= "FunctionRecepcion.InsertSim(event);"/>
                <input class="form-control SIMFin" id="SIMFin" autocomplete="off" value=""  placeholder="escanea la ultima SIM"   type="text" onkeydown= 
"FunctionRecepcion.InsertSim(event);"/> 
<%
		}
%>
...
%>
	        <input class="form-control Pallet"  id= "inputLPN" style="width:30%" placeholder="Escanea el proximo pallet" type="text" autocomplete="off" value=""   onkeydown="FunctionPallet.CargaPallet(event);"/><br/>
	        <input class="form-control SKU"  id= "inputSKU" style="width:20%" placeholder="Escanea el SKU" type="text" autocomplete="off" value=""   onkeydown="FunctionPallet.CargaPallet(event);"/>
...
//		terminacion de escaneo de pallet para ubicar en rack
	var sSQLTr  = "SELECT Pro_TipoProductoCG70, Pro_SerieDigitos, Pro_RFIDCG160  FROM Producto"
        sSQLTr += " WHERE Pro_ID = " + Pro_ID
		   ...
		   var Digitos = rsTPro.Fields.Item("Pro_SerieDigitos").Value
		   var Tipo = rsTPro.Fields.Item("Pro_TipoProductoCG70").Value
	...
	if(caracteres == NoCar){
		...
		if(RFID!=""){
			ExistenciaRFID = BuscaSoloUnDato("Ser_MB","Recepcion_Series","Ser_RFID = '"+RFID+"'",-1,0) 
		}
		if(Existencia > -1||ExistenciaRFID>-1){
			result = 0
			if(Existencia > -1){
 			Pallet_ID = BuscaSoloUnDato("Pt_ID","Recepcion_Series","Ser_Serie = '"+Serie+"'",-1,0) 
 			LPNError = BuscaSoloUnDato("Pt_LPN","Recepcion_Pallet","Pt_ID = '"+Pallet_ID+"'",-1,0) 
			MBError = BuscaSoloUnDato("Ser_MB","Recepcion_Series","Ser_Serie = '"+Serie+"'",-1,0) 

			message = "Error: la serie "+Serie+" ya fue escaneada anteriormente en el masterbox " + MBError + " LPN:" + LPNError
			}
			if(ExistenciaRFID>-1){
 			Pallet_ID = BuscaSoloUnDato("Pt_ID","Recepcion_Series","Ser_RFID = '"+RFID+"'",-1,0) 
 			LPNError = BuscaSoloUnDato("Pt_LPN","Recepcion_Pallet","Pt_ID = '"+Pallet_ID+"'",-1,0) 
		
			message = "Error: el RFID "+RFID+" ya fue escaneada anteriormente en el masterbox " + ExistenciaRFID + " LPN:" + LPNError
			
		}
		}else{
	
			sSQL = "SELECT Pro_Prefijo,Pro_PrefijoVariable FROM Producto WHERE Pro_ID ="+Pro_ID
			var rsPrf = AbreTabla(sSQL,1,0) 

			var validado = -1
			var Prefijo = rsPrf.Fields.Item("Pro_Prefijo").Value
			var Pro_PrefijoVariable = rsPrf.Fields.Item("Pro_PrefijoVariable").Value
			
			if((Prefijo == null || Prefijo == Serie.substr(0,8)) || (Pro_PrefijoVariable == 1  && Prefijo == Serie.substr(0,Prefijo.length)) ){
				validado = 1
			}else{
				validado = 0
			}
		...
		if(!rsCan.EOF){
		   ...
		   var rsCan = AbreTabla(sSQLTr,1,0) 
			...
			if(Pro_PrefijoVariable == 0){
				if(Prefijo == null && Escaneos == 9 && Tipo == 1){
				...
				var rsCan = AbreTabla(Cantidad,1,0) 
					if(!rsCan.EOF){
						var Cantidad = rsCan.Fields.Item("Cantidad").Value
						if(Cantidad ==10){
							sSQL = " select distinct(SUBSTRING(Ser_Serie,1,8)) as prefijo FROM Recepcion_Series WHERE Pt_ID ="+Pt_ID
							var rsPrefijo = AbreTabla(sSQL,1,0) 
							if(rsPrefijo.RecordCount >1){
								sSQL = "DELETE FROM Recepcion_Series WHERE Pt_ID= "+Pt_ID
								Ejecuta(sSQL,0)
									message = "Las series han sido eliminadas por error en escaneo de las series. Por favor vuelva a comenzar el escaneo del masterbox."
									result = 8
				
							}else{
								sSQL = "UPDATE Producto SET Pro_Prefijo = '"+rsPrefijo.Fields.Item("prefijo").Value+"' WHERE Pro_ID= "+Pro_ID
								//Response.Write("'distinct: "+rsPrefijo.RecordCount+"'")
								Ejecuta(sSQL,0)
									}
					...
					message = "La serie "+Serie+" no cumple con el prefijo vuelva a escanear"
					result = 7
...
if(Cantidad_MB > -1 || TriDim > -1|| SIMFin != ""){
				if(MB == 1){
					sSQLTr  = " SELECT a.CliOC_ID, a.Cli_ID, a.OC_ID, a.Prov_ID FROM Recepcion_Pallet a "
					sSQLTr  += " WHERE Pt_ID = "+Pt_ID
						var rsOC = AbreTabla(sSQLTr,1,0) 
...
if(Ejecuta(sSQLTr1,0) && Ejecuta(sSQLTr2,0) && Ejecuta(sSQLTr3,0) && Ejecuta(sSQLTr4,0) && Ejecuta(sSQLTr5,0) && Ejecuta(sSQLTr6,0) && Ejecuta(sSQLTr7,0) && Ejecuta(sSQLTr8,0) && Ejecuta(sSQLTr9,0) && Ejecuta(sSQLTr10,0)){
	...
	case 9:
		var caracteres = Serie.length
	if(caracteres == 19){
		var Existencia = BuscaSoloUnDato("Ser_ID","Recepcion_Series","Ser_Serie = '"+SIMIni+"'",-1,0) 
		var Existencia2 = BuscaSoloUnDato("Ser_ID","Recepcion_Series","Ser_Serie = '"+SIMFin+"'",-1,0) 
	
		if(Existencia > -1||Existencia2 > -1){
			result = 0
			message = "Error: Una o mas sims ya fueron escaneadas anteriormente"
		}else{
		var sSQLTr = ""
 ...
 var sSQLTr="UPDATE Inventario_Recepcion SET IR_EstatusCG52 = 14 WHERE IR_ID="+ IR_ID
