...
var Prov_ID = Parametro("Prov_ID",-1)
	...
	if(!rsRec.EOF){
		...
		NumCitas = rsRec.Fields.Item("Citas").Value
			...
			var ProvCli = "Cliente"
			var ssql = "SELECT * FROM TransferenciaAlmacen t INNER JOIN inventario_Recepcion r ON r.TA_ID=c.TA_ID" 
		...
		var ssql = "SELECT * FROM Cliente_OrdenCompra o"
		     ssql += " INNER JOIN inventario_Recepcion r ON r.CliOC_ID=o.CliOC_ID INNER JOIN Cliente c ON c.Cli_ID=o.Cli_ID where o.CliOC_ID=" + OC_ID
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div id='external-events'>
                        ...
                        <p>&nbsp;</p>
                        <div class='external-event navy-bg' id="Folio">Folio: <%=IR_Folio%></div>
                        <%while(!rsCliente.EOF){
							...
							var day = IR_FechaEntrega.getDay()
							...
							segundos = 	"0"+ segundos 
							 ...
							 var IR_Horario =  horas + ":" +minutos + ":" + segundos
							  IR_FechaEntrega =  rsCliente.Fields.Item("IR_FechaEntrega").Value
							 %>
                           <div class='external-event navy-bg' id="Cita" data-fecha="<%=IR_Fecha%>" data-puerta="<%=IR_Puerta%>" data-conductor="<%=IR_Conductor%>" data-placas="<%=IR_Placas%>" data-vehiculo="<%=IR_DescripcionVehiculo%>" data-almacen="<%=IR_AlmacenRecepcion%>" data-color="<%=IR_Color%>" data-horario="<%=IR_Horario%>"><%=IR_FechaEntrega%></div>
                           <%
						   	   rsCliente.MoveNext() 
                                    }
                                  	 rsCliente.Close()   	
...
var MinutosPallet = 0
	if(OC_ID > -1){
	var sSQL = "SELECT c.Cli_ID, a.Pro_ID, c.CliOC_Folio, a.ClIOCP_SKU, CliOCP_Cantidad as articulos FROM Cliente_OrdenCompra c INNER JOIN  "
	      sSQL += "Cliente_OrdenCompra_Articulos  a ON c.CliOC_ID = a.CliOC_ID WHERE a.CliOC_ID = "+OC_ID+""
   var rsArt = AbreTabla(sSQL,1,0)
	var Folio =  rsArt.Fields.Item("CliOC_Folio").Value

	}else{
	var sSQL = "SELECT t.Cli_ID, a.Pro_ID, t.TA_Folio, a.TAA_SKU, TAA_Cantidad  as articulos FROM TransferenciaAlmacen t INNER JOIN  "
		  sSQL += " TransferenciaAlmacen_Articulos  a ON t.TA_ID = a.TA_ID WHERE a.TA_ID = "+TA_ID
			   var rsArt = AbreTabla(sSQL,1,0)
...
$('#Cita').click(function(e) {
				$('.Robin').attr('disabled',false)
				$('#IR_Conductor').val($(this).data("conductor"))
				$('#IR_Placas').val($(this).data("placas"))
				 $('#IR_DescripcionVehiculo').val($(this).data("vehiculo"))
				$('#Alm_ID').val($(this).data("almacen"))
				$('#IR_Color').val($(this).data("color"))
				$('#IR_FechaEntrega').val($(this).data("fecha"))
				$('#IR_Horario').val($(this).data("horario"))
				$('#IR_Puerta').val($(this).data("puerta"))
