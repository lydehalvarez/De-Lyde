...
var sSQL = "SELECT s.IR_ID, s.IR_Folio, s.CliOC_ID, s.Cli_ID, s.OC_ID, s.Prov_ID FROM Inventario_Recepcion s  INNER JOIN Recepcion_Pallet p ON p.IR_ID= s.IR_ID INNER JOIN Recepcion_Series r ON r.Pt_ID=p.Pt_ID WHERE IR_EstatusCG52 <>18"
...
var rsOrdenes = AbreTabla(sSQL,1,0)
 ...
 sSQL = "SELECT s.Pt_ID, p.Pt_LPN, p.Pt_Incidencia FROM Recepcion_Series s INNER JOIN Recepcion_Pallet p ON  p.Pt_ID=s.Pt_ID WHERE  s.Ser_SerieEscaneada = 1 AND p.IR_ID = "+ IR_ID +" group by  s.Pt_ID, p.Pt_LPN,  p.Pt_Incidencia "
