...
//    rsRol.Close()
    ...
    if(Tarea == 1){
		...
		var condicion3 = "AND ASN_FolioCliente = '" + ASN_Folio + "'"	
		}
		
		if (SKU == ""){
		var condicion4 = ""	
		}else{
		var condicion4 = "AND CliEnt_SKU = '" + SKU + "'"
		}
		
		if (Cli_ID == -1){
		var condicion5 = ""	
		}else{
		var condicion5 = "AND i.Cli_ID = " + Cli_ID
		}
		
		if (IR_EstatusCG52 == -1){
		var condicion6 = ""	
		}else{
		var condicion6 = "AND IR_EstatusCG52 = " + IR_EstatusCG52
		}
		if (IR_Folio == ""){
		var condicion7 = ""	
		}else{
		var condicion7 = "AND IR_Folio = '" + IR_Folio + "'"	
		}
            ...
            var sSQLRece = "SELECT TOP 1 * "
                ...
                sSQLRece += "FROM Inventario_Recepcion i"
                sSQLRece += " LEFT JOIN ASN a ON a.IR_ID= i.IR_ID"
                sSQLRece += " LEFT JOIN Cliente_OrdenCompra_Entrega c ON c.IR_ID= i.IR_ID"
                sSQLRece += " LEFT JOIN Cliente_OrdenCompra_EntregaProducto p ON  p.CliEnt_ID = c.CliEnt_ID AND p.Cli_ID=c.Cli_ID"
				sSQLRece += " LEFT JOIN Proveedor_OrdenCompra_Entrega e ON e.IR_ID= i.IR_ID"
				sSQLRece += " LEFT JOIN Proveedor_OrdenCompra_EntregaProducto pp ON pp.ProvEnt_ID= e.ProvEnt_ID AND pp.Prov_ID=e.Prov_ID"
				sSQLRece += " WHERE  "+condicion2+" "+condicion+" "  +condicion3+" " +condicion4+" " +condicion5+"  "+condicion6 + " " + condicion7
...
//			}
//                
            var sSQLRe = "SELECT * FROM (SELECT IR_Folio, IR_EstatusCG52, i.IR_ID, TA_ID, IR_Puerta , IR_FechaEntrega, i.Cli_ID, i.CliOC_ID, i.Prov_ID, i.OC_ID, IR_DescripcionVehiculo "
				+ " , IR_Conductor, IR_Placas, IR_Color"
				...
				+ " WHERE  "+condicion2+" "+condicion+" "  +condicion3+" " +condicion4+" " +condicion5+"  "+condicion6 + " " + condicion7
				+" AND IR_Habilitado = 1 "
                +  " GROUP BY IR_Folio, IR_EstatusCG52, i.IR_ID, TA_ID, IR_Puerta , IR_FechaEntrega, i.Cli_ID, i.CliOC_ID, i.Prov_ID, i.OC_ID, IR_DescripcionVehiculo "
                ...
                +  "  WHERE  IR_Habilitado = 1 AND cast (IR_FechaEntrega as date)  = '"+f+"'"
                +  " ) as t1  "
                +  " where Documentos > 0 "
                +  " ORDER BY IR_FechaEntrega DESC "
        }
...
%>          
   ...
   <%                                         
                    ...
                    while (!rsASNf.EOF){
                        Response.Write("<br>OC: " + rsASNf.Fields.Item("CliOC_NumeroOrdenCompra").Value)
                        rsASNf.MoveNext() 
                    } 
                    rsASNf.Close()   
%>
...
<% } %>
        ...
        $('.BuscarFecha').click(function(e) {
         ...
         var Params = "?Tarea=" + 1
				...
				Params += "&IR_Folio=" + $("#InputCita").val()
				Params += "&ASN_Folio=" + $("#InputASN").val()
				Params += "&SKU=" + $("#InputSKU").val()
