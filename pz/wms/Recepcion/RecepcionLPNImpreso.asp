...
<%
	...
	if (!rsPallets.EOF) {
	    ...
	    var IR_ID = rsPallets.Fields.Item("IR_ID").Value
	    ...
	    if (Cli_ID > -1) 
		{
	        ...
	        if (!rsProvCli.EOF) {
	            ...
	            Response.Write("No se encontro el cliente. Comunicarse con sistemas ")
	        }
	    } 
		else{
	        ...
	        if(!rsProvCli.EOF) {
	            ...
	            Response.Write("No se encontro el proveedor. Comunicarse con sistemas ")
	        }
	    }
	} else {
	    Response.Write("No se encontro el LPN. Comunicarse con sistemas ")
	}
	var sSQL = "SELECT IR_Folio FROM Inventario_Recepcion WHERE IR_ID= " + IR_ID
	var rsCita = AbreTabla(sSQL, 1, 0)

	if (!rsCita.EOF) {
	    var Cita = rsCita.Fields.Item("IR_Folio").Value
	} else {
	    Response.Write("No se encontro la cita. Comunicarse con sistemas ")
	}
	rsPallets.Close()
       							 ...
       							 <tr>
       					 			<td>
                                         ...
                                         <h3>   <strong><%=Pt_Color%> </strong> </h3>
  								 		...
  								 		<td >
        							    <h3> <strong>Cantidad: </strong></h3></td><td COLSPAN="2">    <h3> <strong><%=Pt_Cantidad%></strong></h3>
         					 		</td>
   </tr>
