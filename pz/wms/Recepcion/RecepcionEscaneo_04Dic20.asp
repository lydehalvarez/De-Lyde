...
<%
	...
	if(CliOC_ID > -1){
	   var sSQL = "SELECT CliOC_Folio FROM Cliente_OrdenCompra "
                + " WHERE  Cli_ID = " + Cli_ID 
                + " AND CliOC_ID = " + CliOC_ID
        
       var rsArt = AbreTabla(sSQL,1,0)
	   Folio = rsArt.Fields.Item("CliOC_Folio").Value
	}
        
    if(OC_ID > -1){
		var sSQL = "SELECT OC_Folio FROM Proveedor_OrdenCompra "
                 + " WHERE Prov_ID = " + Prov_ID 
                 + " AND OC_ID = " + OC_ID
        
        var rsArt = AbreTabla(sSQL,1,0)
	    Folio = rsArt.Fields.Item("OC_Folio").Value
	}
