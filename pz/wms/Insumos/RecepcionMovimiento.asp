...
function RecepImprime(folio){
	...
	$('.btnEliminar').click(function(e) {
		...
		datoAgenda['Tarea'] = 3
		$("#dvMovimiento").load("/pz/wms/Insumos/Insumos_MovimientoEntrada_Ajax.asp",datoAgenda)
			

		$("#Contenido").load("/pz/wms/Insumos/Insumos_MovimientoEntrada.asp")
			sTipo = "info";
...
function GuardaMovimiento(Evento){
			$("#dvMovimiento").load("/pz/wms/Insumos/Insumos_MovimientoEntrada_Ajax.asp",Evento
...
function UpdateCita(id){
	$.post("/pz/wms/Insumos/Insumos_MovimientoEntrada_Ajax.asp",{
		Tarea:3
		,IR_ID:id
...
$('#cboProducto').change(function(e) {
        e.preventDefault()
		$.get("/pz/wms/Insumos/Insumos_MovimientoEntrada_Ajax.asp",{
			Tarea:2
			,Pro_ID:$(this).val()
		}, function(data){
			if (data != "") {
